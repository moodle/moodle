    ...
    if ($edit) { // Teacher's TOC
        ...
        foreach ($chapters as $ch) {
            ...
            if ($ch->id == $chapter->id) {
                ...
                $toc .= html_writer::link(new moodle_url('view.php', array('id' => $cm->id, 'chapterid' => $ch->id)), $title, array('title' => html_entity_decode($title)));
            }
        ...
        foreach ($chapters as $ch) {
            ...
            if (!$ch->hidden) {
                ...
                if ($ch->id == $chapter->id) {
                    ...
                    $toc .= html_writer::link(new moodle_url('view.php', array('id' => $cm->id, 'chapterid' => $ch->id)), $title, array('title' => html_etity_decode($title)));
                }
