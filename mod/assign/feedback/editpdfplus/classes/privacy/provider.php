...
namespace assignfeedback_editpdfplus\privacy;
...
class provider implements metadataprovider, assignfeedback_provider, assignfeedback_user_provider {
    ...
    public static function get_metadata(collection $collection): collection {
        ...
        $collection->add_database_table('assignfeedback_editpp_quick', $quickdata, 'privacy:metadata:tablepurpose');
    ...
    public static function export_feedback_user_data(assign_plugin_request_data $exportdata) {
        ...
        $currentpath[] = get_string('privacy:path', 'assignfeedback_editpdfplus');
        $assign = $exportdata->get_assign();
        $plugin = $assign->get_plugin_by_type('assignfeedback', 'editpdfplus');
        $fileareas = $plugin->get_file_areas();
        $grade = $exportdata->get_pluginobject();
        foreach ($fileareas as $filearea => $notused) {
            writer::with_context($exportdata->get_context())
                    ->export_area_files($currentpath, 'assignfeedback_editpdfplus', $filearea, $grade->id);
        }
    }
    ...
    public static function delete_feedback_for_context(assign_plugin_request_data $requestdata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files($requestdata->get_context()->id, 'assignfeedback_editpdfplus', $filearea);
        }
    ...
    public static function delete_feedback_for_grades(assign_plugin_request_data $deletedata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files_select($deletedata->get_context()->id, 'assignfeedback_editpdfplus', $filearea, $sql, $params);
        }

        // Remove table entries.
        $DB->delete_records_select('assignfeedback_editpp_annot', "gradeid $sql", $params);
        $DB->delete_records_select('assignfeedback_editpp_rot', "gradeid $sql", $params);
        // Submission records in assignfeedback_editpp_queue will be cleaned up in a scheduled task
    }
