 ...
 * @param bool $systemmessage False for non-system messages, true for system messages.
 * @param object $cm The course module object, pass it to save a database query when we trigger the event.
 * @return int The message ID.
 * @since Moodle 2.6
 */
function chat_send_chatmessage($chatuser, $messagetext, $systemmessage = false, $cm = null) {
    global $DB;
    ...
    $message->systemmessage    = $systemmessage ? 1 : 0;
...
function chat_format_message_manually($message, $courseid, $sender, $currentuser, $chatlastrow = null) {
    ...
    if (!empty($message->systemmessage)) {
        // System event.
