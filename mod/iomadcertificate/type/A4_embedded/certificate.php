...
iomadcertificate_print_text($pdf, $x, $y + 36, 'C', 'freesans', '', 30, fullname($USER));
...
iomadcertificate_print_text($pdf, $x, $y + 92, 'C', 'freesans', '', 14,  iomadcertificate_get_date($iomadcertificate, $certrecord, $course));
iomadcertificate_print_text($pdf, $x, $y + 102, 'C', 'freeserif', '', 10, iomadcertificate_get_grade($iomadcertificate, $course));
iomadcertificate_print_text($pdf, $x, $y + 112, 'C', 'freeserif', '', 10, iomadcertificate_get_outcome($iomadcertificate, $course));
if ($iomadcertificate->printhours) {
    iomadcertificate_print_text($pdf, $x, $y + 122, 'C', 'freeserif', '', 10, get_string('credithours', 'iomadcertificate') . ': ' . $iomadcertificate->printhours);
}
...
if ($iomadcertificate->printteacher) {
    ...
    if ($teachers = get_users_by_capability($context, 'mod/iomadcertificate:printteacher', '', $sort = 'u.lastname ASC', '', '', '', '', false)) {
        foreach ($teachers as $teacher) {
            $i++;
            iomadcertificate_print_text($pdf, $sigx, $sigy + ($i * 4), 'L', 'freeserif', '', 12, fullname($teacher));
        }
    }
}
