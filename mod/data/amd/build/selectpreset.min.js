define("mod_data/selectpreset",["exports","core/notification","core/str"],(function(_exports,_notification,_str){var obj;
/**
   * Javascript module to control the form responsible for selecting a preset.
   *
   * @module      mod_data/selectpreset
   * @copyright   2021 Mihail Geshoski <mihail@moodle.com>
   * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=void 0,_notification=(obj=_notification)&&obj.__esModule?obj:{default:obj};var selectors_selectPresetButton='input[name="selectpreset"]',selectors_selectedPresetRadioButton='input[name="fullname"]:checked';_exports.init=function(){document.querySelector(selectors_selectPresetButton).addEventListener("click",(function(event){(event.preventDefault(),document.querySelectorAll(selectors_selectedPresetRadioButton).length>0)?event.target.closest("form").submit():(0,_str.get_string)("presetnotselected","mod_data").then((function(str){return _notification.default.addNotification({type:"error",message:str})})).catch(_notification.default.exception)}))}}));

//# sourceMappingURL=selectpreset.min.js.map