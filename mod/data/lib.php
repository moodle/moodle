...
function data_print_template($template, $records, $data, $search='', $page=0, $return=false) {
    ...
    // Replacing special tags (##Edit##, ##Delete##, ##More##)
        ...
        if (has_capability('mod/data:approve', $context) && ($data->approval) && (!$record->approved)){
            $replacement[] = '<span class="approve"><a href="'.$CFG->wwwroot.'/mod/data/view.php?d='.$data->id.'&amp;approve='.$record->id.'&amp;sesskey='.sesskey().'"><img src="'.$OUTPUT->pix_url('i/approve') . '" class="iconsmall" title="'.get_string('approve').'" alt="'.get_string('approve').'" /></a></span>';
        } else {
            $replacement[] = '';
        }
