...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('qbassignfeedback_comments', $data, 'privacy:metadata:tablesummary');
    ...
    public static function get_context_for_userid_within_feedback(int $userid, contextlist $contextlist) {
        // This uses the qbassign_grades table.
    }
    ...
    public static function export_feedback_user_data(qbassign_plugin_request_data $exportdata) {
        ...
        $plugin = $qbassign->get_plugin_by_type('qbassignfeedback', 'comments');
        ...
        if ($comments && !empty($comments->commenttext)) {
            $currentpath = array_merge(
                ...
                [get_string('privacy:commentpath', 'qbassignfeedback_comments')]
            );

            $comments->commenttext = writer::with_context($qbassign->get_context())->rewrite_pluginfile_urls(
                $currentpath,
            ...
            writer::with_context($exportdata->get_context())->export_area_files($currentpath,
                qbassignFEEDBACK_COMMENTS_COMPONENT, qbassignFEEDBACK_COMMENTS_FILEAREA, $gradeid);
        }
    }
    ...
    public static function delete_feedback_for_context(qbassign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, qbassignFEEDBACK_COMMENTS_COMPONENT,
            qbassignFEEDBACK_COMMENTS_FILEAREA);
    ...
    public static function delete_feedback_for_grade(qbassign_plugin_request_data $requestdata) {
        ...
        $fs = new \file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, qbassignFEEDBACK_COMMENTS_COMPONENT,
            qbassignFEEDBACK_COMMENTS_FILEAREA, $requestdata->get_pluginobject()->id);

        $DB->delete_records('qbassignfeedback_comments', ['qbassignment' => $requestdata->get_qbassignid(),
    ...
    public static function delete_feedback_for_grades(qbassign_plugin_request_data $deletedata) {
        ...
        $DB->delete_records_select('qbassignfeedback_comments', "qbassignment = :qbassignment AND grade $sql", $params);
    }
}
