 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbassignfeedback_offline_grade_importer {
    ...
    /** @var qbassignment $qbassignment - the qbassignment class */
    private $qbassignment;
     ...
     * @param qbassign $qbassignment The current qbassignment
     */
    public function __construct($importid, qbassign $qbassignment, $encoding = 'utf-8', $separator = 'comma') {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'qbassignfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'qbassign');
        $strmodified = get_string('lastmodifiedgrade', 'qbassign');

        foreach ($this->qbassignment->get_feedback_plugins() as $plugin) {
        ...
        $this->validusers = $this->qbassignment->list_participants($groupid, false);
        return true;
    }
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'qbassign')));
            if ($userid = $this->qbassignment->get_user_id_for_uniqueid($id)) {
