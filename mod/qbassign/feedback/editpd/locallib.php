...
/**
 * library class for editpd feedback plugin extending feedback plugin base class
 *
 * @package   qbassignfeedback_editpd
 * @copyright 2012 Davo Smith
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbassign_feedback_editpd extends qbassign_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'qbassignfeedback_editpd');
    }
     ...
     * @param bool $readonly
     * @return qbassignfeedback_editpd_widget
     */
    public function get_widget($userid, $grade, $readonly) {
        ...
        $asscontext = $this->qbassignment->get_context();
        ...
        $currentstamps = $fs->get_area_files($syscontext->id, 'qbassignfeedback_editpd', 'stamps', 0, 'filename', false);

        // Grade stamps are those which have been qbassigned for a specific grade item.
        ...
        $gradestamps = $fs->get_area_files($asscontext->id, 'qbassignfeedback_editpd', 'stamps', $grade->id, 'filename', false);
        ...
        // unique.
        $systemstamps = $fs->get_area_files($syscontext->id, 'qbassignfeedback_editpd', 'systemstamps', 0, 'filename', false);
        ...
        foreach ($currentstamps as $stamp) {
            ...
            $gradeitempathhash = $this->get_qbassignment_stamp_path($stamp, $grade->id);
        ...
        $widget = new qbassignfeedback_editpd_widget(
            $this->qbassignment->get_instance()->id,
            $userid,
            $attempt,
            $url,
    ...
    /**
     * Get the pathnamehash for the specified stamp if in the current qbassignment stamps.
     *
     * @param   stored_file $file
     * @param   int $gradeid
     * @return  string
     */
    protected function get_qbassignment_stamp_path(stored_file $stamp, int $gradeid): string {
        return file_storage::get_pathname_hash(
            $this->qbassignment->get_context()->id,
            'qbassignfeedback_editpd',
            'stamps',
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $renderer = $PAGE->get_renderer('qbassignfeedback_editpd');

        // Links to download the generated pdf...
        if ($attempt > -1 && page_editor::has_annotations_or_comments($grade->id, false)) {
            $html = $this->qbassignment->render_area_files('qbassignfeedback_editpd',
                                                         document_services::FINAL_PDF_FILEAREA,
                                                         $grade->id);
            $mform->addElement('static', 'editpd_files', get_string('downloadfeedback', 'qbassignfeedback_editpd'), $html);
        }

        $widget = $this->get_widget($userid, $grade, false);

        $html = $renderer->render($widget);
        $mform->addElement('static', 'editpd', get_string('editpd', 'qbassignfeedback_editpd'), $html);
        $mform->addHelpButton('editpd', 'editpd', 'qbassignfeedback_editpd');
        $mform->addElement('hidden', 'editpd_source_userid', $userid);
        $mform->setType('editpd_source_userid', PARAM_INT);
        $mform->setConstant('editpd_source_userid', $userid);
    }
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if (!empty($data->editpd_source_userid)) {
            $sourceuserid = $data->editpd_source_userid;
            // Retrieve the grade information for the source user.
            $sourcegrade = $this->qbassignment->get_user_grade($sourceuserid, true, $grade->attemptnumber);
            $pagenumbercount = document_services::page_number_for_attempt($this->qbassignment, $sourceuserid, $sourcegrade->attemptnumber);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        if (!empty($data->editpd_source_userid)) {
            ...
            if ($sourceuserid != $grade->userid) {
                page_editor::copy_drafts_from_to($this->qbassignment, $grade, $sourceuserid);
            }
        }
        if (page_editor::has_annotations_or_comments($grade->id, true)) {
            document_services::generate_feedback_document($this->qbassignment, $grade->userid, $grade->attemptnumber);
        }
    ...
    public function is_empty(stdClass $grade) {
        ...
        $comments = $DB->count_records('qbassignfeedback_editpd_cmnt', array('gradeid'=>$grade->id, 'draft'=>0));
        $annotations = $DB->count_records('qbassignfeedback_editpd_anno', array('gradeid'=>$grade->id, 'draft'=>0));
    ...
    public function delete_instance() {
        global $DB;
        $grades = $DB->get_records('qbassign_grades', array('qbassignment'=>$this->qbassignment->get_instance()->id), '', 'id');
        if ($grades) {
            list($gradeids, $params) = $DB->get_in_or_equal(array_keys($grades), SQL_PARAMS_NAMED);
            $DB->delete_records_select('qbassignfeedback_editpd_anno', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('qbassignfeedback_editpd_cmnt', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('qbassignfeedback_editpd_rot', 'gradeid ' . $gradeids, $params);
        }
        return true;
    }
    ...
    public function is_available() {
        if ($this->enabledcache === null) {
            $testpath = qbassignfeedback_editpd\pdf::test_gs_path(false);
            $this->enabledcache = ($testpath->status == qbassignfeedback_editpd\pdf::GSPATH_OK);
        }
