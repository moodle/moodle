 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class remove_orphaned_editpd_files extends adhoc_task {
    ...
    private function remove_files_and_entries(): void {
        ...
        $sql = "SELECT DISTINCT f.contextid, f.component, f.filearea, f.itemid
                  FROM {files} f
             LEFT JOIN {qbassign_grades} g ON g.id = f.itemid
                 WHERE f.component = :qbassigneditpd
               AND NOT (filearea = :stamps AND f.itemid = 0)
                   AND g.id IS NULL";
        $params = ['qbassigneditpd' => 'qbassignfeedback_editpd', 'stamps' => 'stamps'];
    ...
    private function remove_rotated_table_entries(): void {
        ...
        $rotatesql = "SELECT er.id AS erid
                        FROM {qbassignfeedback_editpd_rot} er
                   LEFT JOIN {qbassign_grades} g ON g.id = er.gradeid
                       WHERE g.id IS NULL";
        $DB->delete_records_subquery('qbassignfeedback_editpd_rot', 'id', 'erid', $rotatesql);
    }
}
