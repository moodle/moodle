...
class convert_submission extends adhoc_task {
    ...
    public function execute() {
        ...
        $submission = $DB->get_record('qbassign_submission', ['id' => $data->submissionid], '*', IGNORE_MISSING);
        if (!$submission) {
            mtrace('Submission no longer exists');
            return;
        }

        $cm = get_coursemodule_from_instance('qbassign', $submission->qbassignment, 0, false, MUST_EXIST);
        ...
        if ($submission->userid) {
            ...
            $users = [];
            $members = $qbassign->get_submission_group_members($submission->groupid, true);
        ...
        foreach ($users as $userid) {
            ...
            //
            // mod/qbassign/feedback/editpd/classes/task/convert_submissions.php@MOODLE_400_STABLE
            //
            ...
            $combineddocument = document_services::get_combined_pdf_for_attempt($qbassign, $userid, $data->submissionattempt);
            ...
            document_services::get_page_images_for_attempt($qbassign, $userid, $data->submissionattempt, false);
            document_services::get_page_images_for_attempt($qbassign, $userid, $data->submissionattempt, true);
        }
