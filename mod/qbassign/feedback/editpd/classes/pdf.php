...
require_once($CFG->dirroot.'/mod/qbassign/feedback/editpd/fpdi/autoload.php');
...
class pdf extends TcpdfFpdi {
    ...
    const BLANK_PDF = '/mod/qbassign/feedback/editpd/fixtures/blank.pdf';
    ...
    public function append_comments($allcomments) {
        ...
        $this->setHeaderData('', 0, '', get_string('commentindex', 'qbassignfeedback_editpd'));
        ...
        foreach ($allcomments as $pageno => $comments) {
            foreach ($comments as $index => $comment) {
                ...
                $label = get_string('commentlabel', 'qbassignfeedback_editpd', array('pnum' => $pageno + 1, 'cnum' => $index + 1));
    ...
    public function get_image($pageno) {
        ...
        if ($generate) {
            ...
            if (!file_exists($imagefile)) {
                $fullerror = '<pre>'.get_string('command', 'qbassignfeedback_editpd')."\n";
                $fullerror .= $command . "\n\n";
                $fullerror .= get_string('result', 'qbassignfeedback_editpd')."\n";
                $fullerror .= htmlspecialchars($result, ENT_COMPAT) . "\n\n";
                $fullerror .= get_string('output', 'qbassignfeedback_editpd')."\n";
                $fullerror .= htmlspecialchars(implode("\n", $output), ENT_COMPAT) . '</pre>';
                throw new \moodle_exception('errorgenerateimage', 'qbassignfeedback_editpd', '', $fullerror);
            }
        }
    ...
    public static function get_error_image($errorimagefolder, $pageno) {
        ...
        $pdf->copy_page();
        $pdf->add_comment(get_string('errorpdfpage', 'qbassignfeedback_editpd'), 250, 300, 200, "red");
    ...
    public static function test_gs_path($generateimage = true) {
        ...
        $testfile = $CFG->dirroot.'/mod/qbassign/feedback/editpd/tests/fixtures/testgs.pdf';
        if (!file_exists($testfile)) {
            $ret->status = self::GSPATH_NOTESTFILE;
            return $ret;
        }

        $testimagefolder = \make_temp_directory('qbassignfeedback_editpd_test');
