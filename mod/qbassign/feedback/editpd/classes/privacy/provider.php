...
use \mod_qbassign\privacy\qbassignfeedback_provider;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('qbassignfeedback_editpd_quic', $quickdata, 'privacy:metadata:tablepurpose');
    ...
    public static function export_feedback_user_data(qbassign_plugin_request_data $exportdata) {
        ...
        $currentpath[] = get_string('privacy:path', 'qbassignfeedback_editpd');
        $qbassign = $exportdata->get_qbassign();
        $plugin = $qbassign->get_plugin_by_type('qbassignfeedback', 'editpd');
        ...
        foreach ($fileareas as $filearea => $notused) {
            writer::with_context($exportdata->get_context())
                    ->export_area_files($currentpath, 'qbassignfeedback_editpd', $filearea, $grade->id);
        }
    }
    ...
    public static function delete_feedback_for_context(qbassign_plugin_request_data $requestdata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files($requestdata->get_context()->id, 'qbassignfeedback_editpd', $filearea);
        }
    ...
    public static function delete_feedback_for_grades(qbassign_plugin_request_data $deletedata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files_select($deletedata->get_context()->id, 'qbassignfeedback_editpd', $filearea, $sql, $params);
        }

        // Remove table entries.
        $DB->delete_records_select('qbassignfeedback_editpd_anno', "gradeid $sql", $params);
        $DB->delete_records_select('qbassignfeedback_editpd_cmnt', "gradeid $sql", $params);
        $DB->delete_records_select('qbassignfeedback_editpd_rot', "gradeid $sql", $params);
    }
}
