 ...
 * @category   test
 * @covers     \qbassignfeedback_editpd\document_services
...
class document_services_test extends advanced_testcase {
    ...
    public function test_save_file_saves_the_file() {
        ...
        $method = new ReflectionMethod('\qbassignfeedback_editpd\document_services', 'save_file');
        $method->setAccessible(true);

        $filearea = document_services::TMP_ROTATED_JPG_FILEAREA;
        $content = 'some random content';
        $tempfile = make_temp_directory('qbassignfeedback_editpd') . DIRECTORY_SEPARATOR . 'mock.file';
        file_put_contents($tempfile, $content);

        // Invoke the method and confirm, that the file is saved.
        $file1 = $method->invoke(null, $qbassign, $user->id, 1, $filearea, $tempfile);
        ...
        $file2 = $method->invoke(null, $qbassign, $user->id, 1, $filearea, $tempfile);
    ...
    public function test_save_rotated_image_file_saves_the_file() {
        ...
        $method = new ReflectionMethod('\qbassignfeedback_editpd\document_services', 'save_rotated_image_file');
        $method->setAccessible(true);
        ...
        // Invoke the method and confirm, that the file is saved.
        $file1 = $method->invoke(null, $qbassign, $user->id, 1, $imageresource, 'testimage.png');
        $this->assertInstanceOf('stored_file', $file1);
        $this->assertEquals(1, $DB->count_records('files', ['id' => $file1->get_id()]));

        // Invoke the method again and confirm, that exising file is returned.
        $file2 = $method->invoke(null, $qbassign, $user->id, 1, $imageresource, 'testimage.png');
        $this->assertEquals($file1->get_id(), $file2->get_id());
    }
    ...
    public function test_get_combined_document_for_attempt_rotates_image() {
        ...
        $qbassign = $this->create_instance($course, $qbassignparams);
        $student = $this->getDataGenerator()->create_user();
        $this->getDataGenerator()->enrol_user($student->id, $course->id, 'student');
        $this->setUser($student);

        $notices = [];
        $submission = $qbassign->get_user_submission($student->id, true, 1);
        ...
        document_services::get_combined_document_for_attempt($qbassign, $student->id, 1);
