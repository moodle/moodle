...
EDITOR.prototype = {
     ...
     * @type M.qbassignfeedback_editpd.edit
     * @protected
     */
    currentedit: new M.qbassignfeedback_editpd.edit(),

    /**
     * Current drawable.
     *
     * @property currentdrawable
     * @type M.qbassignfeedback_editpd.drawable|false
     * @protected
     */
     ...
     *
     * @property drawables
     * @type array(M.qbassignfeedback_editpd.drawable)
     * @protected
     */
    drawables: [],
     ...
     * @type M.qbassignfeedback_editpd.annotation
     * @protected
     */
     ...
     * @property quicklist
     * @type M.qbassignfeedback_editpd.quickcommentlist
     * @protected
     */
    quicklist: null,
    ...
    initializer: function() {
        ...
        if (link) {
            ...
            require(['mod_qbassign/grading_review_panel'], function(ReviewPanelManager) {
                var panelManager = new ReviewPanelManager();

                var panel = panelManager.getReviewPanel('qbassignfeedback_editpd');
                if (panel) {
                    panel = Y.one(panel);
                    panel.empty();
                ...
                if (!this.get('readonly')) {
                    this.quicklist = new M.qbassignfeedback_editpd.quickcommentlist(this);
                }
            }.bind(this));
    ...
    refresh_button_state: function() {
        ...
        imgurl = M.util.image_url('background_colour_' + this.currentedit.commentcolour, 'qbassignfeedback_editpd');
        ...
        imgurl = M.util.image_url('colour_' + this.currentedit.annotationcolour, 'qbassignfeedback_editpd');
        button.one('img').setAttribute('src', imgurl);

        currenttoolnode = this.get_dialogue_element(TOOLSELECTOR[this.currentedit.tool]);
        currenttoolnode.addClass('qbassignfeedback_editpd_selectedbutton');
    ...
    get_canvas_bounds: function() {
        ...
        return new M.qbassignfeedback_editpd.rect(offsetleft, offsettop, width, height);
    },

    /**
     * Called to translate from window coordinates to canvas coordinates.
     * @method get_canvas_coordinates
     * @param M.qbassignfeedback_editpd.point point in window coordinats.
     */
    get_canvas_coordinates: function(point) {
        var bounds = this.get_canvas_bounds(),
            newpoint = new M.qbassignfeedback_editpd.point(point.x - bounds.x, point.y - bounds.y);
    ...
    poll_document_conversion_status: function() {
        ...
        Y.io(AJAXBASE, {
            ...
            data: {
                ...
                qbassignmentid: this.get('qbassignmentid'),
    ...
    warning: function(message, dismissable) {
        ...
        var messageclasses = 'qbassignfeedback_editpd_warningmessages alert alert-warning';
        if (dismissable) {
            delay = 4;
            messageclasses = 'qbassignfeedback_editpd_warningmessages alert alert-info';
        }
    ...
    prepare_pages_for_display: function(data) {
        ...
        if (!data.pagecount) {
            ...
            error = new M.core.alert({message: M.util.get_string('cannotopenpdf', 'qbassignfeedback_editpd')});
            error.show();
            return;
        }
        ...
        for (i = 0; i < this.pages.length; i++) {
            for (j = 0; j < this.pages[i].comments.length; j++) {
                ...
                this.pages[i].comments[j] = new M.qbassignfeedback_editpd.comment(this,
                                                                                 comment.gradeid,
                                                                                 comment.pageno,
                                                                                 comment.x,
                                                                                 comment.y,
        ...
        if (!readonly && data.partial) {
            ...
            this.warning(M.util.get_string('partialwarning', 'qbassignfeedback_editpd'), false);
        }
    ...
    handle_response_data: function(response) {
        ...
        try {
            ...
            if (data.error) {
                ...
                new M.core.alert({
                    message: M.util.get_string('cannotopenpdf', 'qbassignfeedback_editpd'),
                    visible: true
                });
            } else {
                return data;
            ...
            new M.core.alert({
                title: M.util.get_string('cannotopenpdf', 'qbassignfeedback_editpd'),
                visible: true
            });
        }
    ...
    setup_toolbar: function() {
        ...
        picker = new M.qbassignfeedback_editpd.colourpicker({
        ...
        if (stampfiles.length <= 0) {
            ...
            picker = new M.qbassignfeedback_editpd.stamppicker({
    ...
    handle_tool_button: function(e, tool) {
        ...
        currenttoolnode = this.get_dialogue_element(TOOLSELECTOR[this.currentedit.tool]);
        currenttoolnode.removeClass('qbassignfeedback_editpd_selectedbutton');
    ...
    get_current_drawable: function() {
        ...
        if (this.currentedit.tool === 'comment') {
            comment = new M.qbassignfeedback_editpd.comment(this);
    ...
    edit_move: function(e) {
        var bounds = this.get_canvas_bounds(),
            ...
            drawingregion = this.get_dialogue_element(SELECTOR.DRAWINGREGION),
            clientpoint = new M.qbassignfeedback_editpd.point(e.clientX + canvas.get('docScrollX'),
    ...
    create_annotation: function(type, data) {
        ...
        data.editor = this;
        if (type === "line") {
            return new M.qbassignfeedback_editpd.annotationline(data);
        } else if (type === "rectangle") {
            return new M.qbassignfeedback_editpd.annotationrectangle(data);
        } else if (type === "oval") {
            return new M.qbassignfeedback_editpd.annotationoval(data);
        } else if (type === "pen") {
            return new M.qbassignfeedback_editpd.annotationpen(data);
        } else if (type === "highlight") {
            return new M.qbassignfeedback_editpd.annotationhighlight(data);
        } else if (type === "stamp") {
            return new M.qbassignfeedback_editpd.annotationstamp(data);
        }
        return false;
    },
            ...
            on: {
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        this.warning(M.util.get_string('draftchangessaved', 'qbassignfeedback_editpd'), true);
                    } catch (e) {
    ...
    open_search_comments: function(e) {
        if (!this.searchcommentswindow) {
            this.searchcommentswindow = new M.qbassignfeedback_editpd.commentsearch({
                editor: this
            });
        }
    ...
    setup_navigation: function() {
        ...
        if (options.size() <= 1) {
            for (i = 0; i < this.pages.length; i++) {
                ...
                option.setHTML(M.util.get_string('pagexofy', 'qbassignfeedback_editpd', strinfo));
...
Y.extend(EDITOR, Y.Base, EDITOR.prototype, {
    NAME: 'moodle-qbassignfeedback_editpd-editor',
    ATTRS: {
        userid: {
...
M.qbassignfeedback_editpd.editor = M.qbassignfeedback_editpd.editor || {};
 ...
 * @param {Object} params
 */
M.qbassignfeedback_editpd.editor.init = M.qbassignfeedback_editpd.editor.init || function(params) {
    M.qbassignfeedback_editpd.instance = new EDITOR(params);
    return M.qbassignfeedback_editpd.instance;
};
