...
M.mod_qbassign.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_qbassign.init_grading_table = function(Y) {
    Y.use('node', function(Y) {
        ...
        if (batchform) {
            batchform.on('submit', function(e) {
                M.util.js_pending('mod_qbassign/module.js:batch:submit');
                checkboxes = Y.all('td.c0 input');
                var selectedusers = [];
                checkboxes.each(function(node) {
                    if (node.get('checked')) {
                ...
                if (selectedusers.length == 0) {
                    alert(M.util.get_string('nousersselected', 'qbassign'));
                    e.preventDefault();
                } else {
                    ...
                    if (action.indexOf(prefix) == 0) {
                        ...
                        confirmmessage = M.util.get_string('batchoperationconfirm' + action, 'qbassignfeedback_' + plugin);
                    } else {
                        confirmmessage = M.util.get_string('batchoperationconfirm' + operation.get('value'), 'qbassign');
                    }
                    if (!confirm(confirmmessage)) {
                        M.util.js_complete('mod_qbassign/module.js:batch:submit');
                        e.preventDefault();
                    }
...
M.mod_qbassign.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_qbassign.init_plugin_summary = function(Y, subtype, type, submissionid) {
