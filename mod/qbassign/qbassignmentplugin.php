...
/**
 * This file contains the functions for qbassign_plugin abstract class
 *
 *
...
/**
 * Abstract class for qbassign_plugin (submission/feedback).
 *
 * @package   mod_qbassign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qbassign_plugin {

    /** @var qbassign $qbassignment the qbassignment record that contains the global
     *              settings for this qbassign instance
     */
    protected $qbassignment;
    /** @var string $type qbassignment plugin type */
    private $type = '';
     ...
     *
     * @param qbassign $qbassignment
     * @param string $type
     */
    public final function __construct(qbassign $qbassignment, $type) {
     ...
     * form for display in the settings page for the qbassignment.
    ...
    /**
     * The qbassignment subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'qbassignsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - qbassign_submission or qbassign_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this qbassignment.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/qbassign/' . substr($this->get_subtype(), strlen('qbassign')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('qbassignment'=>$this->qbassignment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('qbassign_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('qbassign_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->qbassignment = $this->qbassignment->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('qbassign_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            $qbassignment = $this->qbassignment->get_instance();
            if ($qbassignment) {
                $dbparams = array('qbassignment'=>$qbassignment->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('qbassign_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('qbassignment'=>$this->qbassignment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('qbassign_plugin_config', $dbparams);
     ...
     * and version.
     *
     * @param string $type The old qbassignment subtype
     * @param int $version The old qbassignment version
    ...
    /**
     * Upgrade the settings from the old qbassignment to the new one
     *
     * @param context $oldcontext The context for the old qbassignment module
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldqbassignment, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_qbassign', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade qbassign_submission or qbassign_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_qbassign', $params);
        return false;
    }
    ...
    /**
     * The qbassignment has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this qbassignment plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade qbassign_submission or qbassign_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        $qbassignment = $this->qbassignment;

        if ($this->get_subtype() == 'qbassignsubmission') {
            if ($itemid) {
                $record = $DB->get_record('qbassign_submission', array('id' => $itemid), 'userid,groupid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
                if (!empty($record->userid)) {
                    if (!$qbassignment->can_view_submission($record->userid)) {
                        return null;
                    }

                    // We only report write access for teachers.
                    $writeaccess = $qbassignment->can_grade() && $qbassignment->can_edit_submission($record->userid);
                } else {
                    // Must be a team submission with a group.
                    if (!$qbassignment->can_view_group_submission($record->groupid)) {
                        return null;
                    }
                    // We only report write access for teachers.
                    $writeaccess = $qbassignment->can_grade() && $qbassignment->can_edit_group_submission($record->groupid);
                }
            }
        } else {
    ...
    /**
     * This allows a plugin to render a page in the context of the qbassignment
     *
     * If the plugin creates a link to the qbassignment view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=qbassignfeedback|qbassignsubmission
     ...
     * right below the activity's "intro" section on the main qbassignment page.
     *
     * @return string
     */
     ...
     * enabled/disabled per qbassignment instance.
     *
     * @return bool
     */
