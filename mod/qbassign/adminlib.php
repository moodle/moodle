...
/**
 * This file contains the classes for the admin settings of the qbassign module.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbassign_admin_page_manage_qbassign_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/qbassign/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'qbassign'),
                            $url);
    }
...
class qbassign_plugin_manager {
    ...
    /**
     * Constructor for this qbassignment plugin manager
     * @param string $subtype - either qbassignsubmission or qbassignfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/qbassign/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'qbassign'),
                get_string('version'), get_string('hideshow', 'qbassign'),
        ...
        $shortsubtype = substr($this->subtype, strlen('qbassign'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/qbassign/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'qbassign'));
    }
