 ...
 * @category   test
 * @covers     \mod_qbassign\external\start_submission
 * @author     Andrew Madden <andrewmadden@catalyst-au.net>
 * @copyright  2021 Catalyst IT
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class start_submission_test extends \mod_qbassign\externallib_advanced_testcase {
    /** @var \stdClass $course New course created to hold the qbassignments */
    ...
    /**
     * Test start_submission if qbassignment doesn't exist matching id.
     */
    public function test_start_submission_with_invalid_qbassign_id() {
     ...
     */
    public function test_start_submission_when_user_has_no_capability_to_view_qbassignment() {
        $user = $this->getDataGenerator()->create_user();
        $this->setUser($user);
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_qbassign');
        $qbassign = $generator->create_instance(['course' => $this->course->id]);

        $this->expectException(\require_login_exception::class);
        start_submission::execute($qbassign->id);
    }

    /**
     * Test start_submission if qbassignment cut off date has elapsed.
     */
    public function test_start_submission_when_qbassignment_past_due_date() {
        $fiveminago = time() - 300;
        ...
        $this->assertEquals('This qbassignment is not open for submissions', $warning['item']);
    }
    ...
    public function test_start_submission_when_time_limit_disabled() {
        ...
        $warning = array_pop($filteredwarnings);
        $this->assertEquals($instance->id, $warning['itemid']);
        $this->assertEquals('Time limit is not enabled for qbassignment.', $warning['item']);
    }

    /**
     * Test start_submission if time limit is not set for qbassignment.
     */
    ...
    public function test_start_submission_when_submission_already_open() {
        ...
        $warning = array_pop($filteredwarnings);
        $this->assertEquals($instance->id, $warning['itemid']);
        $this->assertEquals('Open qbassignment submission already exists.', $warning['item']);
    }
    ...
    public function test_start_submission_with_new_submission() {
        ...
        $DB->delete_records('qbassign_submission', ['qbassignment' => $instance->id]);
        $result = start_submission::execute($instance->id);
