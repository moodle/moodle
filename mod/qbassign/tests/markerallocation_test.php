...
/**
 * Unit tests for (some of) mod/qbassign/markerallocaion_test.php.
 *
...
class markerallocation_test extends \advanced_testcase {
    ...
    public function test_markerusers() {
        ...
        $qbassignelement = $this->getDataGenerator()->create_module('qbassign', $record, $modulesettings);

        $coursesectionid = course_add_cm_to_section($this->course->id, $qbassignelement->id, 1);
        ...
        if (!empty($managerrole)) {
            // By default the context of the system is qbassigned.
            $idqbassignment = $this->getDataGenerator()->role_qbassign($managerrole->id, $user4->id);
        }
        ...
        $oldmarkers = get_users_by_capability(\context_course::instance($this->course->id), 'mod/qbassign:grade', '', $sort);
        // New code, it must return 2 users: teacher1 and teacher2.
        $newmarkers = get_enrolled_users(\context_course::instance($this->course->id), 'mod/qbassign:grade', 0, 'u.*', $sort);
