...
class locallib_test extends \advanced_testcase {
    ...
    public function test_return_links() {
        ...
        $PAGE->set_url(new \moodle_url('/mod/qbassign/view.php', ['id' => $qbassign->get_course_module()->id]));

        $qbassign->register_return_link('RETURNACTION', ['param' => 1]);
        $this->assertEquals('RETURNACTION', $qbassign->get_return_action());
        $this->assertEquals(['param' => 1], $qbassign->get_return_params());
    }
    ...
    public function test_get_feedback_plugins() {
        ...
        $qbassign = $this->create_instance($course);
        $installedplugins = array_keys(\core_component::get_plugin_list('qbassignfeedback'));

        foreach ($qbassign->get_feedback_plugins() as $plugin) {
    ...
    public function test_get_submission_plugins() {
        ...
        $this->setUser($teacher);
        $qbassign = $this->create_instance($course);
        $installedplugins = array_keys(\core_component::get_plugin_list('qbassignsubmission'));
    ...
    public function test_is_blind_marking() {
        ...
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertEquals(true, strpos($output, get_string('hiddenuser', 'qbassign')));
        ...
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertEquals(false, strpos($output, get_string('hiddenuser', 'qbassign')));
    ...
    /**
     * Data provider for test_get_qbassign_perpage
     *
     * @return array Provider data
     */
    public function get_qbassign_perpage_provider() {
        return array(
            array(
                'maxperpage' => -1,
                'userprefs' => array(
    ...
    public function test_get_qbassign_perpage($maxperpage, $userprefs) {
        ...
        set_config('maxperpage', $maxperpage, 'qbassign');
        set_user_preference('qbassign_perpage', null);
        $this->assertEquals(10, $qbassign->get_qbassign_perpage());
        foreach ($userprefs as $pref => $perpage) {
            set_user_preference('qbassign_perpage', $pref);
            $this->assertEquals($perpage, $qbassign->get_qbassign_perpage());
        }
    }
     ...
     *
     * This is specifically checking an qbassignment with no grade to make sure we do not
     * get an exception thrown when rendering the grading table for this type of qbassignment.
     */
    public function test_gradingtable_filter_by_requiresgrading_no_grade() {
        ...
        $qbassign = $this->create_instance($course, [
                ...
                'qbassignfeedback_comments_enabled' => 0,
                'grade' => GRADE_TYPE_NONE
            ]);

        $PAGE->set_url(new \moodle_url('/mod/qbassign/view.php', array(
            'id' => $qbassign->get_course_module()->id,
            'action' => 'grading',
        )));

        // Render the table with the requires grading filter.
        $gradingtable = new \qbassign_grading_table($qbassign, 1, qbassign_FILTER_REQUIRE_GRADING, 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);

        // Test that the filter function does not throw errors for qbassignments with no grade.
    ...
    public function test_gradingtable_extension_due_date() {
        ...
        // Check that the qbassignment is late.
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertStringContainsString(get_string('submissionstatus_', 'qbassign'), $output);
        $this->assertStringContainsString(get_string('overdue', 'qbassign', format_time((4 * DAYSECS))), $output);

        // Grant an extension.
        $extendedtime = $time + (2 * DAYSECS);
        $qbassign->testable_save_user_extension($student->id, $extendedtime);
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertStringContainsString(get_string('submissionstatus_', 'qbassign'), $output);
        $this->assertStringContainsString(get_string('userextensiondate', 'qbassign', userdate($extendedtime)), $output);
        ...
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertStringContainsString(get_string('submissionstatus_submitted', 'qbassign'), $output);
        $this->assertStringContainsString(get_string('userextensiondate', 'qbassign', userdate($extendedtime)), $output);
    }
    ...
    public function test_gradingtable_extension_date_calculation_for_lateness() {
        ...
        $this->assertStringContainsString(get_string('submissionstatus_', 'qbassign'), $output);
        $difftime = time() - $time;
        $this->assertStringContainsString(get_string('overdue', 'qbassign', format_time((4 * DAYSECS) + $difftime)), $output);

        // Grant an extension that is in the past.
        $qbassign->testable_save_user_extension($student->id, $time - (2 * DAYSECS));
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertStringContainsString(get_string('submissionstatus_', 'qbassign'), $output);
        $this->assertStringContainsString(get_string('userextensiondate', 'qbassign', userdate($time - (2 * DAYSECS))), $output);
        $difftime = time() - $time;
        $this->assertStringContainsString(get_string('overdue', 'qbassign', format_time((2 * DAYSECS) + $difftime)), $output);
        ...
        $this->assertStringContainsString(get_string('submittedlateshort', 'qbassign', format_time((2 * DAYSECS) + $difftime)),
            $output);
    }
    ...
    public function test_gradingtable_status_rendering() {
        ...
        // Simulate a student viewing the qbassignment without submitting.
        $this->setUser($student);
        $submission = $qbassign->get_user_submission($student->id, true);
        ...
        $this->assertEmpty($xpath->evaluate('string(//td[@id="mod_qbassign_grading-' . $qbassign->get_context()->id. '_r0_c8"])'));
    }
    ...
    public function test_gradingtable_group_submissions_rendering() {
        ...
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $student->id, true, true);

        // Check output.
        $this->setUser($teacher);
        $gradingtable = new \qbassign_grading_table($qbassign, 4, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $document = new \DOMDocument();
        @$document->loadHTML($output);
        $xpath = new \DOMXPath($document);
        ...
        $xpathuniqueidroot = 'mod_qbassign_grading-' . $qbassign->get_context()->id;

        // Check status.
        $this->assertSame(get_string('submissionstatus_submitted', 'qbassign'),
    ...
    public function test_show_intro() {
        ...
        $this->setUser($teacher);
        $qbassign = $this->create_instance($course, ['alwaysshowdescription' => 1]);

        $this->assertEquals(true, $qbassign->testable_show_intro());
        ...
        $this->assertEquals(false, $qbassign->testable_show_intro());
    ...
    public function test_has_submissions_or_grades() {
        ...
        // Should start empty.
        $this->assertEquals(false, $qbassign->has_submissions_or_grades());
        ...
        // Now test again.
        $this->assertEquals(true, $qbassign->has_submissions_or_grades());
    }
    ...
    public function test_delete_grades() {
        ...
        $gradinginfo = grade_get_grades($course->id, 'mod', 'qbassign', $qbassign->get_instance()->id);

        $this->assertNotEquals(0, count($gradinginfo->items));

        $qbassign->testable_delete_grades();
        $gradinginfo = grade_get_grades($course->id, 'mod', 'qbassign', $qbassign->get_instance()->id);
    ...
    public function test_delete_instance() {
        ...
        // Now try and delete.
        $this->setUser($teacher);
        $this->assertEquals(true, $qbassign->delete_instance());
    }
    ...
    public function test_reset_userdata() {
        ...
        // Now try and reset.
        $data = new \stdClass();
        $data->reset_qbassign_submissions = 1;
        $data->reset_gradebook_grades = 1;
        $data->reset_qbassign_user_overrides = 1;
        $data->reset_qbassign_group_overrides = 1;
        $data->courseid = $course->id;
        $data->timeshift = DAYSECS;
        $this->setUser($teacher);
        $qbassign->reset_userdata($data);
        $this->assertEquals(false, $qbassign->has_submissions_or_grades());

        // Reload the instance data.
        $instance = $DB->get_record('qbassign', array('id' => $qbassign->get_instance()->id));
        $this->assertEquals($now + DAYSECS, $instance->duedate);

        // Test reset using qbassign_reset_userdata().
        $qbassignduedate = $instance->duedate; // Keep old updated value for comparison.
        $data->timeshift = (2 * DAYSECS);
        qbassign_reset_userdata($data);
        $instance = $DB->get_record('qbassign', array('id' => $qbassign->get_instance()->id));
        $this->assertEquals($qbassignduedate + (2 * DAYSECS), $instance->duedate);

        // Create one more qbassignment and reset, make sure time shifted for previous qbassignment is not changed.
        $qbassign2 = $this->create_instance($course, [
                'qbassignsubmission_onlinetex_enabled' => 1,
                'duedate' => $now,
            ]);
        $qbassignduedate = $instance->duedate;
        $data->timeshift = 3 * DAYSECS;
        $qbassign2->reset_userdata($data);
        $instance = $DB->get_record('qbassign', array('id' => $qbassign->get_instance()->id));
        $this->assertEquals($qbassignduedate, $instance->duedate);
        $instance2 = $DB->get_record('qbassign', array('id' => $qbassign2->get_instance()->id));
        $this->assertEquals($now + 3 * DAYSECS, $instance2->duedate);

        // Reset both qbassignments using qbassign_reset_userdata() and make sure both qbassignments have same date.
        $qbassignduedate = $instance->duedate;
        $qbassign2duedate = $instance2->duedate;
        $data->timeshift = (4 * DAYSECS);
        qbassign_reset_userdata($data);
        $instance = $DB->get_record('qbassign', array('id' => $qbassign->get_instance()->id));
        $this->assertEquals($qbassignduedate + (4 * DAYSECS), $instance->duedate);
        $instance2 = $DB->get_record('qbassign', array('id' => $qbassign2->get_instance()->id));
        $this->assertEquals($qbassign2duedate + (4 * DAYSECS), $instance2->duedate);
    }
    ...
    public function test_update_calendar() {
        ...
        // Create a new \qbassignment with links to a draft area.
        $now = time();
        $qbassign = $this->create_instance($course, [
                'duedate' => $now,
                'intro' => $fakearealink1,
        ...
        $params = array('modulename' => 'qbassign', 'instance' => $qbassign->get_instance()->id);
        $event = $DB->get_record('event', $params);
        $this->assertNotEmpty($event);
        $this->assertSame('link', $event->description);     // The pluginfile links are removed.

        // Make sure the same works when updating the qbassignment.
        $instance = $qbassign->get_instance();
        ...
        // Create an qbassignment with a description that should be hidden.
        $qbassign = $this->create_instance($course, [
                'duedate' => $now + 160,
        ...
        $event = $DB->get_record('event', [
            'modulename' => 'qbassign',
            'instance' => $qbassign->get_instance()->id,
        ]);

        $this->assertEmpty($event->description);

        // Change the allowsubmissionfromdate to the past - do this directly in the DB
        // because if we call the qbassignment update method - it will update the calendar
        // and we want to test that this works from cron.
        $DB->set_field('qbassign', 'allowsubmissionsfromdate', $now - 60, array('id' => $qbassign->get_instance()->id));
    ...
    public function test_update_instance() {
        ...
        $instance->instance = $instance->id;
        $instance->qbassignsubmission_onlinetex_enabled = 1;

        $qbassign->update_instance($instance);

        $instance = $DB->get_record('qbassign', ['id' => $qbassign->get_instance()->id]);
    ...
    public function test_cannot_submit_empty() {
        ...
        // Test you cannot see the submit button for an offline qbassignment regardless.
        $this->setUser($student);
        $output = $qbassign->view_student_summary($student, true);
        $this->assertStringNotContainsString(get_string('submitqbassignment', 'qbassign'),
            $output, 'Can submit empty offline qbassignment');
    }
    ...
    public function test_cannot_submit_empty_no_submission() {
        ...
        // Test you cannot see the submit button for an online text qbassignment with no submission.
        $this->setUser($student);
        $output = $qbassign->view_student_summary($student, true);
        $this->assertStringNotContainsString(get_string('submitqbassignment', 'qbassign'),
            $output, 'Cannot submit empty onlinetex qbassignment');
    }
    ...
    public function test_can_submit_with_submission() {
        ...
        // Test you can see the submit button for an online text qbassignment with a submission.
        $this->setUser($student);
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $student);
        $this->assertStringContainsString(get_string('submitqbassignment', 'qbassign'),
            $output, 'Can submit non empty onlinetex qbassignment');
    }
    ...
    public function test_new_submission_empty($data, $expected) {
        ...
        if ($data['onlinetex']) {
            $submission->onlinetex_editor = ['text' => $data['onlinetex']];
        }
        ...
        return [
            'With file and onlinetex' => [
                [
                    ...
                    'onlinetex' => 'Balin Fundinul Uzbadkhazaddumu'
                ],
                false
            ]
        ];
    ...
    public function test_list_participants() {
        ...
        $this->setUser($teacher);
        $qbassign = $this->create_instance($course, ['grade' => 100]);

        $this->assertCount(10, $qbassign->list_participants(null, true));
    }
    ...
    public function test_list_participants_with_group_restriction() {
        ...
        $this->assertEquals(2, count($qbassign->list_participants(null, true)));
    }
    ...
    public function test_get_participant_user_not_exist() {
        ...
        $qbassign = $this->create_instance($course);
        $this->assertNull($qbassign->get_participant('-1'));
    }
    ...
    public function test_get_participant_not_enrolled() {
        ...
        $this->assertNull($qbassign->get_participant($user->id));
    }
    ...
    public function test_get_participant_no_submission() {
        ...
        $participant = $qbassign->get_participant($student->id);
    ...
    public function test_get_participant_granted_extension() {
        ...
        $this->setUser($teacher);
        $qbassign->save_user_extension($student->id, time());
        $participant = $qbassign->get_participant($student->id);
    ...
    public function test_get_participant_with_no_submission_no_capability() {
        ...
        qbassign_capability('mod/qbassign:submit', CAP_PROHIBIT, $role, $coursecontext);
    ...
    public function test_count_teams_no_active_non_group_allowed() {
        ...
        $this->setUser($teacher);
        $qbassign = $this->create_instance($course, ['teamsubmission' => 1]);

        $this->assertEquals(2, $qbassign->count_teams());
    }
    ...
    public function test_count_teams_non_group_allowed() {
        ...
        $this->setUser($teacher);
        $this->assertEquals(3, $qbassign->count_teams());

        // Active group only.
        $this->assertEquals(1, $qbassign->count_teams($group1->id));
        $this->assertEquals(1, $qbassign->count_teams($group2->id));
    }
    ...
    public function test_submit_to_default_group() {
        ...
        $usergroup = $qbassign->get_submission_group($student->id);
        ...
        $this->assertEquals(1, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED));

        // Set an active group.
        $SESSION->activegroup[$course->id]['aag'][0] = (int) $group->id;
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED));
    }
    ...
    public function test_count_submissions_no_draft() {
        ...
        $qbassign->get_user_submission($student->id, true);

        // Note: Drafts count as a submission.
        $this->assertEquals(0, $qbassign->count_grades());
        $this->assertEquals(0, $qbassign->count_submissions());
        $this->assertEquals(1, $qbassign->count_submissions(true));
        $this->assertEquals(0, $qbassign->count_submissions_need_grading());
        $this->assertEquals(1, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_NEW));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_DRAFT));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_REOPENED));
    }
    ...
    public function test_count_submissions_draft() {
        ...
        // Note: Drafts count as a submission.
        $this->assertEquals(0, $qbassign->count_grades());
        $this->assertEquals(1, $qbassign->count_submissions());
        $this->assertEquals(1, $qbassign->count_submissions(true));
        $this->assertEquals(0, $qbassign->count_submissions_need_grading());
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_NEW));
        $this->assertEquals(1, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_DRAFT));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_REOPENED));
    }
    ...
    public function test_count_submissions_submitted() {
        ...
        $this->assertEquals(1, $qbassign->count_submissions());
        $this->assertEquals(1, $qbassign->count_submissions(true));
        $this->assertEquals(1, $qbassign->count_submissions_need_grading());
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_NEW));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_DRAFT));
        $this->assertEquals(1, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED));
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_REOPENED));
    ...
    public function test_count_submissions_graded() {
        ...
        $this->assertEquals(1, $qbassign->count_grades());
        $this->assertEquals(1, $qbassign->count_submissions());
        $this->assertEquals(1, $qbassign->count_submissions(true));
        $this->assertEquals(0, $qbassign->count_submissions_need_grading());
        $this->assertEquals(0, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_NEW));
    ...
    public function x_test_count_submissions_for_team() {
        ...
        $data = new \stdClass();
        $data->grade = '50.0';
        $qbassign->testable_apply_grade_to_user($data, $this->extrastudents[0]->id, 0);

        // Simulate a submission.
        $this->setUser($this->extrastudents[1]);
        $submission = $qbassign->get_group_submission($this->extrastudents[1]->id, $groupid, true);
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $this->extrastudents[1]->id, true, false);
        $data = new \stdClass();
        $data->onlinetex_editor = array(
            'itemid' => file_get_unused_draft_itemid(),
            'text' => 'Submission text',
        ...
        $submission = $qbassign->get_group_submission($this->extrastudents[2]->id, $groupid, true);
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $this->extrastudents[2]->id, true, false);
        $data = new \stdClass();
        $data->onlinetex_editor = array(
            'itemid' => file_get_unused_draft_itemid(),
            'text' => 'Submission text',
        ...
        $submission = $qbassign->get_group_submission($this->extrastudents[3]->id, $groupid, true);
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $this->extrastudents[3]->id, true, false);
        $data = new \stdClass();
        $data->onlinetex_editor = array(
            'itemid' => file_get_unused_draft_itemid(),
            'text' => 'Submission text',
        ...
        $data = new \stdClass();
        $data->grade = '50.0';
        $qbassign->testable_apply_grade_to_user($data, $this->extrastudents[3]->id, 0);
        $qbassign->testable_apply_grade_to_user($data, $this->extrasuspendedstudents[0]->id, 0);

        // Create a new submission with status NEW.
        $this->setUser($this->extrastudents[4]);
        $submission = $qbassign->get_group_submission($this->extrastudents[4]->id, $groupid, true);

        $this->assertEquals(2, $qbassign->count_grades());
        $this->assertEquals(4, $qbassign->count_submissions());
        $this->assertEquals(5, $qbassign->count_submissions(true));
        $this->assertEquals(3, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED));
        $this->assertEquals(1, $qbassign->count_submissions_with_status(qbassign_SUBMISSION_STATUS_DRAFT));
    }
    ...
    public function test_get_grading_userid_list_only_active() {
        ...
        $qbassign = $this->create_instance($course);
        $this->assertCount(1, $qbassign->testable_get_grading_userid_list());
    }
    ...
    public function test_get_grading_userid_list_all() {
        ...
        $qbassign = $this->create_instance($course);
        $this->assertCount(2, $qbassign->testable_get_grading_userid_list());
    }
    ...
    public function test_cron() {
        ...
        // Now create an qbassignment and add some feedback.
        $this->setUser($teacher);
        $qbassign = $this->create_instance($course, [
        ...
        $this->expectOutputRegex('/Done processing 1 qbassignment submissions/');
        cron_setup_user();
        ...
        $this->assertEquals($qbassign->get_instance()->name, $messages[0]->contexturlname);
        // Test customdata.
        $customdata = json_decode($messages[0]->customdata);
        $this->assertEquals($qbassign->get_course_module()->id, $customdata->cmid);
        $this->assertEquals($qbassign->get_instance()->id, $customdata->instance);
    ...
    public function test_markingworkflow_cron() {
        ...
        $this->mark_submission($teacher, $qbassign, $student, 50.0, [
            'sendstudentnotifications' => 1,
            'workflowstate' => qbassign_MARKING_WORKFLOW_STATE_READYFORRELEASE,
        ]);
        ...
        $this->setUser($teacher);
        $submission = $qbassign->get_user_submission($student->id, true);
        $submission->workflowstate = qbassign_MARKING_WORKFLOW_STATE_RELEASED;
        $qbassign->testable_apply_grade_to_user($submission, $student->id, 0);
    ...
    public function test_cron_message_includes_courseid() {
        ...
        $this->add_submission($student, $qbassign);
        $this->submit_for_grading($student, $qbassign);
        $this->mark_submission($teacher, $qbassign, $student);
        ...
        $this->assertEquals($qbassign->get_course()->id, $event->other['courseid']);
        $sink->close();
    }
    ...
    public function test_is_graded() {
        ...
        $this->setUser($teacher);
        $this->assertEquals(true, $qbassign->testable_is_graded($student->id));
        $this->assertEquals(false, $qbassign->testable_is_graded($otherstudent->id));
    }
    ...
    public function test_can_grade() {
        ...
        $this->setUser($student);
        $this->assertEquals(false, $qbassign->can_grade());

        $this->setUser($teacher);
        $this->assertEquals(true, $qbassign->can_grade());

        // Test the viewgrades capability for other users.
        $this->setUser();
        $this->assertTrue($qbassign->can_grade($teacher->id));
        $this->assertFalse($qbassign->can_grade($student->id));

        // Test the viewgrades capability - without mod/qbassign:grade.
        $this->setUser($student);

        $studentrole = $DB->get_record('role', array('shortname' => 'student'));
        qbassign_capability('mod/qbassign:viewgrades', CAP_ALLOW, $studentrole->id, $qbassign->get_context()->id);
        $this->assertEquals(false, $qbassign->can_grade());
    }
    ...
    public function test_can_view_submission() {
        ...
        $this->setUser($student);
        $this->assertEquals(true, $qbassign->can_view_submission($student->id));
        $this->assertEquals(false, $qbassign->can_view_submission($otherstudent->id));
        $this->assertEquals(false, $qbassign->can_view_submission($teacher->id));

        $this->setUser($teacher);
        $this->assertEquals(true, $qbassign->can_view_submission($student->id));
        $this->assertEquals(true, $qbassign->can_view_submission($otherstudent->id));
        $this->assertEquals(true, $qbassign->can_view_submission($teacher->id));
        $this->assertEquals(false, $qbassign->can_view_submission($suspendedstudent->id));

        $this->setUser($editingteacher);
        $this->assertEquals(true, $qbassign->can_view_submission($student->id));
        $this->assertEquals(true, $qbassign->can_view_submission($otherstudent->id));
        $this->assertEquals(true, $qbassign->can_view_submission($teacher->id));
        $this->assertEquals(true, $qbassign->can_view_submission($suspendedstudent->id));
    ...
    public function test_update_submission() {
        ...
        $gradinginfo = grade_get_grades($course->id, 'mod', 'qbassign', $qbassign->get_instance()->id, $student->id);
    ...
    public function test_update_submission_team() {
        ...
        $gradinginfo = grade_get_grades($course->id, 'mod', 'qbassign', $qbassign->get_instance()->id, $otherstudent->id);
    ...
    public function test_group_submissions_submit_for_marking_requireallteammemberssubmit() {
        ...
        // Now verify group qbassignments.
        $this->setUser($teacher);
        $PAGE->set_url(new \moodle_url('/mod/qbassign/view.php', ['id' => $qbassign->get_course_module()->id]));
        ...
        $this->setUser($student);
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $student);
        $this->assertStringContainsString(get_string('submitqbassignment', 'qbassign'), $output);
        ...
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $student);
        $this->assertStringNotContainsString(get_string('submitqbassignment', 'qbassign'), $output);

        // Change to another user in the same group.
        $this->setUser($otherstudent);
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $otherstudent);
        $this->assertStringContainsString(get_string('submitqbassignment', 'qbassign'), $output);

        $submission = $qbassign->get_group_submission($otherstudent->id, 0, true);
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $otherstudent->id, true, true);
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $otherstudent);
        $this->assertStringNotContainsString(get_string('submitqbassignment', 'qbassign'), $output);
    }
    ...
    public function test_group_submissions_submit_for_marking() {
        ...
        $this->assertStringContainsString(get_string('submitqbassignment', 'qbassign'), $output);
        $output = $qbassign->view_student_summary($student, true);
        $this->assertStringContainsString(get_string('timeremaining', 'qbassign'), $output);
        $difftime = time() - $time;
        $this->assertStringContainsString(get_string('overdue', 'qbassign', format_time((2 * DAYSECS) + $difftime)), $output);
        ...
        $output = $qbassign->view_student_summary($otherstudent, true);
        $this->assertStringContainsString(get_string('timeremaining', 'qbassign'), $output);
        $difftime = time() - $time;
        $this->assertStringContainsString(get_string('submittedlate', 'qbassign', format_time((2 * DAYSECS) + $difftime)), $output);
    ...
    public function test_submissions_open() {
        ...
        $qbassign = $this->create_instance($course);
        $this->assertEquals(true, $qbassign->testable_submissions_open($student->id));

        $qbassign = $this->create_instance($course, ['duedate' => $tomorrow]);
        $this->assertEquals(true, $qbassign->testable_submissions_open($student->id));

        $qbassign = $this->create_instance($course, ['duedate' => $yesterday]);
        $this->assertEquals(true, $qbassign->testable_submissions_open($student->id));

        $qbassign = $this->create_instance($course, ['duedate' => $yesterday, 'cutoffdate' => $tomorrow]);
        $this->assertEquals(true, $qbassign->testable_submissions_open($student->id));

        $qbassign = $this->create_instance($course, ['duedate' => $yesterday, 'cutoffdate' => $yesterday]);
        $this->assertEquals(false, $qbassign->testable_submissions_open($student->id));
    ...
    public function test_get_graders() {
        ...
        // Create an qbassignment with no groups.
        $qbassign = $this->create_instance($course);
        $this->assertCount(2, $qbassign->testable_get_graders($student->id));
    }
    ...
    public function test_get_graders_separate_groups() {
        ...
        // Force create an qbassignment with SEPARATEGROUPS.
        $group = $this->getDataGenerator()->create_group(['courseid' => $course->id]);
        $grouping = $this->getDataGenerator()->create_grouping(array('courseid' => $course->id));
        ...
        $this->assertCount(4, $qbassign->testable_get_graders($student->id));
        ...
        $this->assertCount(4, $qbassign->testable_get_graders($otherstudent->id));
    }
    ...
    public function test_get_notified_users() {
        ...
        $capability = 'mod/qbassign:receivegradernotifications';
        $coursecontext = \context_course::instance($course->id);
        ...
        $this->assertCount(3, $qbassign->testable_get_notifiable_users($student->id));

        // Change nonediting teachers role to not receive grader notifications.
        qbassign_capability($capability, CAP_PROHIBIT, $role->id, $coursecontext);

        // Only the editing teachers will be returned.
        $this->assertCount(1, $qbassign->testable_get_notifiable_users($student->id));

        // Note the second student is in a group that is not in the grouping.
        // This means that we get all graders that are not in a group in the grouping.
        $this->assertCount(1, $qbassign->testable_get_notifiable_users($otherstudent->id));
    }
    ...
    public function test_get_notified_users_in_grouping() {
        ...
        $this->setUser($student);
        $this->assertCount(2, $qbassign->testable_get_notifiable_users($student->id));
        ...
        $this->assertCount(0, $qbassign->testable_get_notifiable_users($otherstudent->id));
    }
    ...
    public function test_group_members_only() {
        ...
        $this->setUser($teacher);
        $participants = $qbassign->list_participants(0, true);
    ...
    public function test_get_uniqueid_for_user() {
        ...
        foreach ($students as $student) {
            ...
            $this->assertEquals($student->id, $qbassign->get_user_id_for_uniqueid($uniqueid));
        }
    }
    ...
    public function test_show_student_summary() {
        ...
        $gradeitem = new \grade_item(array(
            'itemtype'      => 'mod',
            'itemmodule'    => 'qbassign',
            'iteminstance'  => $qbassign->get_instance()->id,
            'courseid'      => $course->id));
    ...
    public function test_show_student_summary_with_feedback() {
        ...
        $this->add_submission($student, $qbassign);
        $this->submit_for_grading($student, $qbassign);
        $this->mark_submission($teacher, $qbassign, $student, null, [
    ...
    public function test_attempt_reopen_method_manual() {
        ...
        $this->setUser($student);
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $student);
        $this->assertNotEquals(false, strpos($output, get_string('addsubmission', 'qbassign')));
        ...
        $output = $qbassign->view_student_summary($student, true);
        $this->assertEquals(false, strpos($output, get_string('addsubmission', 'qbassign')));
        ...
        $teacher->ignoresesskey = true;
        $this->setUser($teacher);
        $result = $qbassign->testable_process_add_attempt($student->id);
        ...
        $this->assertNotEquals(false, strpos($output, get_string('attempthistory', 'qbassign')));

        // Check that the student now does not have a button for Submit.
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $student);
        $this->assertEquals(false, strpos($output, get_string('submitqbassignment', 'qbassign')));

        // Check that the student now has a button for Add a new attempt".
        $this->assertNotEquals(false, strpos($output, get_string('addnewattempt', 'qbassign')));
        ...
        $gradingtable = new \qbassign_grading_table($qbassign, 100, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertEquals(true, strpos($output, $student->lastname));

        // Should be 1 not 2.
        $this->assertEquals(1, $qbassign->count_submissions());
        $this->assertEquals(1, $qbassign->count_submissions_with_status('reopened'));
        $this->assertEquals(0, $qbassign->count_submissions_need_grading());
        $this->assertEquals(1, $qbassign->count_grades());

        // Change max attempts to unlimited.
        $formdata = clone($qbassign->get_instance());
        $formdata->maxattempts = qbassign_UNLIMITED_ATTEMPTS;
        $formdata->instance = $formdata->id;
        $qbassign->update_instance($formdata);

        // Mark the submission again.
        $this->mark_submission($teacher, $qbassign, $student, 60.0, [], 1);

        // Check the grade exists.
        $this->setUser($teacher);
        $grades = $qbassign->get_user_grades_for_gradebook($student->id);
    ...
    public function test_attempt_reopen_method_untilpass() {
        ...
        $qbassign = $this->create_instance($course, [
            'attemptreopenmethod' => qbassign_ATTEMPT_REOPEN_METHOD_UNTILPASS,
            'maxattempts' => 3,
            'submissiondrafts' => 1,
            'qbassignsubmission_onlinetex_enabled' => 1,
        ]);
        ...
        $gradeitem = $qbassign->get_grade_item();
        ...
        $output = $qbassign->view_student_summary($student, true);
        $this->assertEquals(false, strpos($output, get_string('addnewattempt', 'qbassign')));
        ...
        $this->mark_submission($teacher, $qbassign, $student, 80.0);
        ...
        $this->mark_submission($teacher, $qbassign, $student, 40.0, [], 1);

        // Check that the student now has a button for Add a new attempt.
        $this->setUser($student);
        $output = $qbassign->view_submission_action_bar($qbassign->get_instance(), $student);
        $this->assertMatchesRegularExpression('/' . get_string('addnewattempt', 'qbassign') . '/', $output);
        $this->assertNotEquals(false, strpos($output, get_string('addnewattempt', 'qbassign')));
    }
    ...
    public function test_attempt_reopen_method_untilpass_passing() {
        ...
        // Mark the submission as passing.
        $this->mark_submission($teacher, $qbassign, $student, 100.0);
    ...
    public function test_attempt_reopen_method_untilpass_no_passing_requirement() {
        ...
        $this->mark_submission($teacher, $qbassign, $student, 0.0);
    ...
    public function test_markingworkflow() {
        ...
        $this->mark_submission($teacher, $qbassign, $student, 50.0,  [
            'workflowstate' => qbassign_MARKING_WORKFLOW_STATE_INMARKING,
        ]);
        ...
        $this->mark_submission($teacher, $qbassign, $student, 50.0,  [
            'workflowstate' => qbassign_MARKING_WORKFLOW_STATE_READYFORREVIEW,
        ]);
        ...
        $this->mark_submission($teacher, $qbassign, $student, 50.0,  [
            'workflowstate' => qbassign_MARKING_WORKFLOW_STATE_INREVIEW,
        ]);
        ...
        $this->mark_submission($teacher, $qbassign, $student, 50.0,  [
            'workflowstate' => qbassign_MARKING_WORKFLOW_STATE_RELEASED,
        ]);
    ...
    public function test_markerallocation() {
        ...
        $this->setUser($teacher);
        $users = $qbassign->list_participants(0, true);
    ...
    public function test_teacher_submit_for_student() {
        ...
        $this->add_submission($student, $qbassign, 'Student submission text');
        ...
        $this->add_submission($student, $qbassign, 'Teacher edited submission text', false);
    }
    ...
    public function test_teacher_submit_for_student_with_capability() {
        ...
        qbassign_capability('mod/qbassign:editothersubmission', CAP_ALLOW, $roleid, $qbassign->get_context()->id);
        role_qbassign($roleid, $teacher->id, $qbassign->get_context()->id);
        ...
        $this->setUser($teacher);
        $this->submit_for_grading($student, $qbassign, [], false);
        ...
        $output = $qbassign->view_student_summary($student, true);
        $this->assertStringNotContainsString(get_string('addsubmission', 'qbassign'), $output);
    ...
    public function test_disable_submit_after_cutoff_date() {
        ...
        $this->setUser($student);
        $output = $qbassign->view_student_summary($student, true);
        $this->assertStringNotContainsString($output, get_string('editsubmission', 'qbassign'),
            'Should not be able to edit after cutoff date.');
        $this->assertStringNotContainsString($output, get_string('submitqbassignment', 'qbassign'),
    ...
    public function test_submission_comment_plugin_settings($globalenabled, $instanceconfig, $isenabled) {
        ...
        $qbassign = $this->create_instance($course, $instanceconfig);
    ...
    public function submission_plugin_settings_provider() {
        return [
            ...
            'CFG->usecomments true, config enabled => Comments enabled' => [
                true,
                [
                    'qbassignsubmission_comments_enabled' => 1,
                ],
                true,
            ],
            'CFG->usecomments true, config idisabled => Comments enabled' => [
                true,
                [
                    'qbassignsubmission_comments_enabled' => 0,
                ],
                true,
            ],
...
";
        ...
        // Add a submission but don't submit now.
        $this->add_submission($student, $qbassign, $sourcetext);
        ...
        $pagination = [
            ...
            'rownum' => 0,
            'last' => true,
            'useridlistid' => $qbassign->get_useridlist_key_id(),
            'attemptnumber' => 0,
        ];
        $formparams = array($qbassign, $data, $pagination);
        $mform = new mod_qbassign_grade_form(null, [$qbassign, $data, $pagination]);

        // We need to get the URL these will be transformed to.
        $context = \context_user::instance($USER->id);
        $itemid = $data->qbassignfeedbackcomments_editor['itemid'];
...
";
        ...
        $this->assertEquals($filteredtext, $data->qbassignfeedbackcomments_editor['text']);
    }
    ...
    public function test_is_gradebook_feedback_enabled() {
        ...
        // Create qbassignment with gradebook feedback enabled and grade = 0.
        $qbassign = $this->create_instance($course, [
        ...
        $gradebookplugintype = str_replace('qbassignfeedback_', '', $gradebookplugin);
        $plugin = $qbassign->get_feedback_plugin_by_type($gradebookplugintype);
        $this->assertEquals(1, $plugin->is_enabled('enabled'));
        $this->assertEquals(1, $qbassign->is_gradebook_feedback_enabled());
    }
    ...
    public function test_is_gradebook_feedback_disabled() {
        ...
        // Create qbassignment with gradebook feedback disabled and grade = 0.
        $qbassign = $this->create_instance($course, [
    ...
    public function test_can_edit_submission() {
        ...
        $this->assertTrue($qbassign->can_edit_submission($student->id, $student->id));

        // Check student cannot edit others submission.
        $this->assertFalse($qbassign->can_edit_submission($otherstudent->id, $student->id));

        // Check teacher cannot (by default) edit a students submission.
        $this->assertFalse($qbassign->can_edit_submission($student->id, $teacher->id));
    }
    ...
    public function test_can_edit_submission_with_editothersubmission() {
        ...
        $this->assertTrue($qbassign->can_edit_submission($student->id, $teacher->id));
    }
    ...
    public function test_can_edit_submission_separategroups() {
        ...
        $this->assertTrue($qbassign->can_edit_submission($student1->id, $student1->id));
        $this->assertFalse($qbassign->can_edit_submission($student2->id, $student1->id));
        $this->assertFalse($qbassign->can_edit_submission($student3->id, $student1->id));
        $this->assertFalse($qbassign->can_edit_submission($student4->id, $student1->id));
    }
    ...
    public function test_can_edit_submission_separategroups_with_editothersubmission() {
        ...
        // Add the capability to the new \qbassignment for student 1.
        $roleid = create_role('Dummy role', 'dummyrole', 'dummy role description');
        qbassign_capability('mod/qbassign:editothersubmission', CAP_ALLOW, $roleid, $qbassign->get_context()->id);
        role_qbassign($roleid, $student1->id, $qbassign->get_context()->id);
        accesslib_clear_all_caches_for_unit_testing();

        // Verify student1 has the ability to edit submissions for other users in their group, but not other groups.
        $this->assertTrue($qbassign->can_edit_submission($student1->id, $student1->id));
        $this->assertTrue($qbassign->can_edit_submission($student2->id, $student1->id));
        $this->assertFalse($qbassign->can_edit_submission($student3->id, $student1->id));
        $this->assertFalse($qbassign->can_edit_submission($student4->id, $student1->id));

        // Verify other students do not have the ability to edit submissions for other users.
        $this->assertTrue($qbassign->can_edit_submission($student2->id, $student2->id));
        $this->assertFalse($qbassign->can_edit_submission($student1->id, $student2->id));
        $this->assertFalse($qbassign->can_edit_submission($student3->id, $student2->id));
        $this->assertFalse($qbassign->can_edit_submission($student4->id, $student2->id));
    }
    ...
    public function test_can_view_blind_details() {
        ...
        $this->assertTrue($qbassign->is_blind_marking());

        // Test student names are hidden to teacher.
        $this->setUser($teacher);
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertEquals(true, strpos($output, get_string('hiddenuser', 'qbassign')));    // "Participant" is somewhere on the page.
    ...
    public function test_get_shared_group_members() {
        ...
        $groupmembers = $qbassign->get_shared_group_members($cm, $student1->id);
        ...
        $groupmembers = $qbassign->get_shared_group_members($cm, $student2->id);
        $this->assertCount(2, $groupmembers);
        $this->assertContainsEquals($student1->id, $groupmembers);
        $this->assertContainsEquals($student2->id, $groupmembers);

        $groupmembers = $qbassign->get_shared_group_members($cm, $student3->id);
        $this->assertCount(2, $groupmembers);
        $this->assertContainsEquals($student3->id, $groupmembers);
        $this->assertContainsEquals($student4->id, $groupmembers);

        $groupmembers = $qbassign->get_shared_group_members($cm, $student4->id);
        $this->assertCount(2, $groupmembers);
        $this->assertContainsEquals($student3->id, $groupmembers);
        $this->assertContainsEquals($student4->id, $groupmembers);
    }
    ...
    public function test_get_shared_group_members_override() {
        ...
        qbassign_capability('moodle/site:accessallgroups', CAP_ALLOW, $roleid, $qbassign->get_context()->id);
        role_qbassign($roleid, $student1->id, $qbassign->get_context()->id);
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_get_plugins_file_areas() {
        ...
        $usingfilearea = 0;
        $coreplugins = \core_plugin_manager::standard_plugins_list('qbassignsubmission');
        foreach ($qbassign->get_submission_plugins() as $plugin) {
            ...
            if ($type == 'onlinetex') {
                $this->assertEquals(array('submissions_onlinetex' => 'Online text'), $fileareas);
                $usingfilearea++;
            } else if ($type == 'file') {
        ...
        $usingfilearea = 0;
        $coreplugins = \core_plugin_manager::standard_plugins_list('qbassignfeedback');
        foreach ($qbassign->get_feedback_plugins() as $plugin) {
            $type = $plugin->get_type();
            if (!in_array($type, $coreplugins)) {
                continue;
     ...
     *
     * This function needs to obey the group override logic as per the qbassign grading table and
     * the overview block.
     */
    public function test_override_exists() {
        ...
        $this->assertEquals($overrides[0], $qbassign->override_exists($student1->id));

        // User only in group 2 should see the group 2 override.
        $this->assertEquals($overrides[1], $qbassign->override_exists($student2->id));

        // User only in both groups with an override should see the user override as it has higher priority.
        $this->assertEquals($overrides[2], $qbassign->override_exists($student3->id));

        // User only in both groups with no override should see the group 1 override as it has higher priority.
        $this->assertEquals($overrides[0], $qbassign->override_exists($student4->id));

        // User with no overrides shoudl get nothing.
        $override = $qbassign->override_exists($student5->id);
    ...
    public function test_process_save_quick_grades() {
        ...
        $result = $qbassign->testable_process_save_quick_grades($data);
        $this->assertStringContainsString(get_string('quickgradingchangessaved', 'qbassign'), $result);
        ...
        $qbassign->testable_process_add_attempt($student->id);
        $data = array(
            'grademodified_' . $student->id => $grade->timemodified,
            'gradeattempt_' . $student->id => $grade->attemptnumber,
        ...
        $result = $qbassign->testable_process_save_quick_grades($data);
        $this->assertStringContainsString(get_string('errorrecordmodified', 'qbassign'), $result);
        $grade = $qbassign->get_user_grade($student->id, false);
        ...
        $result = $qbassign->testable_process_save_quick_grades($pastdata);
        $this->assertStringContainsString(get_string('errorrecordmodified', 'qbassign'), $result);
        $grade = $qbassign->get_user_grade($student->id, false);
        $this->assertEquals(30.0, $grade->grade);
    }
    ...
    public function test_update_activity_completion_records_solitary_submission() {
        ...
        // Submit the qbassignment as the student.
        $this->add_submission($student, $qbassign);
        ...
        $submission = $qbassign->get_user_submission($student->id, true);
        $qbassign->testable_update_activity_completion_records(0, 0, $submission,
    ...
    public function test_update_activity_completion_records_team_submission() {
        ...
        $this->add_submission($student, $qbassign);
        $this->submit_for_grading($student, $qbassign, ['groupid' => $group1->id]);
        ...
        $qbassign->testable_update_activity_completion_records(1, 0, $submission, $student->id, COMPLETION_COMPLETE, $completion);
     ...
     * @param number $grade The grade we should set in the qbassign grading table.
    ...
    public function test_fix_null_grades($grade, $gradebookvalue) {
        ...
        $qbassign->get_user_grade($student->id, true);
        ...
        $DB->set_field(
            ...
            [
                'userid' => $student->id,
                'qbassignment' => $qbassign->get_instance()->id,
            ]
        );
        $qbassign->grade = $grade;
        $qbassigntemp = clone $qbassign->get_instance();
        $qbassigntemp->cmidnumber = $qbassign->get_course_module()->idnumber;
        qbassign_update_grades($qbassigntemp);

        // Check that the gradebook was updated with the qbassign grade. So we can guarentee test results later on.
        $expectedgrade = $grade == -1 ? null : $grade; // qbassign sends null to the gradebook for -1 grades.
        $gradegrade = \grade_grade::fetch(array('userid' => $student->id, 'itemid' => $qbassign->get_grade_item()->id));
        $this->assertEquals(-1, $gradegrade->usermodified);
        $this->assertEquals($expectedgrade, $gradegrade->rawgrade);

        // Call fix_null_grades().
        $method = new \ReflectionMethod(\qbassign::class, 'fix_null_grades');
        $method->setAccessible(true);
        $result = $method->invoke($qbassign);
    ...
    public function test_grade_submission_override() {
        ...
        $gradegrade = \grade_grade::fetch([
            'userid' => $student->id,
            'itemid' => $qbassign->get_grade_item()->id,
        ]);
        ...
        // Check that submissionslocked message 'This qbassignment is not accepting submissions' does not appear for student.
        $gradingtable = new \qbassign_grading_table($qbassign, 1, '', 0, true);
        $output = $qbassign->get_renderer()->render($gradingtable);
        $this->assertStringContainsString(get_string('submissionstatus_', 'qbassign'), $output);

        $qbassignsubmissionstatus = $qbassign->get_qbassign_submission_status_renderable($student, true);
        $output2 = $qbassign->get_renderer()->render($qbassignsubmissionstatus);

        // Check that submissionslocked 'This qbassignment is not accepting submissions' message does not appear for student.
        $this->assertStringNotContainsString(get_string('submissionslocked', 'qbassign'), $output2);
        // Check that submissionstatus_marked 'Graded' message does appear for student.
        $this->assertStringContainsString(get_string('submissionstatus_marked', 'qbassign'), $output2);
    }
    ...
    public function test_get_filters() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $qbassign = $this->create_instance($course);
        $valid = $qbassign->get_filters();

        $this->assertEquals(count($valid), 6);
    }

    /**
     * Test qbassign->get_instance() for a number of cases, as defined in the data provider.
     *
     * @dataProvider qbassign_get_instance_provider
     ...
     * @param array $expectedproperties an map containing the expected names and values for the qbassign instance data.
     */
    public function test_qbassign_get_instance(array $courseconfig, array $qbassignconfig, array $enrolconfig,
        ...
        $qbassign = $this->create_instance($course, $qbassignconfig);
        $user = $this->getDataGenerator()->create_and_enrol($course, ...array_values($enrolconfig));

        $instance = $qbassign->get_instance($user->id);
    ...
    /**
     * The test_qbassign_get_instance data provider.
     */
    public function qbassign_get_instance_provider() {
        $timenow = time();
    ...
    /**
     * Test qbassign->get_default_instance() for a number of cases, as defined in the date provider.
     *
     * @dataProvider qbassign_get_default_instance_provider
     * @param array $courseconfig the config to use when creating the course.
     * @param array $qbassignconfig the config to use when creating the qbassignment.
     * @param array $enrolconfig the config to use when enrolling the user (this will be the active user).
     * @param array $expectedproperties an map containing the expected names and values for the qbassign instance data.
     */
    public function test_qbassign_get_default_instance(array $courseconfig, array $qbassignconfig, array $enrolconfig,
            array $expectedproperties) {
        $this->resetAfterTest();
        ...
        $this->setUser($user);
        $defaultinstance = $qbassign->get_default_instance();
    ...
    /**
     * The test_qbassign_get_default_instance data provider.
     */
    public function qbassign_get_default_instance_provider() {
        $timenow = time();
    ...
    public function test_cron_use_task_api_to_get_lastruntime() {
        ...
        // Create an qbassignment which allows submissions from 3 days ago.
        $qbassign1 = $this->create_instance($course, [
        ...
        // Create an qbassignment which allows submissions from 1 day ago.
        $qbassign2 = $this->create_instance($course, [
            'duedate' => time() + DAYSECS,
            'alwaysshowdescription' => 0,
        ...
        $DB->set_field('task_scheduled', 'lastruntime', time() - 2 * DAYSECS, ['classname' => '\mod_qbassign\task\cron_task']);

        // Remove events to make sure cron will update calendar and re-create one of them.
        $params = array('modulename' => 'qbassign', 'instance' => $qbassign1->get_instance()->id);
        $DB->delete_records('event', $params);
        $params = array('modulename' => 'qbassign', 'instance' => $qbassign2->get_instance()->id);
        $DB->delete_records('event', $params);

        // Run cron.
        \qbassign::cron();

        // Assert that calendar hasn't been updated for the first qbassignment as it's supposed to be
        ...
        // Assert that calendar has been updated for the second qbassignment
    ...
    public function test_submissions_need_grading() {
        ...
        $PAGE->set_url(new \moodle_url('/mod/qbassign/view.php', [
            'id' => $qbassign->get_course_module()->id,
            'action' => 'grading',
        ]));

        // Check for 0 submissions.
        $summary = $qbassign->view('viewcourseindex');

        $this->assertStringContainsString('/mod/qbassign/view.php?id=' .
            $qbassign->get_course_module()->id . '&amp;action=grading">' .
            get_string('numberofsubmissionsneedgradinglabel', 'qbassign', 0) . '</a>', $summary);

        // Simulate an qbassignment submission.
        $this->setUser($student);
        $submission = $qbassign->get_user_submission($student->id, true);
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $student->id, true, false);
        ...
        $this->assertStringContainsString('/mod/qbassign/view.php?id=' .
            $qbassign->get_course_module()->id .  '&amp;action=grading">' .
            get_string('numberofsubmissionsneedgradinglabel', 'qbassign', 1) . '</a>', $summary);
    }

    /**
     * Test that attachments should not be provided if \qbassign->show_intro returns false.
     *
     * @covers \qbassign::should_provide_intro_attachments
     */
    public function test_should_provide_intro_attachments_with_show_intro_disabled() {
        $this->resetAfterTest();
        $futuredate = time() + 300;
        list($qbassign, $instance, $student) = $this->create_submission([
            'alwaysshowdescription' => '0',
            'allowsubmissionsfromdate' => $futuredate,
        ]);
        $this->assertFalse($qbassign->should_provide_intro_attachments($student->id));
    }
    ...
    public function test_should_provide_intro_attachments_with_bypass_capability() {
        ...
        $this->assertTrue($qbassign->should_provide_intro_attachments($student->id));
    }
     ...
     *
     * @covers \qbassign::is_attempt_in_progress
     */
    public function test_is_attempt_in_progress_with_open_submission() {
        ...
        $this->assertTrue($qbassign->is_attempt_in_progress());
    }
    ...
    public function test_is_attempt_in_progress_with_open_submission_and_no_timestarted() {
        ...
        $qbassign->get_user_submission($student->id, true);
        $this->assertFalse($qbassign->is_attempt_in_progress());
    }
    ...
    protected function create_submission(array $params = []) {
        ...
        return array($qbassign, $instance, $student);
    }

    /**
     * Test user filtering by First name, Last name and Submission status.
     *
     * @covers \qbassign::is_userid_filtered
     */
    public function test_is_userid_filtered() {
        ...
        $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
        $qbassign->testable_update_submission($submission, $student1->id, true, false);
        $this->setUser($student2);
        $submission = $qbassign->get_user_submission($student2->id, true);
        $submission->status = qbassign_SUBMISSION_STATUS_DRAFT;
        $qbassign->testable_update_submission($submission, $student2->id, true, false);
        $this->setUser($teacher);

        // By default, both users should match filters.
        $this->AssertTrue($qbassign->is_userid_filtered($student1->id));
        $this->AssertTrue($qbassign->is_userid_filtered($student2->id));

        // Filter by First name starting with V.
        $_GET['tifirst'] = 'V';
        $this->AssertTrue($qbassign->is_userid_filtered($student1->id));
        $this->AssertFalse($qbassign->is_userid_filtered($student2->id));

        // Add Last name to filter out both users.
        $_GET['tilast'] = 'G';
        $this->AssertFalse($qbassign->is_userid_filtered($student1->id));
        $this->AssertFalse($qbassign->is_userid_filtered($student2->id));
        ...
        set_user_preference('qbassign_filter', qbassign_SUBMISSION_STATUS_SUBMITTED);
        $this->AssertTrue($qbassign->is_userid_filtered($student1->id));
        $this->AssertFalse($qbassign->is_userid_filtered($student2->id));

        // Display users with drafts.
        set_user_preference('qbassign_filter', qbassign_SUBMISSION_STATUS_DRAFT);
        $this->AssertFalse($qbassign->is_userid_filtered($student1->id));
        $this->AssertTrue($qbassign->is_userid_filtered($student2->id));

        // Reset the filter.
        set_user_preference('qbassign_filter', '');
        $this->AssertTrue($qbassign->is_userid_filtered($student1->id));
        $this->AssertTrue($qbassign->is_userid_filtered($student2->id));
    }
}
