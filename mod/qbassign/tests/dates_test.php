...
/**
 * Contains unit tests for mod_qbassign\dates.
 *
...
/**
 * Class for unit testing mod_qbassign\dates.
 *
 * @copyright 2021 Shamim Rezaie <shamim@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dates_test extends advanced_testcase {
        ...
        return [
            ...
            'only with opening time' => [
                $after, null, null, null, null, null, [
                    ['label' => get_string('activitydate:submissionsopen', 'mod_qbassign'), 'timestamp' => $after,
                        'dataid' => 'allowsubmissionsfromdate'],
                ]
            ],
            'only with closing time' => [
                null, $after, null, null, null, null, [
                    ['label' => get_string('activitydate:submissionsdue', 'mod_qbassign'), 'timestamp' => $after,
                        'dataid' => 'duedate'],
                ]
            ],
            'with both times' => [
                $after, $later, null, null, null, null, [
                    ['label' => get_string('activitydate:submissionsopen', 'mod_qbassign'), 'timestamp' => $after,
                        'dataid' => 'allowsubmissionsfromdate'],
                    ['label' => get_string('activitydate:submissionsdue', 'mod_qbassign'), 'timestamp' => $later,
                        'dataid' => 'duedate'],
                ]
            ],
            'between the dates' => [
                $before, $after, null, null, null, null, [
                    ['label' => get_string('activitydate:submissionsopened', 'mod_qbassign'), 'timestamp' => $before,
                        'dataid' => 'allowsubmissionsfromdate'],
                    ['label' => get_string('activitydate:submissionsdue', 'mod_qbassign'), 'timestamp' => $after,
                        'dataid' => 'duedate'],
                ]
            ...
            'dates are past' => [
                $earlier, $before, null, null, null, null, [
                    ['label' => get_string('activitydate:submissionsopened', 'mod_qbassign'), 'timestamp' => $earlier,
                        'dataid' => 'allowsubmissionsfromdate'],
                    ['label' => get_string('activitydate:submissionsdue', 'mod_qbassign'), 'timestamp' => $before,
                        'dataid' => 'duedate'],
                ]
            ],
            'with user override' => [
     ...
     * @param int|null $from Time of opening submissions in the qbassignment.
     * @param int|null $due qbassignment's due date.
    ...
    public function test_get_dates_for_module(?int $from, ?int $due,
        ...
        $qbassigngenerator = $generator->get_plugin_generator('mod_qbassign');
        ...
        $qbassign = $qbassigngenerator->create_instance($data);
        ...
        if ($userfrom || $userdue || $groupfrom || $groupdue) {
            ...
            if ($userfrom || $userdue) {
                $qbassigngenerator->create_override([
