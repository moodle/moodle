@mod @mod_qbassign
Feature: qbassign user override
  In order to grant a student special access to an qbassignment
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      | activity | name                 | intro                   | course | qbassignsubmission_onlinetex_enabled |
      | qbassign   | Test qbassignment name | Submit your online text | C1     | 1                                   |
  ...
  Scenario: Allow a user to have a different due date
    ...
    And the activity date in "Test qbassignment name" should contain "Due: Saturday, 1 January 2000, 8:00"
    And I log out
    And I am on the "Test qbassignment name" Activity page logged in as student1
    And the activity date in "Test qbassignment name" should contain "Due: Wednesday, 1 January 2020, 8:00"
  ...
  Scenario: Allow a user to have a different start date
    ...
    Then the activity date in "Test qbassignment name" should contain "Opens: Tuesday, 1 January 2030, 8:00"
    And I log out
  ...
  Scenario: Override a user when teacher is in no group, and does not have accessallgroups permission, and the activity's group mode is "separate groups"
    ...
    And the following "activities" exist:
      ...
      | qbassign   | qbassignment 2 | qbassignment 2 description | C1     | 1         |
    And I am on the "qbassignment 2" Activity page logged in as teacher1
    When I navigate to "Overrides" in current page administration
  ...
  Scenario: A teacher without accessallgroups permission should only be able to see the user override for users that he/she shares groups with,
    ...
    And the following "group members" exist:
      ...
      | student1 | G1    |
      | student2 | G2    |
    And I am on the "qbassignment 2" Activity page logged in as admin
    And I navigate to "Overrides" in current page administration
    And I press "Add user override"
    And I set the following fields to these values:
      | Override user          | Student1                            |
  ...
  @javascript
  Scenario: Create a user override when the qbassignment is not available to the student
    Given I am on the "Test qbassignment name" Activity page logged in as teacher1
