...
/**
 * This file contains the definition for the class qbassignment
 *
...
// qbassignment submission statuses.
define('qbassign_SUBMISSION_STATUS_NEW', 'new');
define('qbassign_SUBMISSION_STATUS_REOPENED', 'reopened');
define('qbassign_SUBMISSION_STATUS_DRAFT', 'draft');
define('qbassign_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('qbassign_FILTER_NONE', 'none');
define('qbassign_FILTER_SUBMITTED', 'submitted');
define('qbassign_FILTER_NOT_SUBMITTED', 'notsubmitted');
define('qbassign_FILTER_SINGLE_USER', 'singleuser');
define('qbassign_FILTER_REQUIRE_GRADING', 'requiregrading');
define('qbassign_FILTER_GRANTED_EXTENSION', 'grantedextension');
define('qbassign_FILTER_DRAFT', 'draft');

// Marker filter for grading page.
define('qbassign_MARKER_FILTER_NO_MARKER', -1);

// Reopen attempt methods.
define('qbassign_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('qbassign_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('qbassign_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('qbassign_UNLIMITED_ATTEMPTS', -1);

// Special value means no grade has been set.
define('qbassign_GRADE_NOT_SET', -1);

// Grading states.
define('qbassign_GRADING_STATUS_GRADED', 'graded');
define('qbassign_GRADING_STATUS_NOT_GRADED', 'notgraded');

// Marking workflow states.
define('qbassign_MARKING_WORKFLOW_STATE_NOTMARKED', 'notmarked');
define('qbassign_MARKING_WORKFLOW_STATE_INMARKING', 'inmarking');
define('qbassign_MARKING_WORKFLOW_STATE_READYFORREVIEW', 'readyforreview');
define('qbassign_MARKING_WORKFLOW_STATE_INREVIEW', 'inreview');
define('qbassign_MARKING_WORKFLOW_STATE_READYFORRELEASE', 'readyforrelease');
define('qbassign_MARKING_WORKFLOW_STATE_RELEASED', 'released');

/** qbassign_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("qbassign_MAX_EVENT_LENGTH", "432000");

// Name of file area for intro attachments.
define('qbassign_INTROATTACHMENT_FILEAREA', 'introattachment');

// Name of file area for activity attachments.
define('qbassign_ACTIVITYATTACHMENT_FILEAREA', 'activityattachment');

// Event types.
define('qbassign_EVENT_TYPE_DUE', 'due');
define('qbassign_EVENT_TYPE_GRADINGDUE', 'gradingdue');
define('qbassign_EVENT_TYPE_OPEN', 'open');
define('qbassign_EVENT_TYPE_CLOSE', 'close');
...
require_once($CFG->dirroot . '/mod/qbassign/submissionplugin.php');
require_once($CFG->dirroot . '/mod/qbassign/renderable.php');
require_once($CFG->dirroot . '/mod/qbassign/gradingtable.php');
require_once($CFG->libdir . '/portfolio/caller.php');

use mod_qbassign\event\submission_removed;
use mod_qbassign\event\submission_status_updated;
use \mod_qbassign\output\grading_app;
use \mod_qbassign\output\qbassign_header;
use \mod_qbassign\output\qbassign_submission_status;
use mod_qbassign\output\timelimit_panel;
use mod_qbassign\downloader;

/**
 * Standard base class for mod_qbassign (qbassignment types).
 *
 * @package   mod_qbassign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class qbassign {
    ...
    /** @var stdClass the qbassignment record that contains the global settings for this qbassign instance */
    private $instance;

    /** @var array $var array an array containing per-user qbassignment records, each having calculated properties (e.g. dates) */
    private $userinstances = [];

    /** @var grade_item the grade_item record for this qbassign instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this qbassign instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this qbassign instance belongs to */
    private $course;

    /** @var stdClass the admin config for all qbassign instances  */
    private $adminconfig;

    /** @var qbassign_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this qbassign instance */
    ...
    /** @var array cached list of participants for this qbassignment. The cache key will be group, showactive and the context id */
    ...
    /** @var array Array of error messages encountered during the execution of qbassignment related operations. */
    ...
    /**
     * Constructor for the base qbassign class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('qbassignsubmission');
        $this->feedbackplugins = $this->load_plugins('qbassignfeedback');

        // Extra entropy is required for uniqid() to work on cygwin.
        $this->useridlistid = clean_param(uniqid('', true), PARAM_ALPHANUM);

        if (!isset($SESSION->mod_qbassign_useridlist)) {
            $SESSION->mod_qbassign_useridlist = [];
        }
    }
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new moodle_url('/mod/qbassign/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new moodle_url('/mod/qbassign/index.php', array('id' => $this->get_course()->id));
        }
    ...
    /**
     * Based on the current qbassignment settings should we display the intro.
     *
     ...
     *
     * This just checks the qbassignment settings. Remember to check
    ...
    /**
     * Does an qbassignment have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype qbassignsubmission | qbassignfeedback
     * @param string $type
     * @return mixed qbassign_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('qbassign'));
     ...
     * @return mixed qbassign_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('qbassignfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed qbassign_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('qbassignsubmission', $type);
    }
    ...
    public function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('qbassign'));
                $pluginclass = 'qbassign_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof qbassign_plugin) {
    ...
    /**
     * Display the qbassignment, used by view.php
     *
     * The qbassignment is displayed differently depending on your role,
     * the settings for the qbassignment and the status of the qbassignment.
     *
    ...
    public function view($action='', $args = array()) {
        ...
        if (!empty($action)) {
            $PAGE->set_pagetype('mod-qbassign-' . $action);
        }
        // Now show the right view page.
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/qbassign/view.php', $nextpageparams);
            $messages = '';
            ...
            $message = get_string('gradingchangessaved', 'qbassign');
        ...
        } else if ($action == 'viewpluginqbassignfeedback') {
            $o .= $this->view_plugin_content('qbassignfeedback');
        } else if ($action == 'viewpluginqbassignsubmission') {
            $o .= $this->view_plugin_content('qbassignsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'qbassign'), $notices);
     ...
     *             when upgrading an old qbassignment to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->attemptreopenmethod = qbassign_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('qbassign', $update);
        $this->instance = $DB->get_record('qbassign', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        if ($callplugins) {
            ...
            if (!empty($formdata->completionexpected)) {
                \core_completion\api::update_completion_date_event($formdata->coursemodule, 'qbassign', $this->instance,
        ...
        $DB->update_record('qbassign', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this qbassignment.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/qbassign',
                               $this->get_course()->id,
                               'mod',
                               'qbassign',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this qbassignment.
        $fs = get_file_storage();
        ...
        $DB->delete_records('qbassign_submission', array('qbassignment' => $this->get_instance()->id));
        $DB->delete_records('qbassign_grades', array('qbassignment' => $this->get_instance()->id));
        $DB->delete_records('qbassign_plugin_config', array('qbassignment' => $this->get_instance()->id));
        $DB->delete_records('qbassign_user_flags', array('qbassignment' => $this->get_instance()->id));
        $DB->delete_records('qbassign_user_mapping', array('qbassignment' => $this->get_instance()->id));
        ...
        $DB->delete_records('qbassign', array('id'=>$this->get_instance()->id));

        return $result;
    }

    /**
     * Deletes a qbassign override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        if (empty($cm)) {
            ...
            $cm = get_coursemodule_from_instance('qbassign', $instance->id, $instance->course);
        }

        $override = $DB->get_record('qbassign_overrides', array('id' => $overrideid), '*', MUST_EXIST);

        // Delete the events.
        $conds = array('modulename' => 'qbassign', 'instance' => $this->get_instance()->id);
        ...
        $DB->delete_records('qbassign_overrides', array('id' => $overrideid));
        ...
        $params = array(
            ...
            'other' => array(
                'qbassignid' => $override->qbassignid
            )
        );
        // Determine which override deleted event to fire.
        if (!empty($override->userid)) {
            $params['relateduserid'] = $override->userid;
            $event = \mod_qbassign\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_qbassign\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('qbassign_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all qbassign overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('qbassign_overrides', array('qbassignid' => $this->get_instance()->id), 'id');
    ...
    /**
     * Updates the qbassign properties with override information for a user.
     *
     * Algorithm:  For each qbassign setting, if there is a matching user-specific override,
     *   then use that otherwise, if there are group-specific overrides, return the most
     *   lenient combination of them.  If neither applies, leave the qbassign setting unchanged.
     *
    ...
    /**
     * Returns whether an qbassign has any overrides.
     *
    ...
    public function has_overrides() {
        ...
        $override = $DB->record_exists('qbassign_overrides', array('qbassignid' => $this->get_instance()->id));
     ...
     *   lowest sort order. If neither applies, leave the qbassign setting unchanged.
     *
     * @param int $userid The userid.
    ...
    public function override_exists($userid) {
        ...
        $getuseroverride = function($userid) use ($DB) {
            $useroverride = $DB->get_record('qbassign_overrides', ['qbassignid' => $this->get_instance()->id, 'userid' => $userid]);
        ...
        $getgroupoverride = function($userid) use ($DB) {
            ...
            $sql = "SELECT * FROM {qbassign_overrides}
                    WHERE groupid $extra AND qbassignid = ? ORDER BY sortorder ASC";
    ...
    public function is_override_calendar_event(\calendar_event $event) {
        ...
        if ($DB->get_record('qbassign_overrides', $overrideparams)) {
            return true;
        } else {
            return false;
        }
    ...
    function get_valid_calendar_event_timestart_range(\calendar_event $event) {
        ...
        if ($event->eventtype == qbassign_EVENT_TYPE_DUE) {
            ...
            if ($submissionsfromdate) {
                $mindate = [
                    $submissionsfromdate,
                    get_string('duedatevalidation', 'qbassign'),
                ];
            }
            ...
            if ($cutoffdate) {
                $maxdate = [
                    $cutoffdate,
                    get_string('cutoffdatevalidation', 'qbassign'),
                ];
            }
            ...
            if ($gradingduedate) {
                ...
                if (!$cutoffdate || $gradingduedate < $cutoffdate) {
                    $maxdate = [
                        $gradingduedate,
                        get_string('gradingdueduedatevalidation', 'qbassign'),
                    ];
                }
            }
            ...
            if ($duedate) {
                ...
                $mindate = [
                    $submissionsfromdate,
                    get_string('gradingduefromdatevalidation', 'qbassign'),
                ];
            }
        }
     ...
     * qbassignment submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'qbassign');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_qbassign_submissions)) {
            // Delete files associated with this qbassignment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'qbassign'),
            ...
            $qbassignids = $DB->get_records('qbassign', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($qbassignids));

            $DB->delete_records_select('qbassign_submission', "qbassignment $sql", $params);
            $DB->delete_records_select('qbassign_user_flags', "qbassignment $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('qbassign_grades', "qbassignment $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/qbassign/lib.php');
                qbassign_reset_gradebook($data->courseid);
            }

            // Reset revealidentities for qbassign if blindmarking is enabled.
            if ($this->get_instance()->blindmarking) {
                $DB->set_field('qbassign', 'revealidentities', 0, array('id' => $this->get_instance()->id));
            }
        }
        ...
        if (!empty($data->reset_qbassign_user_overrides)) {
            $DB->delete_records_select('qbassign_overrides',
                'qbassignid IN (SELECT id FROM {qbassign} WHERE course = ?) AND userid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('useroverridesdeleted', 'qbassign'),
                'error' => false);
            $purgeoverrides = true;
        }
        // Remove group overrides.
        if (!empty($data->reset_qbassign_group_overrides)) {
            $DB->delete_records_select('qbassign_overrides',
                'qbassignid IN (SELECT id FROM {qbassign} WHERE course = ?) AND groupid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('groupoverridesdeleted', 'qbassign'),
                'error' => false);
            $purgeoverrides = true;
        }
        ...
        if ($data->timeshift) {
            $DB->execute("UPDATE {qbassign_overrides}
                         SET allowsubmissionsfromdate = allowsubmissionsfromdate + ?
                       WHERE qbassignid = ? AND allowsubmissionsfromdate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {qbassign_overrides}
                         SET duedate = duedate + ?
                       WHERE qbassignid = ? AND duedate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {qbassign_overrides}
                         SET cutoffdate = cutoffdate + ?
                       WHERE qbassignid =? AND cutoffdate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            ...
            // See MDL-9367.
            shift_course_mod_dates('qbassign',
        ...
        if ($purgeoverrides) {
            cache::make('mod_qbassign', 'overrides')->purge();
        }
     ...
     *
     * @param qbassign_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(qbassign_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this qbassignment.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this qbassignment
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/qbassign/lib.php');
        $qbassign = clone $this->get_instance();
        $qbassign->cmidnumber = $coursemoduleid;

        // Set qbassign gradebook feedback plugin status (enabled and visible).
        $qbassign->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return qbassign_grade_item_update($qbassign, $param);
    }
     ...
     * @return integer
     */
    public function get_qbassign_perpage() {
        $perpage = (int) get_user_preferences('qbassign_perpage', 10);
        $adminconfig = $this->get_admin_config();
        $maxperpage = -1;
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('qbassign');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this qbassignment.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            $event->name = get_string('calendardue', 'qbassign', $instance->name);
            ...
            $select = "modulename = :modulename
                       ...
                       AND groupid = 0
                       AND courseid <> 0";
            $params = array('modulename' => 'qbassign', 'instance' => $instance->id, 'eventtype' => $eventtype);
            ...
            $DB->delete_records('event', array('modulename' => 'qbassign', 'instance' => $instance->id,
                'eventtype' => $eventtype));
        }

        $eventtype = qbassign_EVENT_TYPE_GRADINGDUE;
        if ($instance->gradingduedate) {
            $event->name = get_string('calendargradingdue', 'qbassign', $instance->name);
            $event->eventtype = $eventtype;
            $event->timestart = $instance->gradingduedate;
            $event->timesort = $instance->gradingduedate;
            $event->id = $DB->get_field('event', 'id', array('modulename' => 'qbassign',
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('qbassign', $update);
        $this->instance = $DB->get_record('qbassign', array('id'=>$update->id), '*', MUST_EXIST);

        $this->save_intro_draft_files($formdata);

        // Load the qbassignment so the plugins have access to it.
        ...
        \core_completion\api::update_completion_date_event($this->get_course_module()->id, 'qbassign', $this->instance,
                $completionexpected);
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_qbassign', qbassign_INTROATTACHMENT_FILEAREA, 0);
        }
    }
        ...
        if (isset($formdata->activityeditor)) {
            ...
            if (isset($formdata->activityeditor['itemid'])) {
                $text = file_save_draft_area_files($formdata->activityeditor['itemid'], $this->get_context()->id,
                    'mod_qbassign', qbassign_ACTIVITYATTACHMENT_FILEAREA,
     ...
     *
     * @param qbassign_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(qbassign_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'qbassign'));
     ...
     *
     * @return string the module name (qbassignment)
     */
    protected function get_module_name() {
        if (isset(self::$modulename)) {
            return self::$modulename;
        }
        self::$modulename = get_string('modulename', 'qbassign');
        return self::$modulename;
    }
     ...
     *
     * @return string the module name plural (qbassignments)
     */
    protected function get_module_name_plural() {
        if (isset(self::$modulenameplural)) {
            return self::$modulenameplural;
        }
        self::$modulenameplural = get_string('modulenameplural', 'qbassign');
        return self::$modulenameplural;
    }

    /**
     * Has this qbassignment been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this qbassignment.
     *
    ...
    public function get_default_instance() {
        ...
        if (!$this->instance && $this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('qbassign', $params, '*', MUST_EXIST);
    ...
    /**
     * Get the settings for the current instance of this qbassignment
     * @param int|null $userid the id of the user to load the qbassign instance for.
     * @return stdClass The settings
     */
     ...
     *
     * @param stdClass $record the raw qbassign record.
    ...
    /**
     * Get the primary grade item for this qbassign instance.
     *
    ...
    public function get_grade_item() {
        ...
        if (!$this->gradeitem) {
            throw new coding_exception('Improper use of the qbassignment class. ' .
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_qbassign', qbassign_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
    ...
    public function should_provide_intro_attachments(int $userid): bool {
        ...
        // Check if user has permission to view attachments regardless of qbassignment settings.
        if (has_capability('moodle/course:manageactivities', $this->get_context())) {
            return true;
        }

        // If qbassignment does not show intro, we never provide intro attachments.
        if (!$this->show_intro()) {
            return false;
        }
    ...
    /**
     * Get the submission status/grading status for all submissions in this qbassignment for the
     * given paticipants.
     *
     * These statuses match the available filters (requiregrading, submitted, notsubmitted, grantedextension).
     * If this is a group qbassignment, group info is also returned.
     *
    ...
    private function get_submission_info_for_participants($participants) {
        ...
        $qbassignid = $this->get_instance()->id;
        $params['qbassignmentid1'] = $qbassignid;
        $params['qbassignmentid2'] = $qbassignid;
        $params['qbassignmentid3'] = $qbassignid;
        ...
        $from = ' FROM {user} u
                         LEFT JOIN {qbassign_submission} s
                                ON u.id = s.userid
                               AND s.qbassignment = :qbassignmentid1
                               AND s.latest = 1
                         LEFT JOIN {qbassign_grades} g
                                ON u.id = g.userid
                               AND g.qbassignment = :qbassignmentid2
                               AND g.attemptnumber = s.attemptnumber
                         LEFT JOIN {qbassign_user_flags} uf
                                ON u.id = uf.userid
                               AND uf.qbassignment = :qbassignmentid3
            ';
        $where = ' WHERE u.id ' . $insql;

        if (!empty($this->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {qbassign_user_mapping} um
                             ON u.id = um.userid
                            AND um.qbassignment = :qbassignmentid4 ';
            $params['qbassignmentid4'] = $qbassignid;
        ...
        foreach ($records as $userid => $submissioninfo) {
            ...
            if (!empty($submissioninfo->stime) && $submissioninfo->status == qbassign_SUBMISSION_STATUS_SUBMITTED) {
                $submitted = true;
            }
    ...
    /**
     * Get the submission status/grading status for all submissions in this qbassignment.
     * These statuses match the available filters (requiregrading, submitted, notsubmitted, grantedextension).
     * If this is a group qbassignment, group info is also returned.
     *
    ...
    private function get_grading_sort_sql() {
        $usersort = flexible_table::get_sort_for_table('mod_qbassign_grading');
    ...
    /**
     * Returns array with sql code and parameters returning all ids of users who have submitted an qbassignment.
     *
    ...
    protected function get_submitted_sql($group = 0) {
        ...
        $prefix = 'sa' . $i . '_';
        $params = [
            "{$prefix}qbassignment" => (int) $this->get_instance()->id,
            "{$prefix}status" => qbassign_SUBMISSION_STATUS_NEW,
        ];
        ...
        $sql = "SELECT {$prefix}s.userid
                  FROM {qbassign_submission} {$prefix}s
                  JOIN {user} {$prefix}u ON {$prefix}u.id = {$prefix}s.userid
                  $capjoin->joins
                 WHERE {$prefix}s.qbassignment = :{$prefix}qbassignment
    ...
    public function list_participants($currentgroup, $idsonly, $tablesort = false) {
        ...
        if (!isset($this->participants[$key])) {
            list($esql, $params) = get_enrolled_sql($this->context, 'mod/qbassign:submit', $currentgroup,
            ...
            if (!empty($instance->blindmarking)) {
                $additionaljoins .= " LEFT JOIN {qbassign_user_mapping} um
                                  ON u.id = um.userid
                                 AND um.qbassignment = :qbassignmentid1
                           LEFT JOIN {qbassign_submission} s
                                  ON u.id = s.userid
                                 AND s.qbassignment = :qbassignmentid2
                                 AND s.latest = 1
                        ";
                $params['qbassignmentid1'] = (int) $instance->id;
                $params['qbassignmentid2'] = (int) $instance->id;
                $fields .= ', um.id as recordid ';
            ...
            if ($instance->markingworkflow &&
                    ...
                    !has_capability('mod/qbassign:manageallocations', $this->get_context()) &&
                    has_capability('mod/qbassign:grade', $this->get_context())) {

                $additionaljoins .= ' LEFT JOIN {qbassign_user_flags} uf
                                     ON u.id = uf.userid
                                     AND uf.qbassignment = :qbassignmentid3';

                $params['qbassignmentid3'] = (int) $instance->id;
    ...
    public function get_participant($userid) {
        ...
        if (!$submissioninfo->submitted && !has_capability('mod/qbassign:submit', $this->context, $userid)) {
            return null;
        }
    ...
    /**
     * Load a count of valid teams for this qbassignment.
     *
    ...
    public function count_submissions_need_grading($currentgroup = null) {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group qbassignment because the submission is shared.
            return 0;
        }
        ...
        $params['qbassignid'] = $this->get_instance()->id;
        ...
        $sql = 'SELECT COUNT(s.userid)
                   ...
                   LEFT JOIN {qbassign_grades} g ON
                        s.qbassignment = g.qbassignment AND
                        s.userid = g.userid AND
                        g.attemptnumber = s.attemptnumber
                   JOIN(' . $esql . ') e ON e.id = s.userid
                   WHERE
                        s.latest = 1 AND
                        s.qbassignment = :qbassignid AND
    ...
    public function count_grades() {
        ...
        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/qbassign:submit', $currentgroup, true);
        ...
        $sql = 'SELECT COUNT(g.userid)
                   ...
                   WHERE g.qbassignment = :qbassignid';
    ...
    public function count_submissions($includenew = false) {
        ...
        if (!$includenew) {
            ...
            $params['status'] = qbassign_SUBMISSION_STATUS_NEW;
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/qbassign:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status, $currentgroup = null) {
        ...
        $params['qbassignid'] = $this->get_instance()->id;
        $params['qbassignid2'] = $this->get_instance()->id;
    ...
    protected function get_grading_userid_list($cached = false, $useridlistid = '') {
        ...
        if ($cached) {
            ...
            if (empty($SESSION->mod_qbassign_useridlist[$useridlistkey])) {
                $SESSION->mod_qbassign_useridlist[$useridlistkey] = $this->get_grading_userid_list(false, '');
            }
            return $SESSION->mod_qbassign_useridlist[$useridlistkey];
        }
        $filter = get_user_preferences('qbassign_filter', '');
        $table = new qbassign_grading_table($this, 0, $filter, 0, false);
    ...
    /**
     * Finds all qbassignment notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities, a.hidegrader,
                 ...
                 FROM {qbassign} a
                 JOIN {qbassign_grades} g ON g.qbassignment = a.id
            LEFT JOIN {qbassign_user_flags} uf ON uf.qbassignment = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'qbassign'
                 JOIN {grade_items} gri ON gri.iteminstance = a.id AND gri.courseid = a.course AND gri.itemmodule = md.name
            LEFT JOIN {qbassign_user_mapping} um ON g.id = um.userid AND um.qbassignment = a.id
        ...
        $params = array(
            'yesterday' => $yesterday,
            'today' => $timenow,
            'wfreleased' => qbassign_MARKING_WORKFLOW_STATE_RELEASED,
        );
        $submissions = $DB->get_records_sql($sql, $params);

        if (!empty($submissions)) {

            mtrace('Processing ' . count($submissions) . ' qbassignment submissions ...');
            ...
            foreach ($submissions as $submission) {
                ...
                mtrace("Processing qbassignment submission $submission->id ...");
                ...
                if (!$cm->uservisible) {
                    // Hold mail notification for qbassignments the user cannot access until later.
                    continue;
                }
                ...
                if ($submission->hidegrader && !has_capability('mod/qbassign:showhiddengrader', $contextmodule, $user)) {
                ...
                $eventtype = 'qbassign_notification';
                ...
                if ($submission->blindmarking && !$submission->revealidentities) {
                    if (empty($submission->recordid)) {
                        $uniqueid = self::get_uniqueid_for_user_static($submission->qbassignment, $grader->id);
                    } else {
                        $uniqueid = $submission->recordid;
                    }
                }
                $showusers = $submission->blindmarking && !$submission->revealidentities;
                self::send_qbassignment_notification($grader,
                                                   $user,
                                                   $messagetype,
                                                   $eventtype,
                                                   $updatetime,
                ...
                $flags = $DB->get_record('qbassign_user_flags', array('userid'=>$user->id, 'qbassignment'=>$submission->qbassignment));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('qbassign_user_flags', $flags);
                } else {
                    $flags = new stdClass();
                    $flags->userid = $user->id;
                    $flags->qbassignment = $submission->qbassignment;
                    $flags->mailed = 1;
            ...
            mtrace('Done processing ' . count($submissions) . ' qbassignment submissions');
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('qbassign', $record->id, 0, false, MUST_EXIST);
            $context = context_module::instance($cm->id);
    ...
    /**
     * Update user flags for this user in this qbassignment.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->qbassignment <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('qbassign_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the qbassignment and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @param bool $reopenattempt If the attempt reopen method is manual, allow another attempt at this qbassignment.
     * @return bool true for success
     */
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('qbassign_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_qbassign\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $CFG;
        require_once($CFG->dirroot . '/mod/qbassign/extensionform.php');
        ...
        if (empty($mform)) {
            $mform = new mod_qbassign_extension_form(null, $formparams);
        }
        $mform->set_data($data);
        $header = new qbassign_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'qbassign'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new qbassign_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the qbassignment.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == qbassign_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'qbassign') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('qbassignment'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        if ($create) {
            ...
            $submission->qbassignment = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('qbassign_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('qbassign_submission', 'latest', 0, $params);
            }
            $submission->status = qbassign_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('qbassign_submission', $submission);
            $transaction->allow_commit();
            return $DB->get_record('qbassign_submission', array('id' => $sid));
        }
        return false;
    }

    /**
     * View a summary listing of all qbassignments in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('qbassign', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new qbassign_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['qbassign'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            $timedue = $qbassignment->get_instance()->duedate;

            if (has_capability('mod/qbassign:submit', $context) &&
                !has_capability('moodle/site:config', $context)) {
                $cangrade = false;
                if ($qbassignment->get_instance()->teamsubmission) {
                    $usersubmission = $qbassignment->get_group_submission($USER->id, 0, false);
                } else {
                    $usersubmission = $qbassignment->get_user_submission($USER->id, false);
                }

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'qbassign');
                } else {
                    $submitted = get_string('submissionstatus_', 'qbassign');
                }

                $gradinginfo = grade_get_grades($course->id, 'mod', 'qbassign', $cm->instance, $USER->id);
            ...
            } else if (has_capability('mod/qbassign:grade', $context)) {
                $submitted = $qbassignment->count_submissions_with_status(qbassign_SUBMISSION_STATUS_SUBMITTED);
                $grade = $qbassignment->count_submissions_need_grading();
                $cangrade = true;
            }

            $courseindexsummary->add_qbassign_info($cm->id, $cm->get_formatted_name(),
    ...
    /**
     * This is used for team qbassignments to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_PLUGIN);
        $item = null;
        if ($pluginsubtype == 'qbassignsubmission') {
            ...
            $o .= $this->get_renderer()->render(new qbassign_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new qbassign_submission_plugin_submission($plugin,
                                                              $item,
                                                              qbassign_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_qbassign\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new qbassign_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new qbassign_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              qbassign_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_qbassign\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param qbassign_plugin $plugin - The qbassignment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        // The groupname prefix for the urls doesn't depend on the group mode of the qbassignment instance.
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            if ($submissiongroup) {
                ...
                $groupname = get_string('defaultteam', 'qbassign') . '-';
            }
        }
        ...
        if (!get_user_preferences('qbassign_downloadasfolders', 1)) {
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component, $shortentext = false) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/qbassign:exportownsubmission', $this->context)) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('qbassign_portfolio_caller', $portfolioparams, 'mod_qbassign');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new qbassign_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'qbassign')));
        $gradingresult = new qbassign_gradingmessage(get_string('savegradingresult', 'qbassign'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new qbassign_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'qbassign')));
        $gradingerror = in_array($message, $this->get_error_messages());
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new qbassign_gradingmessage(get_string('quickgradingresult', 'qbassign'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   $gradingerror,
                                                   $lastpage);
    ...
    public function require_view_group_submission($groupid) {
        if (!$this->can_view_group_submission($groupid)) {
            throw new required_capability_exception($this->context, 'mod/qbassign:viewgrades', 'nopermission', '');
        }
    }
    ...
    /**
     * Throw an error if the permissions to view grades in this qbassignment are missing.
     *
     * @throws required_capability_exception
     * @return none
     */
    ...
    /**
     * Does this user have view grade or grade permission for this qbassignment?
     *
    ...
    public function can_view_grades($groupid = null) {
        // Permissions check.
        if (!has_any_capability(array('mod/qbassign:viewgrades', 'mod/qbassign:grade'), $this->context)) {
            return false;
        }
    ...
    /**
     * Does this user have grade permission for this qbassignment?
     *
    ...
    public function can_grade($user = null) {
        // Permissions check.
        if (!has_capability('mod/qbassign:grade', $this->context, $user)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all qbassignment submissions.
     *
     * @param array|null $userids Array of user ids to download qbassignment submissions in a zip file
    ...
    protected function download_submissions($userids = null) {
        ...
        $header = new qbassign_header(
            $this->get_instance(),
            $this->get_context(),
            '',
            $cm->id,
            get_string('downloadall', 'mod_qbassign')
        );
        $result = $renderer->render($header);
        $result .= $renderer->notification(get_string('nosubmission', 'mod_qbassign'));
        $url = new moodle_url('/mod/qbassign/view.php', ['id' => $cm->id, 'action' => 'grading']);
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_qbassign add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_qbassign', null, RENDERER_TARGET_GENERAL);
        return $this->output;
    }
     ...
     *
     * For team qbassignments there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('qbassignment'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('qbassignment'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('qbassign_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('qbassignment'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('qbassign_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->qbassignment = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('qbassign_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('qbassign_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->qbassignment   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('qbassign_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('qbassignment'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('qbassign_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grading_panel($args) {
        ...
        // Need submit permission to submit an qbassignment.
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new qbassign_submission_status_compact($instance->allowsubmissionsfromdate,
                                                                     ...
                                                                     $this->get_course_module()->id,
                                                                     $this->get_course()->id,
                                                                     qbassign_submission_status::GRADER_VIEW,
                                                                     $showedit,
                                                                     false,
                                                                     $viewfullnames,
                                                                     $extensionduedate,
        ...
        if ($attemptnumber != -1 && ($attemptnumber + 1) != count($allsubmissions)) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'qbassign', $params);
        ...
        $mform = new mod_qbassign_grade_form(null,
                                           $formparams,
                                           'post',
                                           '',
        ...
        $o .= $this->get_renderer()->render(new qbassign_form('gradingform', $mform));

        if (count($allsubmissions) > 1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new qbassign_attempt_history_chooser($allsubmissions,
                                                          $allgrades,
                                                          $this->get_course_module()->id,
                                                          $userid);
        ...
        \mod_qbassign\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_single_grade_page($mform) {
        ...
        $header = new qbassign_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'qbassign'));
        $o .= $this->get_renderer()->render($header);
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->get_context());
            $usergroups = $this->get_all_groups($user->id);
            $submissionstatus = new qbassign_submission_status($instance->allowsubmissionsfromdate,
                                                             $instance->alwaysshowdescription,
                                                             $submission,
                                                             $instance->teamsubmission,
                                                             $teamsubmission,
        ...
        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new qbassign_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_remove_submission_confirm() {
        ...
        $urlparams = array('id' => $this->get_course_module()->id,
                           ...
                           'userid' => $userid,
                           'sesskey' => sesskey());
        $confirmurl = new moodle_url('/mod/qbassign/view.php', $urlparams);

        $urlparams = array('id' => $this->get_course_module()->id,
                           'action' => 'view');
        $cancelurl = new moodle_url('/mod/qbassign/view.php', $urlparams);

        if ($userid == $USER->id) {
            if ($this->is_time_limit_enabled($userid)) {
                $confirmstr = get_string('removesubmissionconfirmwithtimelimit', 'qbassign');
            } else {
                $confirmstr = get_string('removesubmissionconfirm', 'qbassign');
            }
        } else {
            if ($this->is_time_limit_enabled($userid)) {
                $confirmstr = get_string('removesubmissionconfirmforstudentwithtimelimit', 'qbassign', $this->fullname($user));
            } else {
                $confirmstr = get_string('removesubmissionconfirmforstudent', 'qbassign', $this->fullname($user));
            }
        }
        ...
        \mod_qbassign\event\remove_submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/qbassign:revealidentities', $this->get_context());
        ...
        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'qbassign'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_qbassign\event\reveal_identities_confirmation_page_viewed::create_from_qbassign($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/qbassign/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this qbassignment.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/qbassign/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/qbassign/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/qbassign/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'qbassign');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/qbassign:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/qbassign/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'qbassign');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/qbassign/view.php' .
                           '?id=' .  $cmid .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'qbassign'));
        $gradingactions->class .= ' mb-1';

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_qbassign', 'submissions');

        $perpage = $this->get_qbassign_perpage();
        $filter = get_user_preferences('qbassign_filter', '');
        $markerfilter = get_user_preferences('qbassign_markerfilter', '');
        $workflowfilter = get_user_preferences('qbassign_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('qbassign_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);
        $downloadasfolders = get_user_preferences('qbassign_downloadasfolders', 1);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/qbassign:manageallocations', $this->context);
        // Get markers to use in drop lists.
        $markingallocationoptions = array();
        if ($markingallocation) {
            list($sort, $params) = users_order_by_sql('u');
            // Only enrolled users could be qbassigned as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/qbassign:grade', 0, 'u.*', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'qbassign');
            $markingallocationoptions[qbassign_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'qbassign');
        ...
        $gradingoptionsform = new mod_qbassign_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_qbassign_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        $buttons = new \mod_qbassign\output\grading_actionmenu($this->get_course_module()->id,
             $this->is_any_submission_plugin_enabled(), $this->count_submissions());
        $actionformtext = $this->get_renderer()->render($buttons);
        $PAGE->activityheader->set_attrs(['hidecompletion' => true]);

        $currenturl = new moodle_url('/mod/qbassign/view.php', ['id' => $this->get_course_module()->id, 'action' => 'grading']);
        ...
        $o .= $this->get_renderer()->heading(get_string('gradeitem:submissions', 'mod_qbassign'), 2);
        ...
        if ($this->is_blind_marking() && has_capability('mod/qbassign:viewblinddetails', $this->get_context())) {
            $o .= $this->get_renderer()->notification(get_string('blindmarkingenabledwarning', 'qbassign'), 'notifymessage');
        }

        // Load and print the table of submissions.
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new qbassign_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_qbassign_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new qbassign_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new qbassign_grading_table($this, $perpage, $filter, 0, false);
        ...
        if ($this->can_grade()) {
            ...
            $SESSION->mod_qbassign_useridlist[$this->get_useridlist_key()] = $useridlist;
        }
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $qbassignform = new qbassign_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($qbassignform);
        }
        $qbassignform = new qbassign_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_qbassign.init_grading_options');
        $o .= $this->get_renderer()->render($qbassignform);
        return $o;
    }
    ...
    protected function view_grader() {
        ...
        $args = [
            ...
            'subpage' => get_string('grading', 'qbassign')
        ];
        ...
        $gradingtable = new qbassign_grading_table($this, 10, '', 0, false);
        $gradingtable->setup();
        ...
        \mod_qbassign\event\grading_table_viewed::create_from_qbassign($this)->trigger();
    ...
    /**
     * Message for students when qbassignment submissions have been closed.
     *
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/qbassign/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/qbassign:viewblinddetails', $this->get_context());
            ...
            if ($hasviewblind) {
                return get_string('participant', 'qbassign') . ' ' . $uniqueid . ' (' .
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/qbassign/submission_form.php');
        // Need submit permission to submit an qbassignment.
        $userid = optional_param('userid', $USER->id, PARAM_INT);
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/qbassign:submit', $this->context);
            ...
            $title = get_string('editsubmissionother', 'qbassign', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'qbassign'));
            return $this->view_notices($title, $message);
        }

        $postfix = '';
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_qbassign', qbassign_INTROATTACHMENT_FILEAREA, 0);
        }

        $data = new stdClass();
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_qbassign_submission_form(null, array($this, $data));
        }
        ...
        if ($timelimitenabled && !empty($submission->timestarted) && $this->get_instance()->timelimit) {
            ...
            $bc->attributes['id'] = 'mod_qbassign_timelimit_block';
            $bc->attributes['role'] = 'navigation';
            $bc->attributes['aria-labelledby'] = 'mod_qbassign_timelimit_block_title';
            $bc->title = get_string('qbassigntimeleft', 'qbassign');
            $bc->content = $navbc;
            $PAGE->blocks->add_fake_block($bc, reset($regions));
        }

        $o .= $this->get_renderer()->render(
            new qbassign_header($this->get_instance(),
                              $this->get_context(),
                              $this->show_intro(),
                              $this->get_course_module()->id,
                              $title,
        ...
        $o .= $this->get_renderer()->render(new qbassign_form('editsubmissionform', $mform));
        $o .= $this->view_footer();

        \mod_qbassign\event\submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    /**
     * See if this qbassignment has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_qbassign_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/qbassign/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            if ($usercount >= 5) {
                $usershtml .= get_string('moreusers', 'qbassign', count($userlist) - 5);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usershtml .= $this->get_renderer()->render(new qbassign_user_summary($user,
                                                                $this->get_course()->id,
                                                                $viewfullnames,
                                                                $this->is_blind_marking(),
                                                                $this->get_uniqueid_for_user($user->id),
        ...
        $mform = new mod_qbassign_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new qbassign_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'qbassign'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new qbassign_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_qbassign\event\batch_set_workflow_state_viewed::create_from_qbassign($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/qbassign/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql('u');
        // Only enrolled users could be qbassigned as potential markers.
        $markers = get_enrolled_users($this->get_context(), 'mod/qbassign:grade', 0, 'u.*', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_qbassign_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new qbassign_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'qbassign'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new qbassign_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_qbassign\event\batch_set_marker_allocation_viewed::create_from_qbassign($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/qbassign/submissionconfirmform.php');
        ...
        if (!$hassubmission && !$notifications) {
            $notifications[] = get_string('addsubmission_help', 'qbassign');
        }
        ...
        if ($mform == null) {
            $mform = new mod_qbassign_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $o .= $this->get_renderer()->render(new qbassign_header($this->get_instance(),
                                                              ...
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              get_string('confirmsubmissionheading', 'qbassign')));
        $submitforgradingpage = new qbassign_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_qbassign\event\submission_confirmation_form_viewed::create_from_qbassign($this)->trigger();

        return $o;
    }

    /**
     * Creates an qbassign_submission_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @param bool $showlinks return plain text or links to the profile
     * @return qbassign_submission_status renderable object
     */
    public function get_qbassign_submission_status_renderable($user, $showlinks) {
        global $PAGE;
        ...
        $gradingmanager = get_grading_manager($this->context, 'mod_qbassign', 'submissions');
        ...
        $submissionstatus = new qbassign_submission_status($instance->allowsubmissionsfromdate,
                                                          ...
                                                          $this->get_course_module()->id,
                                                          $this->get_course()->id,
                                                          qbassign_submission_status::STUDENT_VIEW,
                                                          $showedit,
                                                          $showsubmit,
                                                          $viewfullnames,
                                                          $extensionduedate,
    ...
    /**
     * Creates an qbassign_feedback_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return qbassign_feedback_status renderable object
     */
    public function get_qbassign_feedback_status_renderable($user) {
        global $CFG, $DB, $PAGE;
        ...
        if ($this->get_instance()->markingworkflow && $gradingstatus != qbassign_MARKING_WORKFLOW_STATE_RELEASED) {
            $emptyplugins = true; // Don't show feedback plugins until released either.
        }

        $cangrade = has_capability('mod/qbassign:grade', $this->get_context());
        ...
        if (($hasgrade || !$emptyplugins) && $gradevisible) {
            ...
            if ($hasgrade) {
                ...
                if (in_array($gradingstatus, [qbassign_GRADING_STATUS_GRADED, qbassign_MARKING_WORKFLOW_STATE_RELEASED])){
                    if (isset($grade->grader) && $grade->grader > 0) {
                        $grader = $DB->get_record('user', array('id' => $grade->grader));
                    } else if (isset($gradebookgrade->usermodified)
                        && $gradebookgrade->usermodified > 0
                        && has_capability('mod/qbassign:grade', $this->get_context(), $gradebookgrade->usermodified)) {
                        // Grader not provided. Check that usermodified is a user who can grade.
                        // Case 1: When an qbassignment is reopened an empty qbassign_grade is created so the feedback
                        // plugin can know which attempt it's referring to. In this case, usermodifed is a student.
                        // Case 2: When an qbassignment's grade is overrided via the gradebook, usermodified is a grader
            ...
            if ($grade) {
                \mod_qbassign\event\feedback_viewed::create_from_grade($this, $grade)->trigger();
            }
            $feedbackstatus = new qbassign_feedback_status(
                $gradefordisplay,
                $gradeddate,
                $grader,
                $this->get_feedback_plugins(),
            ...
            $showgradername = (
                    has_capability('mod/qbassign:showhiddengrader', $this->context) or
    ...
    /**
     * Creates an qbassign_attempt_history renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return qbassign_attempt_history renderable object
     */
    public function get_qbassign_attempt_history_renderable($user) {
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            if (has_capability('mod/qbassign:viewownsubmissionsummary', $this->get_context(), $user, false)) {
                // The user can view the submission summary.
                $submissionstatus = $this->get_qbassign_submission_status_renderable($user, $showlinks);
                $o .= $this->get_renderer()->render($submissionstatus);
            }

            // If there is a visible grade, show the feedback.
            $feedbackstatus = $this->get_qbassign_feedback_status_renderable($user);
            if ($feedbackstatus) {
                $o .= $this->get_renderer()->render($feedbackstatus);
            }

            // If there is more than one submission, show the history.
            $history = $this->get_qbassign_attempt_history_renderable($user);
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if (!has_capability('mod/qbassign:submit', $this->get_context(), $userid, false)) {
            // The user does not have the capability to submit.
            return false;
        }
        if ($teamsubmission) {
            if ($teamsubmission->status === qbassign_SUBMISSION_STATUS_SUBMITTED) {
                // The qbassignment submission has been completed.
                return false;
            } else if ($this->submission_empty($teamsubmission)) {
                // There is nothing to submit yet.
                return false;
            } else if ($submission && $submission->status === qbassign_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($submission->status === qbassign_SUBMISSION_STATUS_SUBMITTED) {
                // The qbassignment submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('qbassign_grades', $params, 'attemptnumber ASC');
        ...
        // Show the grader's identity if 'Hide Grader' is disabled or has the 'Show Hidden Grader' capability.
        $showgradername = (
            has_capability('mod/qbassign:showhiddengrader', $this->context, $userid) or
            !$this->is_hidden_grader()
        );
    ...
    public function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('qbassign_submission', $params, 'attemptnumber ASC');

        return $submissions;
    }

    /**
     * Creates an qbassign_grading_summary renderable.
     *
     * @param mixed $activitygroup int|null the group for calculating the grading summary (if null the function will determine it)
     * @return qbassign_grading_summary renderable object
     */
    public function get_qbassign_grading_summary_renderable($activitygroup = null) {

        $instance = $this->get_default_instance(); // Grading summary requires the raw dates, regardless of relativedates mode.
        $cm = $this->get_course_module();
        $course = $this->get_course();

        $draft = qbassign_SUBMISSION_STATUS_DRAFT;
        $submitted = qbassign_SUBMISSION_STATUS_SUBMITTED;
        ...
        if ($instance->teamsubmission) {
            $warnofungroupedusers = qbassign_grading_summary::WARN_GROUPS_NO;
            ...
            if (count($defaultteammembers) > 0) {
                if ($instance->preventsubmissionnotingroup) {
                    $warnofungroupedusers = qbassign_grading_summary::WARN_GROUPS_REQUIRED;
                } else {
                    $warnofungroupedusers = qbassign_grading_summary::WARN_GROUPS_OPTIONAL;
                }
            }

            $summary = new qbassign_grading_summary(
            ...
            $summary = new qbassign_grading_summary(
                ...
                $this->count_submissions_need_grading($activitygroup),
                $instance->teamsubmission,
                qbassign_grading_summary::WARN_GROUPS_NO,
                $course->relativedatesmode,
                $course->startdate,
                $this->can_grade(),
                $isvisible,
    ...
    private function get_override_data(int $activitygroup) {
        ...
        $params = ['groupid' => $activitygroup, 'qbassignid' => $instanceid];
        $this->overridedata[$cachekey] = $DB->get_record('qbassign_overrides', $params);
        return $this->overridedata[$cachekey];
    }
    ...
    protected function view_submission_page() {
        ...
        $o .= $this->get_renderer()->render(new qbassign_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new qbassign_plugin_header($plugin));
            }
        }

        if ($this->can_view_grades()) {
            $actionbuttons = new \mod_qbassign\output\actionmenu($this->get_course_module()->id);
            $o .= $this->get_renderer()->submission_actionmenu($actionbuttons);

            $summary = $this->get_qbassign_grading_summary_renderable();
        ...
        \mod_qbassign\event\submission_status_viewed::create_from_qbassign($this)->trigger();
     ...
     *
     * @param stdClass $instance The settings for the current instance of this qbassignment
    ...
    public function view_submission_action_bar(stdClass $instance, stdClass $user): string {
        ...
        $actionbuttons = new \mod_qbassign\output\user_submission_actionmenu(
            $this->get_course_module()->id,
            $showsubmit,
            $showedit,
            $submission,
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        if ($this->get_instance()->markingworkflow && !empty($grade) &&
                $this->get_grading_status($grade->userid) != qbassign_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        $qbassign = clone $this->get_instance();
        $qbassign->cmidnumber = $this->get_course_module()->idnumber;
        // Set qbassign gradebook feedback plugin status (enabled and visible).
        $qbassign->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return qbassign_grade_item_update($qbassign, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this qbassignment qualifies as submitted or draft.
        $team = $this->get_submission_group_members($submission->groupid, true);

        $allsubmitted = true;
        $anysubmitted = false;
        $result = true;
        if (!in_array($submission->status, [qbassign_SUBMISSION_STATUS_NEW, qbassign_SUBMISSION_STATUS_REOPENED])) {
            foreach ($team as $member) {
                ...
                if (!$membersubmission || $membersubmission->status != qbassign_SUBMISSION_STATUS_SUBMITTED
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('qbassign_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = qbassign_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $result = $DB->update_record('qbassign_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('qbassign_submission', $submission) && $result;
        }
    ...
    /**
     * Is this qbassignment open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the qbassignment locked?
     *
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == qbassign_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the qbassignment.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->qbassign_files($this->context, $submissionid, $area, $component,
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid) {
            if ($this->submissions_open($userid) &&
                    has_capability('mod/qbassign:submit', $this->context, $graderid)) {
        ...
        if (!has_capability('mod/qbassign:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/qbassign:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/qbassign:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($qbassignmentname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'qbassign', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/qbassign/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/qbassign/view.php?id=' . $coursemodule->id . '">' .
                     format_string($qbassignmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'qbassign', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_qbassignment_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'qbassign') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'qbassign');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->qbassignment = format_string($qbassignmentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/qbassign/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'qbassign', $info);
        ...
        $eventdata->component       = 'mod_qbassign';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->qbassignment;
        $customdata = [
    ...
    public function send_notification($userfrom, $userto, $messagetype, $eventtype, $updatetime) {
        ...
        self::send_qbassignment_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'qbassign');
            return false;
        }
        ...
        if ($submission->status != qbassign_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_qbassign\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_qbassign\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    /**
     * qbassignment submission is processed before grading.
     *
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an qbassignment.
        require_capability('mod/qbassign:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/qbassign:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_qbassign\event\extension_granted::create_from_qbassign($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            $message = get_string('errorquickgradingvsadvancedgrading', 'qbassign');
        ...
        $sql = 'SELECT u.id AS userid, g.grade AS grade, g.timemodified AS lastmodified,
             ...
             LEFT JOIN {qbassign_grades} g ON
                       u.id = g.userid AND
                       g.qbassignment = :qbassignid2 AND
                       g.attemptnumber = gmx.maxattempt
             LEFT JOIN {qbassign_user_flags} uf ON uf.qbassignment = g.qbassignment AND uf.userid = g.userid
                 WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            $message = get_string('errorrecordmodified', 'qbassign');
                            $this->set_error_message($message);
                            return $message;
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('qbassignfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_qbassign\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/qbassign',
                                          $this->course->id,
                                          'mod',
                                          'qbassign',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'qbassign');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/qbassign:revealidentities', $this->context);

        if ($this->get_instance()->revealidentities || empty($this->get_instance()->blindmarking)) {
            return false;
        }

        // Update the qbassignment record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('qbassignfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('qbassign_grades', array('qbassignment'=>$this->get_instance()->id));
        ...
        \mod_qbassign\event\identities_revealed::create_from_qbassign($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $gradingoptionsparams = array('cm'=>$this->get_course_module()->id,
                                      ...
                                      'showonlyactiveenrol' => $this->show_only_active_users(),
                                      'downloadasfolders' => get_user_preferences('qbassign_downloadasfolders', 1));
        $mform = new mod_qbassign_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('qbassign_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('qbassign_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('qbassign_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('qbassign_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('qbassign_quickgrading', isset($formdata->quickgrading));
            }
            if (isset($formdata->downloadasfolders)) {
                set_user_preference('qbassign_downloadasfolders', 1); // Enabled.
            } else {
                set_user_preference('qbassign_downloadasfolders', 0); // Disabled.
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'qbassign', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('gradenoun') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'qbassign');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'qbassign');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'qbassign');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'qbassign', $params) . ' <br>';
    ...
    /**
     * Copy the current qbassignment submission from the last submitted attempt.
     *
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
     * @return bool
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != qbassign_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'qbassign');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'qbassign');
            return false;
        }
        ...
        \mod_qbassign\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted qbassignments
            // and they haven't been notified.
            \mod_qbassign\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    /**
     * Save qbassignment submission for the current user.
     *
     * @param  stdClass $data
     * @param  array $notices Any error messages that should be shown
     *                        to the user.
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        if ($this->new_submission_empty($data)) {
            $notices[] = get_string('submissionempty', 'mod_qbassign');
            return false;
        }
        ...
        if (isset($data->lastmodified) && ($submission->timemodified > $data->lastmodified)) {
            ...
            if ($submission->status !== qbassign_SUBMISSION_STATUS_NEW) {
                $notices[] = $instance->teamsubmission ? get_string('submissionmodifiedgroup', 'mod_qbassign')
                                                       : get_string('submissionmodified', 'mod_qbassign');
                return false;
            }
        }
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            throw new \moodle_exception('submissionslocked', 'qbassign');
            return true;
        }
     ...
     * This is specific to the qbassignment, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'qbassign', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'qbassign');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $strgradelocked = get_string('gradelocked', 'qbassign');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'qbassign');
                }
            } else {
        ...
        $usergrade = get_string('notgraded', 'qbassign');
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'qbassign')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'qbassign'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'qbassign');
            $gradingstatus = $this->get_grading_status($userid);
            if ($gradingstatus != qbassign_MARKING_WORKFLOW_STATE_RELEASED) {
                if ($grade->grade && $grade->grade != -1) {
                    $qbassigngradestring = html_writer::span(
                        make_grades_menu($settings->grade)[grade_floatval($grade->grade)], 'currentgrade'
                    );
                    $label = get_string('currentqbassigngrade', 'qbassign');
                    $mform->addElement('static', 'currentqbassigngrade', $label, $qbassigngradestring);
                }
            }
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            // Only enrolled users could be qbassigned as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/qbassign:grade', 0, 'u.*', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'qbassign'));
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->context);
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker, $viewfullnames);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'qbassign'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'qbassign');
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', qbassign_MARKING_WORKFLOW_STATE_READYFORREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', qbassign_MARKING_WORKFLOW_STATE_INREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', qbassign_MARKING_WORKFLOW_STATE_READYFORRELEASE);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', qbassign_MARKING_WORKFLOW_STATE_RELEASED);
        }

        $gradestring = '<span class="currentgrade">' . $gradestring . '</span>';
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'qbassign'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'qbassign', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'qbassign'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'qbassign'));
        ...
        if (($attemptnumber == -1 ||
            ...
            $this->get_instance()->attemptreopenmethod != qbassign_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'qbassign'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'qbassign');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'qbassign'), $attemptreopenmethod);
            ...
            if ($maxattempts == qbassign_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'qbassign');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'qbassign'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'qbassign'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == qbassign_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == qbassign_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'qbassign'));
                $mform->setDefault('addattempt', 0);
            }
        }
        if (!$gradingpanel) {
            $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'qbassign'));
        } else {
            $mform->addElement('hidden', 'sendstudentnotifications', get_string('sendstudentnotifications', 'qbassign'));
            $mform->setType('sendstudentnotifications', PARAM_BOOL);
        }
        // Get qbassignment visibility information for student.
        $modinfo = get_fast_modinfo($settings->course, $userid);
        $cm = $modinfo->get_cm($this->get_course_module()->id);

        // Don't allow notification to be sent if the student can't access the qbassignment,
        ...
        if (!$cm->uservisible) {
            ...
            $mform->setDefault('sendstudentnotifications', 0);
            if (!$gradingpanel) {
                $mform->disabledIf('sendstudentnotifications', 'workflowstate', 'neq', qbassign_MARKING_WORKFLOW_STATE_RELEASED);
            }
        } else {
        ...
        if (!$gradingpanel) {
            ...
            $buttonarray = array();
            $name = get_string('savechanges', 'qbassign');
            $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
            if (!$last) {
                $name = get_string('savenext', 'qbassign');
            ...
            if ($rownum > 0) {
                $name = get_string('previous', 'qbassign');
                $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
            }

            if (!$last) {
                $name = get_string('nosavebutnext', 'qbassign');
    ...
    public function add_submission_form_elements(MoodleQuickForm $mform, stdClass $data) {
        ...
        $mform->addElement('header', 'submission header', get_string('addsubmission', 'mod_qbassign'));
    ...
    public function remove_submission($userid) {
        ...
        if (!$this->can_edit_submission($userid, $USER->id)) {
            ...
            $message = get_string('usersubmissioncannotberemoved', 'qbassign', fullname($user));
            $this->set_error_message($message);
            return false;
        }
        ...
        $submission->status = $submission->attemptnumber ? qbassign_SUBMISSION_STATUS_REOPENED : qbassign_SUBMISSION_STATUS_NEW;
    ...
    public function revert_to_draft($userid) {
        ...
        \mod_qbassign\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        return true;
    }
    ...
    protected function process_set_batch_marking_workflow_state() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                // notified previously, the student can access the qbassignment, and the state
                // is "Released".
                $modinfo = get_fast_modinfo($this->course, $userid);
                $cm = $modinfo->get_cm($this->get_course_module()->id);
                if ($formdata->sendstudentnotifications && $cm->uservisible &&
                        $state == qbassign_MARKING_WORKFLOW_STATE_RELEASED) {
                    $flags->mailed = 0;
                }

                $gradingdisabled = $this->grading_disabled($userid);

                // Will not apply update if user does not have permission to qbassign this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    ...
                    $qbassign = clone $this->get_instance();
                    $qbassign->cmidnumber = $this->get_course_module()->idnumber;
                    // Set qbassign gradebook feedback plugin status.
                    $qbassign->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_qbassign\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
    ...
    protected function process_set_batch_marking_allocation() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                $flags = $this->get_user_flags($userid, true);
                if ($flags->workflowstate == qbassign_MARKING_WORKFLOW_STATE_READYFORREVIEW ||
                    $flags->workflowstate == qbassign_MARKING_WORKFLOW_STATE_INREVIEW ||
                    $flags->workflowstate == qbassign_MARKING_WORKFLOW_STATE_READYFORRELEASE ||
                    $flags->workflowstate == qbassign_MARKING_WORKFLOW_STATE_RELEASED) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_qbassign\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_qbassign\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function apply_grade_to_user($formdata, $userid, $attemptnumber) {
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_qbassign\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
        ...
        if ($this->get_instance()->markingworkflow &&
                isset($formdata->workflowstate) &&
                $formdata->workflowstate != qbassign_MARKING_WORKFLOW_STATE_RELEASED) {
    ...
    protected function reopen_submission_if_required($userid, $submission, $addattempt) {
        ...
        $maxattemptsreached = !empty($submission) &&
                              ...
                              $instance->maxattempts != qbassign_UNLIMITED_ATTEMPTS;
        $shouldreopen = false;
        if ($instance->attemptreopenmethod == qbassign_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
    ...
    protected function process_save_grade(&$mform) {
        ...
        $userid = optional_param('userid', 0, PARAM_INT);
        if (!$userid) {
            if (empty($SESSION->mod_qbassign_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new moodle_url('/mod/qbassign/view.php', array('id' => $this->get_course_module()->id));
                throw new moodle_exception('useridlistnotcached', 'mod_qbassign', $url);
            }
            $useridlist = $SESSION->mod_qbassign_useridlist[$this->get_useridlist_key($useridlistid)];
        } else {
            $useridlist = array($userid);
            $rownum = 0;
        }
     ...
     * @return bool
     */
    public static function can_upgrade_qbassignment($type, $version) {
        $qbassignment = new qbassign(null, null, null);
        return $qbassignment->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an qbassignment from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old qbassignment files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old qbassignment context id
     * @param int $oldcomponent The old qbassignment component ('qbassignment')
     * @param int $oldfilearea The old qbassignment filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new qbassignment context id
     * @param int $newcomponent The new qbassignment component ('qbassignment')
     * @param int $newfilearea The new qbassignment filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group qbassignment
     * at most 1 time.
     *
    ...
    protected function add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == qbassign_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != qbassign_UNLIMITED_ATTEMPTS &&
        ...
        $newsubmission->status = qbassign_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        global $DB, $CFG;
        $grades = array();
        $qbassignmentid = $this->get_instance()->id;
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('qbassignfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('qbassignid1'=>$qbassignmentid,
                        'qbassignid2'=>$qbassignmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                FROM {user} u
                                                LEFT JOIN {qbassign_submission} s
                                                    ON u.id = s.userid and s.qbassignment = :qbassignid1 AND
                                                    s.latest = 1
                                                JOIN {qbassign_grades} g
                                                    ON u.id = g.userid and g.qbassignment = :qbassignid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - qbassign them a random id.
     *
     * @param int $qbassignid The qbassignid to lookup
     */
    public static function allocate_unique_ids($qbassignid) {
        global $DB;

        $cm = get_coursemodule_from_instance('qbassign', $qbassignid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/qbassign:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('qbassign_user_mapping',
                                      array('qbassignment'=>$qbassignid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->qbassignment = $qbassignid;
                $record->userid = $user->id;
                $DB->insert_record('qbassign_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this qbassignment.
     *
     * @param int $qbassignid The qbassignment id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($qbassignid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('qbassignment'=>$qbassignid, 'userid'=>$userid);
        if ($record = $DB->get_record('qbassign_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($qbassignid);
        ...
        return $DB->insert_record('qbassign_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this qbassignment.
     *
     * @param int $qbassignid The id of the qbassignment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($qbassignid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('qbassign_user_mapping',
                                      array('qbassignment'=>$qbassignid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/qbassign:grade', $this->context)) {
            $states[qbassign_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'qbassign');
            $states[qbassign_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'qbassign');
        }
        if (has_any_capability(array('mod/qbassign:reviewgrades',
                                     'mod/qbassign:managegrades'), $this->context)) {
            $states[qbassign_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'qbassign');
            $states[qbassign_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'qbassign');
        }
        if (has_any_capability(array('mod/qbassign:releasegrades',
                                     'mod/qbassign:managegrades'), $this->context)) {
            $states[qbassign_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'qbassign');
        }
    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            return qbassign_MARKING_WORKFLOW_STATE_NOTMARKED;
        } else {
            $attemptnumber = optional_param('attemptnumber', -1, PARAM_INT);
            $grade = $this->get_user_grade($userid, false, $attemptnumber);

            if (!empty($grade) && $grade->grade !== null && $grade->grade >= 0) {
                return qbassign_GRADING_STATUS_GRADED;
            } else {
                return qbassign_GRADING_STATUS_NOT_GRADED;
            }
        }
    }

    /**
     * The id used to uniquily identify the cache for this instance of the qbassign object.
     *
     * @return string
     */
    ...
    public function set_module_viewed() {
        ...
        $qbassigninstance = $this->get_instance();
        $params = [
            'objectid' => $qbassigninstance->id,
            'context' => $this->get_context()
        ];
        if ($this->is_blind_marking()) {
            $params['anonymous'] = 1;
        ...
        $event->add_record_snapshot('qbassign', $qbassigninstance);
        $event->trigger();
    }

    /**
     * Checks for any grade notices, and adds notifications. Will display on qbassignment main page and grading table.
     *
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/qbassign:grade', $this->get_context()) && get_config('qbassign', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \moodle_url('/mod/qbassign/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
            \core\notification::warning(get_string('fixrescalednullgrades', 'mod_qbassign', ['link' => $link->out()]));
        }
    }
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        require_capability('mod/qbassign:grade', $this->get_context());
        ...
        if ($confirm) {
            ...
            unset_config('has_rescaled_null_grades_' . $instance->id, 'qbassign');

            // Display the notice.
            $o .= $this->get_renderer()->notification(get_string('fixrescalednullgradesdone', 'qbassign'), 'notifysuccess');
            $url = new moodle_url(
                '/mod/qbassign/view.php',
                array(
            ...
            $continue = new \moodle_url('/mod/qbassign/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \moodle_url('/mod/qbassign/view.php', array('id' => $this->get_course_module()->id));
            $o .= $OUTPUT->confirm(get_string('fixrescalednullgradesconfirm', 'mod_qbassign'), $continue, $cancel);
        }
     ...
     * attempt on a group qbassignment, and whether the gradebook should be updated.
     *
     * @since Moodle 3.4
    ...
    public function get_marking_allocation_filters($export = false) {
        ...
        if ($export) {
            $allocationfilter = get_user_preferences('qbassign_markerfilter', '');
            $result = [];
    ...
    public function get_marking_workflow_filters($export = false) {
        ...
        if ($markingworkflow) {
            $notmarked = get_string('markingworkflowstatenotmarked', 'qbassign');
            $markingworkflowoptions[''] = get_string('filternone', 'qbassign');
            $markingworkflowoptions[qbassign_MARKING_WORKFLOW_STATE_NOTMARKED] = $notmarked;
    ...
    public function get_filters() {
        $filterkeys = [
            qbassign_FILTER_NOT_SUBMITTED,
            qbassign_FILTER_DRAFT,
            qbassign_FILTER_SUBMITTED,
            qbassign_FILTER_REQUIRE_GRADING,
            qbassign_FILTER_GRANTED_EXTENSION
        ];

        $current = get_user_preferences('qbassign_filter', '');

        $filters = [];
        // First is always "no filter" option.
        array_push($filters, [
            'key' => 'none',
            'name' => get_string('filternone', 'qbassign'),
            'active' => ($current == '')
        ]);
        ...
        foreach ($filterkeys as $key) {
            array_push($filters, [
                'key' => $key,
                'name' => get_string('filter' . $key, 'qbassign'),
                'active' => ($current == $key)
            ]);
        }
        return $filters;
    ...
    /**
     * Check if time limit for qbassignment enabled and set up.
     *
    ...
    public function is_time_limit_enabled(?int $userid = null): bool {
        $instance = $this->get_instance($userid);
        return get_config('qbassign', 'enabletimelimit') && !empty($instance->timelimit);
    }

    /**
     * Check if an qbassignment submission is already started and not yet submitted.
     *
     * @param int|null $userid User ID. If null, use global user.
    ...
    public function is_attempt_in_progress(?int $userid = null, int $groupid = 0, int $attemptnumber = -1): bool {
        ...
        return !empty($submission) && $submission->status !== qbassign_SUBMISSION_STATUS_SUBMITTED && $timedattemptstarted;
    }
}

/**
 * Portfolio caller class for mod_qbassign.
 *
 * @package   mod_qbassign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbassign_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the qbassignment we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('qbassign', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the qbassignment type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_qbassign');
            }

            $submission = $DB->get_record('qbassign_submission', array('id' => $this->sid));
        } else {
            $submissionid = $DB->get_field('files', 'itemid', array('id' => $this->fileid, 'contextid' => $context->id));
            if ($submissionid) {
                $submission = $DB->get_record('qbassign_submission', array('id' => $submissionid));
            }
        }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'qbassignment.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'qbassign' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return qbassign_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        $qbassignment = new qbassign($context, null, null);
        return $qbassignment->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/qbassign:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'qbassign');
    }
 ...
 * @return void
 */
function qbassign_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.qbassignid, o.groupid
                  FROM {qbassign_overrides} o
                  JOIN {qbassign} qbassign ON qbassign.id = o.qbassignid
                 WHERE qbassign.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all qbassign in course.
        $sql = "SELECT o.id, o.qbassignid, o.groupid
                  FROM {qbassign_overrides} o
                  JOIN {qbassign} qbassign ON qbassign.id = o.qbassignid
    ...
    $DB->delete_records_list('qbassign_overrides', 'id', array_keys($records));
    $cache = cache::make('mod_qbassign', 'overrides');
    foreach ($records as $record) {
        $cache->delete("{$record->qbassignid}_g_{$record->groupid}");
    }
}
 ...
 * @param int $qbassignid of the qbassignment
 * @return bool success of operation
 */
function qbmove_group_override($id, $move, $qbassignid) {
    global $DB;

    // Get the override object.
    if (!$override = $DB->get_record('qbassign_overrides', ['id' => $id, 'qbassignid' => $qbassignid], 'id, sortorder, groupid')) {
        return false;
    }
    // Count the number of group overrides.
    $overridecountgroup = $DB->count_records('qbassign_overrides', array('userid' => null, 'qbassignid' => $qbassignid));
    ...
    $params = ['sortorder' => $neworder, 'qbassignid' => $qbassignid];
    if ($swapoverride = $DB->get_record('qbassign_overrides', $params, 'id, sortorder, groupid')) {
        ...
        $DB->update_record('qbassign_overrides', $override);
        $DB->update_record('qbassign_overrides', $swapoverride);

        // Delete cache for the 2 records we updated above.
        $cache = cache::make('mod_qbassign', 'overrides');
        $cache->delete("{$qbassignid}_g_{$override->groupid}");
        $cache->delete("{$qbassignid}_g_{$swapoverride->groupid}");
    }

    qbreorder_group_overrides($qbassignid);
    return true;
}
 ...
 *
 * @param int $qbassignid of the assigment
 */
function qbreorder_group_overrides($qbassignid) {
    global $DB;

    $i = 1;
    if ($overrides = $DB->get_records('qbassign_overrides', array('userid' => null, 'qbassignid' => $qbassignid), 'sortorder ASC')) {
        $cache = cache::make('mod_qbassign', 'overrides');
        foreach ($overrides as $override) {
            $f = new stdClass();
            $f->id = $override->id;
            $f->sortorder = $i++;
            $DB->update_record('qbassign_overrides', $f);
            $cache->delete("{$qbassignid}_g_{$override->groupid}");

            // Update priorities of group overrides.
            $params = [
                'modulename' => 'qbassign',
                'instance' => $override->qbassignid,
...
/**
 * Get the information about the standard qbassign JavaScript module.
 * @return array a standard jsmodule structure.
 */
function qbassign_get_js_module() {
    return array(
        'name' => 'mod_qbassign',
        'fullpath' => '/mod/qbassign/module.js',
    );
}
