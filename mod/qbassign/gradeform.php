...
/**
 * qbassignment grade form
 *
 * @package   mod_qbassign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_qbassign_grade_form extends moodleform {
    /** @var qbassignment $qbassignment */
    ...
    public function definition() {
        ...
        list($qbassignment, $data, $params) = $this->_customdata;
        // Visible elements.
        $this->qbassignment = $qbassignment;
        $qbassignment->add_grade_form_elements($mform, $data, $params);
    ...
    public function validation($data, $files) {
        ...
        $instance = $this->qbassignment->get_instance();
        ...
        if ($instance->grade > 0) {
            if (unformat_float($data['grade'], true) === false && (!empty($data['grade']))) {
                $errors['grade'] = get_string('invalidfloatforgrade', 'qbassign', $data['grade']);
            } else if (unformat_float($data['grade']) > $instance->grade) {
                $errors['grade'] = get_string('gradeabovemaximum', 'qbassign', $instance->grade);
            } else if (unformat_float($data['grade']) < 0) {
                $errors['grade'] = get_string('gradebelowzero', 'qbassign');
            }
        } else {
            // This is a scale.
            if ($scale = $DB->get_record('scale', array('id'=>-($instance->grade)))) {
                ...
                if ((int)$data['grade'] !== -1 && !array_key_exists((int)$data['grade'], $scaleoptions)) {
                    $errors['grade'] = get_string('invalidgradeforscale', 'qbassign');
                }
            }
        }
        return $errors;
