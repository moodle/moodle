...
class user_submission_actionmenu implements templatable, renderable {
    ...
    protected function get_current_status(): string {
        if (!is_null($this->teamsubmission) && isset($this->teamsubmission->status)) {
            ...
            return qbassign_SUBMISSION_STATUS_NEW;
        }
    }
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if ($this->showedit) {
            $url = new moodle_url('/mod/qbassign/view.php', ['id' => $this->cmid, 'action' => 'editsubmission']);
            $button = new single_button($url, get_string('editsubmission', 'mod_qbassign'), 'get');
            $data['edit'] = [
                'button' => $button->export_for_template($output),
            ];
            $status = $this->get_current_status();
            if ($status !== qbassign_SUBMISSION_STATUS_NEW && $status !== qbassign_SUBMISSION_STATUS_REOPENED) {
                $url = new moodle_url('/mod/qbassign/view.php', ['id' => $this->cmid, 'action' => 'removesubmissionconfirm']);
                $button = new single_button($url, get_string('removesubmission', 'mod_qbassign'), 'get');
                $data['remove'] = ['button' => $button->export_for_template($output)];
            }
            if ($status === qbassign_SUBMISSION_STATUS_REOPENED) {
                $params = ['id' => $this->cmid, 'action' => 'editprevioussubmission', 'sesskey' => sesskey()];
                $url = new moodle_url('/mod/qbassign/view.php', $params);
                $button = new single_button($url, get_string('addnewattemptfromprevious', 'mod_qbassign'), 'get');
                $help = new help_icon('addnewattemptfromprevious', 'mod_qbassign');
                $data['previoussubmission'] = [
                    'button' => $button->export_for_template($output),
                    'help' => $help->export_for_template($output)
                ];
                $url = new moodle_url('/mod/qbassign/view.php', ['id' => $this->cmid, 'action' => 'editsubmission']);
                $newattemptbutton = new single_button($url, get_string('addnewattempt', 'mod_qbassign'), 'get', true);
                $newattempthelp = new help_icon('addnewattempt', 'mod_qbassign');
                $data['edit']['button'] = $newattemptbutton->export_for_template($output);
                $data['edit']['help'] = $newattempthelp->export_for_template($output);
            }
            if ($status === qbassign_SUBMISSION_STATUS_NEW) {

                if ($this->timelimit && empty($this->submission->timestarted)) {
                    $confirmation = new \confirm_action(
                        get_string('confirmstart', 'qbassign', format_time($this->timelimit)),
                        null,
                        get_string('beginqbassignment', 'qbassign')
                    );
                    $urlparams = array('id' => $this->cmid, 'action' => 'editsubmission');
                    $beginbutton = new \action_link(
                        new moodle_url('/mod/qbassign/view.php', $urlparams),
                            get_string('beginqbassignment', 'qbassign'),
                            $confirmation,
                    ...
                    $newattemptbutton = new single_button($url, get_string('addsubmission', 'mod_qbassign'), 'get', true);
                    $data['edit']['button'] = $newattemptbutton->export_for_template($output);
                    $data['edit']['help'] = '';
                }
            }
        ...
        if ($this->showsubmit) {
            $url = new moodle_url('/mod/qbassign/view.php', ['id' => $this->cmid, 'action' => 'submit']);
            $button = new single_button($url, get_string('submitqbassignment', 'mod_qbassign'), 'get', true);
            $help = new help_icon('submitqbassignment', 'mod_qbassign');
            $data['submit'] = [
                'button' => $button->export_for_template($output),
                'help' => $help->export_for_template($output)
            ];
