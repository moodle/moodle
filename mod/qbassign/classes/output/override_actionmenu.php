...
class override_actionmenu implements templatable, renderable {
    ...
    protected function get_select_menu(): url_select {
        $userlink = new moodle_url('/mod/qbassign/overrides.php', ['cmid' => $this->cm->id, 'mode' => 'user']);
        $grouplink = new moodle_url('/mod/qbassign/overrides.php', ['cmid' => $this->cm->id, 'mode' => 'group']);
        $menu = [
            $userlink->out(false) => get_string('useroverrides', 'mod_qbassign'),
            $grouplink->out(false) => get_string('groupoverrides', 'mod_qbassign'),
        ];
        return new url_select($menu, $this->currenturl->out(false), null, 'mod_qbassign_override_select');
    }

    /**
     * Whether to show groups or not. qbassignments can be have group overrides if there are groups available in the course.
     * There is no restriction related to the qbassignment group setting.
     *
     * @return bool
     */
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if ($type == 'user') {
            $text = get_string('addnewuseroverride', 'mod_qbassign');
        } else {
            $text = get_string('addnewgroupoverride', 'mod_qbassign');
        }
        $action = ($type == 'user') ? 'adduser' : 'addgroup';

        $params = ['cmid' => $this->currenturl->get_param('cmid'), 'action' => $action];
        $url = new moodle_url('/mod/qbassign/overrideedit.php', $params);
