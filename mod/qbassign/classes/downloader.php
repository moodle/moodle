...
use mod_qbassign\event\all_submissions_downloaded;
 ...
 */
class downloader {
    /** @var qbassign the module manager instance. */
    private $manager;

    /** @var stdClass the qbassign instance record. */
    private $instance;
     ...
     * Class constructor.
     *
     * @param qbassign $manager the instance manager
     * @param array|null $userids the user ids to download.
     */
    public function __construct(qbassign $manager, ?array $userids = null) {
        $this->manager = $manager;
        $this->userids = $userids;
        $this->instance = $manager->get_instance();

        $this->downloadasfolders = get_user_preferences('qbassign_downloadasfolders', 1);
    ...
    private function get_student_prefix(stdClass $student): string {
        ...
        if ($this->instance->teamsubmission) {
            ...
            if ($submissiongroup) {
                ...
                $groupname = get_string('defaultteam', 'mod_qbassign');
                $groupinfo = '';
            }
        ...
        if ($manager->is_blind_marking()) {
            $fullname = get_string('participant', 'mod_qbassign');
        } else {
     ...
     *
     * @param stdClass $student the user record
     * @param qbassign_plugin $plugin the submission plugin instance
    ...
    private function load_submissionplugin_filelist(
        ...
        if ($this->downloadasfolders) {
            // Create a folder for each user for each qbassignment plugin.
            ...
            // Create a single folder for all users of all qbassignment plugins.
    ...
    public function download_zip() {
        ...
        all_submissions_downloaded::create_from_qbassign($this->manager)->trigger();
        ...
        foreach ($this->filesforzipping as $pathinzip => $file) {
            if ($file instanceof stored_file) {
                ...
                // Save $file as contents, from onlinetex subplugin.
                $content = reset($file);
