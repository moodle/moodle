...
/**
 * The mod_qbassign remove submission form viewed event.
 *
...
/**
 * The mod_qbassign remove submission form viewed event class.
 *
 * @package    mod_qbassign
 * @copyright  2019 Damyon Wiese
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class remove_submission_form_viewed extends base {
    ...
    public static function get_name() {
        return get_string('eventremovesubmissionformviewed', 'mod_qbassign');
    }
    ...
    public function get_description() {
        ...
        return "The user with id '$this->userid' viewed their remove submission form for the qbassignment with course module id " .
    ...
    protected function get_legacy_logdata() {
        if ($this->relateduserid == $this->userid) {
            $title = get_string('removesubmission', 'qbassign');
        } else {
            $user = $this->get_record_snapshot('user', $this->relateduserid);
            $title = get_string('removesubmissionforstudent', 'qbassign', array('id' => $user->id, 'fullname' => fullname($user)));
        }
        $this->set_legacy_logdata('view remove submission qbassignment form', $title);
