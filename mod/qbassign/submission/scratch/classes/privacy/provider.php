...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        $detail = [
                    ...
                    'scratch' => 'privacy:metadata:textpurpose'
                  ];
        $collection->add_database_table('qbassignsubmission_scratch', $detail, 'privacy:metadata:tablepurpose');
    ...
    public static function export_submission_user_data(qbassign_plugin_request_data $exportdata) {
        ...
        $plugin = $qbassign->get_plugin_by_type('qbassignsubmission', 'scratch');
        $submission = $exportdata->get_pluginobject();
        $editortext = $plugin->get_editor_text('scratch', $submission->id);
        $context = $exportdata->get_context();
        if (!empty($editortext)) {
            $submissiontext = new \stdClass();
            $currentpath = $exportdata->get_subcontext();
            $currentpath[] = get_string('privacy:path', 'qbassignsubmission_scratch');
            $submissiontext->text = writer::with_context($context)->rewrite_pluginfile_urls($currentpath,
                    'qbassignsubmission_scratch', 'submissions_scratch', $submission->id, $editortext);
            writer::with_context($context)
                    ->export_area_files($currentpath, 'qbassignsubmission_scratch', 'submissions_scratch', $submission->id)
    ...
    public static function delete_submission_for_context(qbassign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, 'qbassignsubmission_scratch',
                qbassignsubmission_scratch_FILEAREA);

        // Delete the records in the table.
        $DB->delete_records('qbassignsubmission_scratch', ['qbassignment' => $requestdata->get_qbassignid()]);
    }
    ...
    public static function delete_submission_for_userid(qbassign_plugin_request_data $deletedata) {
        ...
        // Delete related files.
        $fs = get_file_storage();
        $fs->delete_area_files($deletedata->get_context()->id, 'qbassignsubmission_scratch', qbassignsubmission_scratch_FILEAREA,
                $submissionid);

        // Delete the records in the table.
        $DB->delete_records('qbassignsubmission_scratch', ['qbassignment' => $deletedata->get_qbassignid(),
    ...
    public static function delete_submissions(qbassign_plugin_request_data $deletedata) {
        ...
        $fs->delete_area_files_select($deletedata->get_context()->id,
                'qbassignsubmission_scratch', qbassignsubmission_scratch_FILEAREA, $sql, $params);

        $params['qbassignid'] = $deletedata->get_qbassignid();
        $DB->delete_records_select('qbassignsubmission_scratch', "qbassignment = :qbassignid AND submission $sql", $params);
    }
}
