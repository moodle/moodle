...
/**
 * Unit tests for qbassignsubmission_comments.
 *
...
/**
 * Unit tests for mod/qbassign/submission/comments/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider_test extends \mod_qbassign\privacy\provider_test {
     ...
     */
    protected function create_comment_submission($qbassign, $student, $submissiontext) {

        $submission = $qbassign->get_user_submission($student->id, true);

        $plugin = $qbassign->get_submission_plugin_by_type('comments');

        $context = $qbassign->get_context();
        $options = new \stdClass();
        $options->area = 'submission_comments';
        $options->course = $qbassign->get_course();
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('qbassignsubmission_comments');
        $collection = \qbassignsubmission_comments\privacy\provider::get_metadata($collection);
        $this->assertNotEmpty($collection);
    }

    /**
     * Test returning the context for a user who has made a comment in an qbassignment.
     */
    public function test_get_context_for_userid_within_submission() {
        ...
        list($plugin, $submission, $comment) = $this->create_comment_submission($qbassign, $user1, $studentcomment);
        $teachercomment = 'From the teacher';
        $this->setUser($user2);
        $comment->add($teachercomment);

        $contextlist = new \core_privacy\local\request\contextlist();
        \qbassignsubmission_comments\privacy\provider::get_context_for_userid_within_submission($user2->id, $contextlist);
    ...
    public function test_get_student_user_ids() {
        ...
        $useridlist = new useridlist($user2->id, $qbassign->get_instance()->id);
        \qbassignsubmission_comments\privacy\provider::get_student_user_ids($useridlist);
    ...
    public function test_get_userids_from_context() {
        ...
        $qbassigncontext1 = $qbassign1->get_context();
        $qbassigncontext2 = $qbassign2->get_context();

        $user1comment = 'Comment from user 1';
        list($plugin, $submission, $comment) = $this->create_comment_submission($qbassign1, $user1, $user1comment);
        ...
        list($plugin, $submission, $comment) = $this->create_comment_submission($qbassign2, $user4, $user4comment);
        $user3secondcomment = 'Comment on user 4 post.';
        $this->setUser($user3);
        $comment->add($user3comment);

        $userlist = new \core_privacy\local\request\userlist($qbassigncontext1, 'qbassignsubmission_comments');
        \qbassignsubmission_comments\privacy\provider::get_userids_from_context($userlist);
    ...
    public function test_export_submission_user_data() {
        ...
        $exportdata = new \mod_qbassign\privacy\qbassign_plugin_request_data($context, $qbassign, $submission);
        \qbassignsubmission_comments\privacy\provider::export_submission_user_data($exportdata);
    ...
    public function test_delete_submission_for_context() {
        ...
        list($plugin2, $submission2, $comment2) = $this->create_comment_submission($qbassign, $user2, $studentcomment);
        ...
        \qbassignsubmission_comments\privacy\provider::delete_submission_for_context($requestdata);
    ...
    public function test_delete_submission_for_userid() {
        ...
        $requestdata = new \mod_qbassign\privacy\qbassign_plugin_request_data($context, $qbassign, null, [], $user1);
        \qbassignsubmission_comments\privacy\provider::delete_submission_for_userid($requestdata);
    ...
    public function test_delete_submissions() {
        ...
        $results = $DB->get_records('comments', ['contextid' => $qbassigncontext1->id]);
        $this->assertCount(3, $results);

        $deletedata = new \mod_qbassign\privacy\qbassign_plugin_request_data($qbassigncontext1, $qbassign1);
        $deletedata->set_userids([$user1->id, $user3->id]);
        \qbassignsubmission_comments\privacy\provider::delete_submissions($deletedata);
