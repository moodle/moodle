...
namespace qbassignsubmission_comments\privacy;
...
class provider implements metadataprovider,
    ...
    public static function get_context_for_userid_within_submission(int $userid, contextlist $contextlist) {
        $sql = "SELECT contextid
                 ...
                 WHERE component = :component
                       ...
                       AND userid = :userid";
        $params = ['userid' => $userid, 'component' => 'qbassignsubmission_comments', 'commentarea' => 'submission_comments'];
        $contextlist->add_from_sql($sql, $params);
    }

    /**
     * Due to the fact that we can't rely on the queries in the mod_qbassign provider we have to add some additional sql.
     *
    ...
    public static function get_student_user_ids(\mod_qbassign\privacy\useridlist $useridlist) {
        $params = ['qbassignid' => $useridlist->get_qbassignid(), 'commentuserid' => $useridlist->get_teacherid(),
                'commentuserid2' => $useridlist->get_teacherid()];
        $sql = "SELECT DISTINCT c.userid AS id
                  FROM {comments} c
                  JOIN (SELECT c.itemid
                          FROM {comments} c
                          JOIN {qbassign_submission} s ON s.id = c.itemid AND s.qbassignment = :qbassignid
    ...
    public static function get_userids_from_context(\core_privacy\local\request\userlist $userlist) {
        ...
        comments_provider::get_users_in_context_from_sql($userlist, 'c', 'qbassignsubmission_comments', 'submission_comments',
                $context->id);
    }
     ...
     *
     * @param  qbassign_plugin_request_data $requestdata Data to fulfill the deletion request.
     */
    public static function delete_submission_for_context(qbassign_plugin_request_data $requestdata) {
        comments_provider::delete_comments_for_all_users($requestdata->get_context(), 'qbassignsubmission_comments',
                'submission_comments');
    }
     ...
     *
     * @param  qbassign_plugin_request_data $exportdata Details about the user and context to focus the deletion.
     */
    public static function delete_submission_for_userid(qbassign_plugin_request_data $exportdata) {
        // Create an approved context list to delete the comments.
        $contextlist = new \core_privacy\local\request\approved_contextlist($exportdata->get_user(), 'qbassignsubmission_comments',
            [$exportdata->get_context()->id]);
        comments_provider::delete_comments_for_user($contextlist, 'qbassignsubmission_comments', 'submission_comments');
    }
    ...
    public static function delete_submissions(qbassign_plugin_request_data $deletedata) {
        $userlist = new \core_privacy\local\request\approved_userlist($deletedata->get_context(), 'qbassignsubmission_comments',
                $deletedata->get_userids());
        comments_provider::delete_comments_for_users($userlist, 'qbassignsubmission_comments', 'submission_comments');
    }
