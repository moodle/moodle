 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbassign_submission_comments extends qbassign_submission_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'qbassignsubmission_comments');
    }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        $options->course    = $this->qbassignment->get_course();
        $options->context = $this->qbassignment->get_context();
        $options->itemid  = $submission->id;
        $options->component = 'qbassignsubmission_comments';
        $options->showcount = true;
        $options->displaycancel = true;

        $comment = new comment($options);

        $o = $this->qbassignment->get_renderer()->container($comment->output(true), 'commentscontainer');
        return $o;
    }
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldqbassignment, & $log) {
        if ($oldqbassignment->qbassignmenttype == 'upload') {
            // Disable if allow notes was not enabled.
            if (!$oldqbassignment->var2) {
                $this->disable();
            }
        }
        return true;
