 ...
 * @return bool
 */
function qbassignsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('qbassign_submission', array('id'=>$options->itemid))) {
        throw new comment_exception('invalidcommentitemid');
    }
    $context = $options->context;

    require_once($CFG->dirroot . '/mod/qbassign/locallib.php');
    static $qbassignment = null;
    if (is_null($qbassignment) || $qbassignment->get_context() != $context) {
        $qbassignment = new qbassign($context, null, null);
    }

    if ($qbassignment->get_instance()->id != $submission->qbassignment) {
 ...
 * @return array
 */
function qbassignsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($qbassignment->get_instance()->teamsubmission &&
        !$qbassignment->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$qbassignment->get_instance()->teamsubmission &&
        !$qbassignment->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
 ...
 */
function qbassignsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($qbassignment->is_blind_marking() && !empty($comments)) {
        ...
        $userinteam = false;
        if ($qbassignment->get_instance()->teamsubmission && has_capability('mod/qbassign:submit', $context)) {
            $qbassignment->set_course(get_course($course));
            $userinteam = $qbassignment->can_edit_group_submission($submission->groupid);
        }

        foreach ($comments as $comment) {

            if (has_capability('mod/qbassign:viewblinddetails', $context) && $USER->id != $comment->userid) {
                $anonid = $qbassignment->get_uniqueid_for_user($comment->userid);
                ...
                $comment->fullname = get_string('blindmarkingviewfullname', 'qbassignsubmission_comments', $a);
                ...
                if (empty($usermappings[$comment->userid])) {
                    ...
                    $commenter->firstname = get_string('blindmarkingname', 'qbassignsubmission_comments', $anonid);
                    $commenter->lastname = '';
                    ...
                    $usermappings[$comment->userid]->avatar = $qbassignment->get_renderer()->user_picture($commenter,
 ...
 */
function qbassignsubmission_comments_comment_add(stdClass $comment, stdClass $param) {
