...
define([
    ...
    'core/ajax',
    'core/str',
    'mod_qbassign/grading_form_change_checker',
    'mod_qbassign/grading_events',
    'core_form/events',
    'core/toast',
     ...
     * GradingPanel class.
     *
     * @class mod_qbassign/grading_panel
    ...
    GradingPanel.prototype._submitForm = function(event, nextUserId, nextUser) {
        ...
        var qbassignmentid = this._region.attr('data-qbassignmentid');

        // Now we can continue...
        ajax.call([{
            methodname: 'mod_qbassign_submit_grading_form',
            args: {qbassignmentid: qbassignmentid, userid: this._lastUserId, jsonformdata: JSON.stringify(data)},
    ...
    GradingPanel.prototype._handleFormSubmissionResponse = function(formdata, nextUserId, nextUser, response) {
        ...
        if (response.length) {
            ...
            str.get_string('gradechangessaveddetail', 'mod_qbassign')
            .then(function(str) {
                Toast.add(str);
                return str;
            })
    ...
    GradingPanel.prototype._chooseAttempt = function(e) {
        ...
        str.get_strings([
            {key: 'viewadifferentattempt', component: 'mod_qbassign'},
    ...
    GradingPanel.prototype._addPopoutButtons = function(selector) {
        ...
        templates.render('mod_qbassign/popout_button', {}).done(function(html) {
    ...
    GradingPanel.prototype._refreshGradingPanel = function(event, userid, submissiondata, attemptnumber) {
        ...
        window.M.util.js_pending('mod-qbassign-loading-user');
        ...
        templates.render('mod_qbassign/loading', {}).done(function(html, js) {
            ...
            this._niceReplaceNodeContents(this._region, html, js).done(function() {
                if (userid > 0) {
                    ...
                    fragment.loadFragment('mod_qbassign', 'gradingpanel', contextid, params).done(function(html, js) {
                        ...
                        .done(function() {
                            ...
                            window.M.util.js_complete('mod-qbassign-loading-user');
                        }.bind(this))
                        .fail(notification.exception);
                    }.bind(this)).fail(notification.exception);
