...
require_once($CFG->dirroot.'/mod/blti/TrivialStore.php');

error_reporting(E_ALL & ~E_NOTICE);
ini_set("display_errors", 1);

$PAGE->set_context(get_context_instance(CONTEXT_SYSTEM));
$PAGE->set_url('/mod/blti/service.php');
...
if (! $basiclti = $DB->get_record('blti', array('id'=>$placement))) {
    do_error("Bad sourcedid (4)");
}

$basiclti_types_config = (object)$basiclti_types_config;

$typeconfig = blti_get_type_config($basiclti->typeid);
...
if ($message_type == "basicoutcome") {
    ...
    $record = $DB->get_record('blti', array('id'=>$basiclti->id));
    $record->setting = $setting;
    $success = $DB->update_record('blti', $record);
    if ($success) {
