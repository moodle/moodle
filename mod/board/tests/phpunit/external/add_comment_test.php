...
/**
 * Test external method for adding of note comments.
 *
 * @package    mod_board
 * @copyright  2025 Brickfield Education Labs <https://www.brickfield.ie/>
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers     \mod_board\external\add_comment
 */
final class add_comment_test extends \advanced_testcase {
    public function test_execute(): void {
        ...
        $result = add_comment::execute($note1x1->id, 'Great job');
        $result = add_comment::clean_returnvalue(add_comment::execute_returns(), $result);
        ...
        $this->assertTimeCurrent($comment->timecreated);

        $this->setCurrentTimeStart();
        $result = add_comment::execute($note1x1->id, 'Really');
        $result = add_comment::clean_returnvalue(add_comment::execute_returns(), $result);
        $this->assertSame([], $result['warnings']);
        $comment = $DB->get_record('board_comments', ['id' => $result['id']]);
        $this->assertSame($note1x1->id, $comment->noteid);
        ...
        $this->setCurrentTimeStart();
        $result = add_comment::execute($note1x1->id, 'OK');
        $result = add_comment::clean_returnvalue(add_comment::execute_returns(), $result);
        $this->assertSame([], $result['warnings']);
        $comment = $DB->get_record('board_comments', ['id' => $result['id']]);
        $this->assertSame($note1x1->id, $comment->noteid);
        $this->assertSame($student2->id, $comment->userid);
        $this->assertSame('OK', $comment->content);
        $this->assertSame('0', $comment->deleted);
        $this->assertTimeCurrent($comment->timecreated);
