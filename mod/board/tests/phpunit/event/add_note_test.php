 ...
 * @covers     \mod_board\event\add_note
 */
final class add_note_test extends \advanced_testcase {
    public function test_event(): void {
        ...
        set_config('addnotetolog', 1, 'mod_board');
        ...
        $event = $events[0];
        $this->assertInstanceOf(\mod_board\event\add_note::class, $event);
        ...
        $this->assertSame('Post added', $event->get_name());
        ...
        set_config('addnotetolog', 0, 'mod_board');
        set_config('addheadingtolog', 0, 'mod_board');
        set_config('addattachmenttolog', 0, 'mod_board');

        $sink = $this->redirectEvents();
        note::create($column->id, $user->id, 0, 'NH 1', 'NC 1', []);
        $events = $sink->get_events();
        $sink->close();
        $this->assertCount(1, $events);
        $event = $events[0];
        $this->assertSame(null, $event->other['content']);
        $this->assertSame(null, $event->other['heading']);
        $this->assertSame(null, $event->other['attachment']);
        $this->assertIsString($event->get_description());
    }
}
