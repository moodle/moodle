...
/**
 * Kaltura video presentation formslib class.
 *
...
class mod_kalvidpres_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $mform->setDefault('video_entry_id', 'x');
        $mform->setType('video_entry_id', PARAM_INT);

        $attr = array('id' => 'doc_entry_id');
        $mform->addElement('hidden', 'doc_entry_id', '', $attr);
        $mform->setDefault('doc_entry_id', 'x');
        $mform->setType('doc_entry_id', PARAM_INT);
    ...
    private function add_video_definition($mform) {
        ...
        $videogroup = array();
        if ($addinstance) {
            $videogroup[] =& $mform->createElement('button', $this->addvideobutton, get_string('add_video', 'kalvidpres'));
        } else {
            $videogroup[] =& $mform->createElement('button', $this->addvideobutton, get_string('replace_video', 'kalvidpres'));
        }
    ...
    private function get_thumbnail_markup($hide = false) {
        ...
        $alt    = get_string('add_video', 'kalvidpres');
        $title  = get_string('add_video', 'kalvidpres');
    ...
    private function get_iframe_video_preview_markup($hide = true) {
        ...
        if (!empty($source)) {
            ...
            $url = new moodle_url('/mod/kalvidpres/lti_launch.php', $ltiparams);
    ...
    public function validation($data, $files) {
        ...
        if (empty($data['source'])) {
            $errors['add_video_thumb'] = get_string('novidsource', 'kalvidpres');
        }
