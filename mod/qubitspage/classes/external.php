 ...
 * @since      Moodle 3.0
 */
class mod_qubitspage_external extends external_api {
     ...
     * @since Moodle 3.0
     */
    public static function view_qubitspage_parameters() {
     ...
     */
    public static function view_qubitspage($pageid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/qubitspage/lib.php");

        $params = self::validate_parameters(self::view_qubitspage_parameters(),
                                            array(
                                                'pageid' => $pageid
                                            ));
        $warnings = array();
        ...
        $page = $DB->get_record('qubitspage', array('id' => $params['pageid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($page, 'qubitspage');
     ...
     * @since Moodle 3.0
     */
    public static function view_qubitspage_returns() {
     ...
     * @since Moodle 3.3
     */
    public static function get_qubitspages_by_courses_parameters() {
        return new external_function_parameters (
            array(
     ...
     * @since Moodle 3.3
     */
    public static function get_qubitspages_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            foreach ($pages as $page) {
                helper_for_get_mods_by_courses::format_name_and_intro($page, 'mod_qubitspage');
                ...
                list($page->content, $page->contentformat) = external_format_text(
                        ...
                        $context->id, 'mod_qubitspage', 'content', $page->revision, ['noclean' => true]);
                $page->contentfiles = external_util::get_area_files($context->id, 'mod_qubitspage', 'content');
     ...
     * @since Moodle 3.3
     */
    public static function get_qubitspages_by_courses_returns() {
        return new external_single_structure(
            array(
