...
define('QTYPE_ddmarker_DRAGIMAGE_MAXWIDTH', 150);
define('QTYPE_ddmarker_DRAGIMAGE_MAXHEIGHT', 100);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddmarker extends question_type {
    ...
    public function get_question_options($question) {
        global $DB;
        $question->options = $DB->get_record('qtype_ddmarker',
                array('questionid' => $question->id), '*', MUST_EXIST);
        $question->options->drags = $DB->get_records('qtype_ddmarker_drags',
                array('questionid' => $question->id), 'no ASC', '*');
        $question->options->drops = $DB->get_records('qtype_ddmarker_drops',
                array('questionid' => $question->id), 'no ASC', '*');
        parent::get_question_options($question);
    }

    protected function make_choice($dragdata) {
        return new qtype_ddmarker_drag_item($dragdata->label, $dragdata->no,
                                        $dragdata->draggroup, $dragdata->infinite, $dragdata->id);
    }

    protected function make_place($dropzonedata) {
        return new qtype_ddmarker_drop_zone($dropzonedata->label, $dropzonedata->no,
                                                    $dropzonedata->group,
            ...
            if ($info['filecount'] > 0 || !empty($formdata->drags[$dragno]['draglabel'])) {
                ...
                if ($formdata->dragitemtype[$dragno] == 'image') {
                    self::constrain_image_size_in_draft_area($draftitemid,
                                        QTYPE_ddmarker_DRAGIMAGE_MAXWIDTH,
                                        QTYPE_ddmarker_DRAGIMAGE_MAXHEIGHT);
                    file_save_draft_area_files($draftitemid, $formdata->context->id,
                                        'qtype_ddmarker', 'dragimage', $drag->id,
                                        array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 1));
                } else {
                    //delete any existing files for draggable text item type
                    $fs = get_file_storage();
                    $fs->delete_area_files($formdata->context->id, 'qtype_ddmarker',
                                                                'dragimage', $drag->id);
                }
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $dragids = $DB->get_records_menu('qtype_ddmarker_drags',
                                                array('questionid' => $questionid), 'id', 'id,1');
        foreach ($dragids as $dragid => $notused) {
            $fs->move_area_files_to_new_context($oldcontextid,
                                    $newcontextid, 'qtype_ddmarker', 'dragimage', $dragid);
        }
    ...
    protected function delete_files($questionid, $contextid) {
        ...
        $dragids = $DB->get_records_menu('qtype_ddmarker_drags',
                                                array('questionid' => $questionid), 'id', 'id,1');
        foreach ($dragids as $dragid => $notused) {
            $fs->delete_area_files($contextid, 'qtype_ddmarker', 'dragimage', $dragid);
        }
