...
class mnet_profile_form extends moodleform {
    ...
    function definition() {
        ...
        $defaultstr = get_string('defaultfields', 'mnet');

        $select = &$mform->addElement('select', 'importfields', get_string('importfields', 'mnet'), $fields['optional']);
        $select->setMultiple(true);
        $mform->setHelpButton('importfields', array('importfields', $defaultstr . ': ' . str_replace(',', ', ', $CFG->mnetprofileimportfields)));

        $mform->addElement('checkbox', 'importdefault', get_string('leavedefault', 'mnet'));

        $select = &$mform->addElement('select', 'exportfields', get_string('exportfields', 'mnet'), $fields['optional']);
        $select->setMultiple(true);

        $mform->setHelpButton('exportfields', array('exportfields', $defaultstr . ': ' . str_replace(',', ', ', $CFG->mnetprofileexportfields)));

        $mform->addElement('checkbox', 'exportdefault', get_string('leavedefault', 'mnet'));
