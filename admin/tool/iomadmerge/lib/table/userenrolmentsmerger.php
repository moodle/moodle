    ...
    public function merge($data, &$actionLog, &$errorMessages)
    {
        ...
        if (!empty($enrolmentsToUpdate)) { // it's possible we won't have any
            ...
            if ($DB->execute($sql)) {
                ...
                $errorMessages[] = get_string('tableko', 'tool_iomadmerge', "user_enrolments (#1)") .
                        ': ' . $DB->get_last_error();
            }
        }
        ...
        if (!empty($idsToDisable)) {
            ...
            if ($DB->execute($sql)) {
                ...
                // a database error occurred.
                $errorMessages[] = get_string('tableko', 'tool_iomadmerge', "user_enrolments (#2)") .
                        ': ' . $DB->get_last_error();
            }
        }
        ...
        if (!empty($enrolmentsToReactivate)) {
            ...
            if ($DB->execute($sql)) {
                ...
                // a database error occurred.
                $errorMessages[] = get_string('tableko', 'tool_iomadmerge', "user_enrolments (#3)") .
                        ': ' . $DB->get_last_error();
            }
        }
