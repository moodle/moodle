...
Feature: Optional policies
  ...
  Scenario: Configuring a iomadpolicy as optional
    ...
    # Optional status can be set when creating a new iomadpolicy.
    And I set the following fields to these values:
      | Name                                      | ConsentPageOptional1  |
      | Version                                   | v1                    |
      | Summary                                   | Policy summary        |
      | Full iomadpolicy                               | Full text             |
      | Active                                    | 1                     |
      | Show iomadpolicy before showing other policies | No                    |
      | Agreement optional                        | Yes                   |
    When I press "Save"
    Then the following should exist in the "tool-iomadpolicy-managedocs-wrapper" table:
      | Name                                                    | Policy status | Version |
      | ConsentPageOptional1 Site iomadpolicy, All users, Optional   | Active        | v1      |
    ...
    And I press "Save"
    And the following should exist in the "tool-iomadpolicy-managedocs-wrapper" table:
      | Name                                                    | Policy status | Version |
      | ConsentPageOptional1 Site iomadpolicy, All users, Compulsory | Active        | v1      |
  ...
  Scenario: Users can withdraw an accepted optional iomadpolicy and re-accept it again (js off)
    ...
    Then I should see "Withdrawing iomadpolicy"
    ...
    And I should see "Accepting iomadpolicy"
    And I should see "User One"
    And I should see "OwnPageOptional1"
  ...
  Scenario: Administrators can see accepted, declined and pending acceptances of optional policies and also change them on behalf of other users
    ...
    # Accept iomadpolicy on User One's behalf.
