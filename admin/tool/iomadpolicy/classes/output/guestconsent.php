...
class guestconsent implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data = (object) [];
        $data->pluginbaseurl = (new moodle_url('/admin/tool/iomadpolicy'))->out(true);
        if (strpos(qualified_me(), '/tool/iomadpolicy/view.php') === false) {
            // Current page is not a iomadpolicy doc, so returnurl parameter will be it.
            $data->returnurl = qualified_me();
        } else {
            // If current page is also a iomadpolicy doc to view, get previous returnurl parameter to avoid error.
        ...
        $policies = api::list_current_versions(iomadpolicy_version::AUDIENCE_GUESTS);
