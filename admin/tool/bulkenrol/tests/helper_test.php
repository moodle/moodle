...
class helper_test extends \advanced_testcase {
    ...
    public function test_generate_shortname() {
        ...
        $this->assertSame($data->fullname, tool_bulkenrol_helper::generate_shortname($data, '%f'));
        $this->assertSame($data->idnumber, tool_bulkenrol_helper::generate_shortname($data, '%i'));
        $this->assertSame('Ah Bh Ch', tool_bulkenrol_helper::generate_shortname($data, '%~8f'));
        $this->assertSame('AH BH CH', tool_bulkenrol_helper::generate_shortname($data, '%+8f'));
        $this->assertSame('id123', tool_bulkenrol_helper::generate_shortname($data, '%-i'));
        $this->assertSame('[Ah bh Ch] = ID123', tool_bulkenrol_helper::generate_shortname($data, '[%8f] = %i'));
        $this->assertSame('0', tool_bulkenrol_helper::generate_shortname($data, '0'));
        $this->assertSame('%unknown', tool_bulkenrol_helper::generate_shortname($data, '%unknown'));

        $this->assertNull(tool_bulkenrol_helper::generate_shortname($data, ''));
        $this->assertNull(tool_bulkenrol_helper::generate_shortname(array(), '%f'));
    }

    public function test_get_course_formats() {
        $result = tool_bulkenrol_helper::get_course_formats();
        $this->assertSame(array_keys(\core_component::get_plugin_list('format')), $result);
        // Should be similar as first result, as cached.
        $this->assertSame($result, tool_bulkenrol_helper::get_course_formats());
    }
    ...
    public function test_get_enrolment_data() {
        ...
        $this->assertSame(tool_bulkenrol_helper::get_enrolment_data($data), $expected);
    }

    public function test_get_enrolment_plugins() {
        $this->resetAfterTest(true);
        $actual = tool_bulkenrol_helper::get_enrolment_plugins();
        $this->assertSame(array_keys(enrol_get_plugins(false)), array_keys($actual));
        // This should be identical as cached.
        $secondactual = tool_bulkenrol_helper::get_enrolment_plugins();
    ...
    public function test_get_restore_content_dir() {
        ...
        $dir = tool_bulkenrol_helper::get_restore_content_dir($c1backupfile, null);
        ...
        $dir2 = tool_bulkenrol_helper::get_restore_content_dir($c1backupfile, null);
        $this->assertEquals($dir, $dir2);

        // Get the second course.
        $dir = tool_bulkenrol_helper::get_restore_content_dir($c2backupfile, null);
        $bcinfo = \backup_general_helper::get_backup_information($dir);
        ...
        $dir = tool_bulkenrol_helper::get_restore_content_dir(null, $c1->shortname);
        $bcinfo = \backup_general_helper::get_backup_information($dir);
        $this->assertEquals($bcinfo->original_course_id, $c1->id);
        $this->assertEquals($bcinfo->original_course_fullname, $c1->fullname);

        // Do it again, it should be the same directory.
        $dir2 = tool_bulkenrol_helper::get_restore_content_dir(null, $c1->shortname);
        $this->assertEquals($dir, $dir2);

        // Get the second course.
        $dir = tool_bulkenrol_helper::get_restore_content_dir(null, $c2->shortname);
        $bcinfo = \backup_general_helper::get_backup_information($dir);
        $this->assertEquals($bcinfo->original_course_id, $c2->id);
        $this->assertEquals($bcinfo->original_course_fullname, $c2->fullname);
        ...
        $errors = array();
        $dir = tool_bulkenrol_helper::get_restore_content_dir(null, 'DoesNotExist', $errors);
        ...
        $this->assertFalse($dir);
        $this->assertArrayHasKey('coursetorestorefromdoesnotexist', $errors);
        $dir2 = tool_bulkenrol_helper::get_restore_content_dir(null, 'DoesNotExist', $errors);
        $this->assertEquals($dir, $dir2);
    ...
    public function test_get_role_ids() {
        ...
        $actual = tool_bulkenrol_helper::get_role_ids();
        $this->assertSame($actual, $expected);

        // Check cache.
        $this->assertSame($actual, tool_bulkenrol_helper::get_role_ids());
    }
    ...
    public function test_get_role_names() {
        ...
        $roleids = tool_bulkenrol_helper::get_role_ids(true);
        ...
        $errors = array();
        $actual = tool_bulkenrol_helper::get_role_names($data, $errors);
        $this->assertSame($actual, $expected);
    ...
    public function test_get_custom_course_field_data() {
        ...
        $fields = tool_bulkenrol_helper::get_custom_course_fields();
        ...
        $this->assertEquals($expected, tool_bulkenrol_helper::get_custom_course_field_data($data, [], $context));
        ...
        $fields = tool_bulkenrol_helper::get_custom_course_field_data($data, [], $context);
    ...
    public function test_increment_idnumber() {
        ...
        $this->assertEquals('C3', tool_bulkenrol_helper::increment_idnumber('C1'));
        $this->assertEquals('Yo_2', tool_bulkenrol_helper::increment_idnumber('Yo'));
        $this->assertEquals('DoesNotExist', tool_bulkenrol_helper::increment_idnumber('DoesNotExist'));
    }
    ...
    public function test_increment_shortname() {
        ...
        $this->assertEquals('C3', tool_bulkenrol_helper::increment_shortname('C1'));
        $this->assertEquals('Yo_2', tool_bulkenrol_helper::increment_shortname('Yo'));
        $this->assertEquals('DoesNotExist_2', tool_bulkenrol_helper::increment_shortname('DoesNotExist'));
    }
    ...
    public function test_resolve_category() {
        ...
        $this->assertEquals($c1->id, tool_bulkenrol_helper::resolve_category($data));
        unset($data['category']);
        $this->assertEquals($c3->id, tool_bulkenrol_helper::resolve_category($data));
        unset($data['category_idnumber']);
        $this->assertEquals($c2->id, tool_bulkenrol_helper::resolve_category($data));

        // Adding unexisting data.
        $errors = array();
        $data['category_idnumber'] = 1234;
        $this->assertEquals($c2->id, tool_bulkenrol_helper::resolve_category($data, $errors));
        ...
        $this->assertEmpty(tool_bulkenrol_helper::resolve_category($data, $errors));
    ...
    public function test_resolve_category_by_idnumber() {
        ...
        $this->assertEquals($c1->id, tool_bulkenrol_helper::resolve_category_by_idnumber('C1'));
        $this->assertEquals($c1->id, tool_bulkenrol_helper::resolve_category_by_idnumber('C1'));
        $this->assertEquals($c2->id, tool_bulkenrol_helper::resolve_category_by_idnumber('C2'));
        $this->assertEquals($c2->id, tool_bulkenrol_helper::resolve_category_by_idnumber('C2'));
        $this->assertEmpty(tool_bulkenrol_helper::resolve_category_by_idnumber('DoesNotExist'));
        $this->assertEmpty(tool_bulkenrol_helper::resolve_category_by_idnumber('DoesNotExist'));
    }
    ...
    public function test_resolve_category_by_path() {
        ...
        $path = array('Cat 1');
        $this->assertEquals($cat1->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat1->id, tool_bulkenrol_helper::resolve_category_by_path($path));

        $path = array('Cat 1', 'Cat 1.1', 'Cat 1.1.2');
        $this->assertEquals($cat1_1_2->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat1_1_2->id, tool_bulkenrol_helper::resolve_category_by_path($path));

        $path = array('Cat 1', 'Cat 1.2');
        $this->assertEquals($cat1_2->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat1_2->id, tool_bulkenrol_helper::resolve_category_by_path($path));

        $path = array('Cat 2');
        $this->assertEquals($cat2->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat2->id, tool_bulkenrol_helper::resolve_category_by_path($path));

        // Hidden category.
        $path = array('Cat 2', 'Cat 2.1');
        $this->assertEquals($cat2_1->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat2_1->id, tool_bulkenrol_helper::resolve_category_by_path($path));

        // Hidden parent.
        $path = array('Cat 2', 'Cat 2.1', 'Cat 2.1.2');
        $this->assertEquals($cat2_1_2->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat2_1_2->id, tool_bulkenrol_helper::resolve_category_by_path($path));

        // Does not exist.
        $path = array('No cat 3', 'Cat 1.2');
        $this->assertEmpty(tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEmpty(tool_bulkenrol_helper::resolve_category_by_path($path));
        ...
        $this->assertEquals($cat3_fakedouble->id, tool_bulkenrol_helper::resolve_category_by_path($path));
        $this->assertEquals($cat3_fakedouble->id, tool_bulkenrol_helper::resolve_category_by_path($path));
    }
