...
namespace tool_bulkenrol;

use tool_bulkenrol_processor;
use tool_bulkenrol_course;
...
class course_test extends \advanced_testcase {
    ...
    public function test_proceed_without_prepare() {
        ...
        $mode = tool_bulkenrol_processor::MODE_CREATE_NEW;
        $updatemode = tool_bulkenrol_processor::UPDATE_NOTHING;
        $data = array();
        $co = new tool_bulkenrol_course($mode, $updatemode, $data);
    ...
    public function test_invalid_shortname_too_long() {
        ...
        $upload = new tool_bulkenrol_course($mode, $updatemode, [
            'category' => 1,
            'fullname' => 'New course',
    ...
    public function test_create() {
        ...
        $mode = tool_bulkenrol_processor::MODE_CREATE_ALL;
        $data = array('shortname' => 'c1', 'fullname' => 'C1FN', 'summary' => 'C1', 'category' => 1);
        $co = new tool_bulkenrol_course($mode, $updatemode, $data);
        $this->assertTrue($co->prepare());
        $co->proceed();
    ...
    public function test_delete() {
        ...
        $mode = tool_bulkenrol_processor::MODE_CREATE_OR_UPDATE;
        $updatemode = tool_bulkenrol_processor::UPDATE_ALL_WITH_DATA_ONLY;

        // Try delete when option not available.
        $importoptions = array('candelete' => false);
        $data = array('shortname' => $c1->shortname, 'delete' => 1);
        $co = new tool_bulkenrol_course($mode, $updatemode, $data, array(), $importoptions);
        $this->assertFalse($co->prepare());
    ...
    public function test_update() {
        ...
        $mode = tool_bulkenrol_processor::MODE_UPDATE_ONLY;
        $updatemode = tool_bulkenrol_processor::UPDATE_NOTHING;
        ...
        $mode = tool_bulkenrol_processor::MODE_UPDATE_ONLY;
        $updatemode = tool_bulkenrol_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS;
        $data = array('shortname' => 'c1', 'fullname' => 'Another fullname');
        $defaults = array('fullname' => 'Not this one', 'summary' => 'Awesome summary');
        $co = new tool_bulkenrol_course($mode, $updatemode, $data, $defaults);
        $this->assertTrue($co->prepare());
        $co->proceed();
        ...
        $mode = tool_bulkenrol_processor::MODE_UPDATE_ONLY;
        $updatemode = tool_bulkenrol_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS;
    ...
    public function test_default_data_saved() {
        ...
        $this->assertFalse($DB->record_exists('course', array('shortname' => 'c1')));
        $co = new tool_bulkenrol_course($mode, $updatemode, $data, $defaultdata);
        $this->assertTrue($co->prepare());
        $co->proceed();
        $this->assertTrue($DB->record_exists('course', array('shortname' => 'c1')));
        $course = $DB->get_record('course', array('shortname' => 'c1'));
    ...
    public function test_rename() {
        ...
        $co = new tool_bulkenrol_course($mode, $updatemode, $data, $defaultdata, $importoptions);
        $this->assertTrue($co->prepare());
        $co->proceed();
    ...
    public function test_reset() {
        ...
        $importoptions = array('canreset' => true);
        $co = new tool_bulkenrol_course($mode, $updatemode, $data, array('reset' => 1), $importoptions);
        $this->assertTrue($co->prepare());
        $co->proceed();
        $this->assertTrue($DB->record_exists('groups', array('id' => $g1->id)));
        $this->assertTrue($DB->record_exists('groups_members', array('groupid' => $g1->id, 'userid' => $u1->id)));
    ...
    public function test_enrolment_error_create_course(string $expectederror, array $uploaddata): void {
        ...
        $upload = new tool_bulkenrol_course($mode, $updatemode, array_merge($uploaddata, [
            'category' => $category->id,
            'fullname' => 'My course',
        ]));
        ...
        $this->assertEquals(get_string($expectederror, 'tool_bulkenrol', 'Manual enrolments'),
    ...
    public function test_enrolment_error_update_course(string $expectederror, array $uploaddata): void {
        ...
        $upload = new tool_bulkenrol_course($mode, $updatemode, $uploaddata);
    ...
    public function test_custom_fields_data() {
        ...
        $uploader = new tool_bulkenrol_course($mode, $updatemode, $dataupload);
    ...
    public function test_custom_fields_data_required() {
        ...
        $uploader = new tool_bulkenrol_course($mode, $updatemode, $dataupload, $defaults);
        $this->assertTrue($uploader->prepare());
        $uploader->proceed();
    ...
    public function test_idnumber_problems() {
        ...
        $data = array('shortname' => $c1->shortname, 'idnumber' => $c1->idnumber, 'summary' => 'S', 'fullname' => 'F',
            'category' => 1);
        $co = new tool_bulkenrol_course($mode, $updatemode, $data, array(), array());
        $this->assertTrue($co->prepare());
     ...
     *
     * @covers \tool_bulkenrol_course::prepare
     */
     ...
     *
     * @covers \tool_bulkenrol_course::proceed
     */
