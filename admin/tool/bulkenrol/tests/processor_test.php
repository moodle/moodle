...
class processor_test extends \advanced_testcase {
    ...
    public function test_basic() {
        ...
        $content = implode("\n", $content);
        $iid = \csv_import_reader::get_new_iid('bulkenrol');
        $cir = new \csv_import_reader($iid, 'bulkenrol');
        $cir->load_csv_content($content, 'utf-8', 'comma');
        $cir->init();

        $options = array('mode' => tool_bulkenrol_processor::MODE_CREATE_ALL);
        $defaults = array('category' => '1');

        $p = new tool_bulkenrol_processor($cir, $options, $defaults);
    ...
    public function test_restore_template_course() {
        ...
        $options = array('mode' => tool_bulkenrol_processor::MODE_CREATE_NEW, 'templatecourse' => $c1->shortname);
        $defaults = array('category' => '1');
    ...
    public function test_shortname_template() {
        ...
        $options = array('mode' => tool_bulkenrol_processor::MODE_CREATE_NEW, 'shortnametemplate' => '%i: %f');
        $defaults = array('category' => '1');
    ...
    public function test_empty_csv() {
        ...
        $p = new tool_bulkenrol_processor($cir, $options, array());
    }
