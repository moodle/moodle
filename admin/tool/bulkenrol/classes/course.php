...
class tool_bulkenrol_course {
    ...
    static protected $validfields = array('fullname', 'shortname', 'idnumber', 'category');
...
//        }
    ...
    protected function get_valid_fields() {
        return array_merge(self::$validfields, \tool_bulkenrol_helper::get_custom_course_field_names());
    }
    ...
    protected function get_restore_content_dir() {
        ...
        $errors = array();
        $dir = tool_bulkenrol_helper::get_restore_content_dir($backupfile, $shortname, $errors);
        if (!empty($errors)) {
...
//        $updatemode = $this->updatemode;
        ...
        $errors = array();
        $catid = tool_bulkenrol_helper::resolve_category($this->rawdata, $errors);
        if (empty($errors)) {
...
//        if (!$exists || $mode === tool_bulkenrol_processor::MODE_CREATE_ALL) {
...
//        switch ($mode) {
//            case tool_bulkenrol_processor::MODE_CREATE_NEW:
...
//                }
//                // No break!
//            case tool_bulkenrol_processor::MODE_CREATE_OR_UPDATE:
//                if ($exists) {
...
//        // Get final data.
//        if ($exists) {
//            $missingonly = ($updatemode === tool_bulkenrol_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS);
...
//        // Add role renaming.
//        $errors = array();
//        $rolenames = tool_bulkenrol_helper::get_role_names($this->rawdata, $errors);
//        if (!empty($errors)) {
//            foreach ($errors as $key => $message) {
//                $this->error($key, $message);
//            }
...
//        }
//        $customfielddata = tool_bulkenrol_helper::get_custom_course_field_data($this->rawdata, $this->defaults, $context,
//            $errors);
//        if (!empty($errors)) {
//            foreach ($errors as $key => $message) {
//                $this->error($key, $message);
...
//                $numsections = tool_bulkenrol_helper::get_coursesection_count($this->options['templatecourse']);
//                if ($numsections != 0) {
...
//        }
        ...
        $this->enrolmentdata = tool_bulkenrol_helper::get_enrolment_data($this->rawdata);
...
//        }
    ...
    protected function validate_enrolment_data(int $courseid, array $enrolmentdata): array {
        ...
        $enrolmentplugins = tool_bulkenrol_helper::get_enrolment_plugins();
        ...
        foreach ($enrolmentdata as $enrolmethod => $method) {
            ...
            if ($todelete) {
                ...
                if (isset($method['role'])) {
                    ...
                    $roleids = tool_bulkenrol_helper::get_role_ids();
...
//
//        // Add roles.
//        $roles = tool_bulkenrol_helper::get_role_ids();
