...
class tool_bulkenrol_helper {
    ...
    public static function generate_shortname($data, $templateshortname) {
        ...
        $callback = partial(array('tool_bulkenrol_helper', 'generate_shortname_callback'), $fullname, $idnumber);
    ...
    public static function get_enrolment_plugins() {
        $cache = cache::make('tool_bulkenrol', 'helper');
    ...
    public static function get_restore_content_dir($backupfile = null, $shortname = null, &$errors = array()) {
        ...
        if (!empty($backupfile)) {
            ...
            if (empty($backupfile) || !is_readable($backupfile)) {
                $errors['cannotreadbackupfile'] = new lang_string('cannotreadbackupfile', 'tool_bulkenrol');
                return false;
            }
        ...
        if (!$usecache || (($backupid = $cache->get($cachekey)) === false || !is_dir(get_backup_temp_directory($backupid)))) {
            ...
            if (!empty($backupfile)) {
                ...
                if (!$result) {
                    $errors['invalidbackupfile'] = new lang_string('invalidbackupfile', 'tool_bulkenrol');
                }
            } else if (!empty($shortname) || is_numeric($shortname)) {
                ...
                if (!empty($courseid)) {
                    ...
                    $errors['coursetorestorefromdoesnotexist'] =
                        new lang_string('coursetorestorefromdoesnotexist', 'tool_bulkenrol');
                }
            }
    ...
    public static function get_role_names($data, &$errors = array()) {
        ...
        if (!empty($invalidroles)) {
            $errors['invalidroles'] = new lang_string('invalidroles', 'tool_bulkenrol', implode(', ', $invalidroles));
        }
        ...
        foreach ($data as $name => $originalvalue) {
            if (preg_match('/^customfield_(?<name>.*)?$/', $name, $matches)
                ...
                if (count($validationerrors) > 0) {
                    $errors['customfieldinvalid'] = new lang_string('customfieldinvalid', 'tool_bulkenrol',
     ...
     */
    public static function resolve_category($data, $user_resolveby, &$errors = array()) {
        $catid = null;
        ...
        if (!empty($data['category'])) {
            ...
            if (!empty($category) && !empty($category->id)) {
                ...
                $errors['couldnotresolvecatgorybyid'] =
                    new lang_string('couldnotresolvecatgorybyid', 'tool_bulkenrol');
            }
        }
        ...
        if (empty($catid) && !empty($data['category_idnumber'])) {
            ...
            if (empty($catid)) {
                $errors['couldnotresolvecatgorybyidnumber'] =
                    new lang_string('couldnotresolvecatgorybyidnumber', 'tool_bulkenrol');
            }
        }
        if (empty($catid) && !empty($data['category_path'])) {
            ...
            if (empty($catid)) {
                $errors['couldnotresolvecatgorybypath'] =
                    new lang_string('couldnotresolvecatgorybypath', 'tool_bulkenrol');
            }
        }
