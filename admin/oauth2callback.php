...
// The state parameter we've given (used in moodle as the session key).
$sesskey = required_param('state', PARAM_RAW);
...
// The session parameter was not set, this means that we didn't pass
// in the $client->get_login_url() just before calling this page.
if (!isset($SESSION->oauth_post_login_redirect)) {
    $SESSION->loginerrormsg = get_string('invalidaccess', 'error');
    redirect(new moodle_url(get_login_url()));
}

$redirecturl = new moodle_url($SESSION->oauth_post_login_redirect);
