...
class block_mediasearch_entry_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('editor', 'decription', get_string('entry_description', 'block_mediasearch'));
        ...
        $mform->setType('link', PARAM_HTML);
        $mform->addRule('link', get_string('required'), 'required');

        // Add the course select.
        $courses = $DB->get_records('courses', array(), null, 'id,fullname');
        $mform->addElement('select', 'courseid', get_string('entry_course'), $courses);

        $mform->addElement('textarray', 'keywords', get_string('entry_keywords', 'block_mediasearch'));
