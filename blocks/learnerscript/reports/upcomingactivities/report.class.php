...
class report_upcomingactivities extends reportbase implements report {
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->columns = array('activityfield' => ['activityfield'], 
                               'upcomingactivities' => array('activityname','course','startdate'));
        $this->basicparams = array(['name' => 'users']);
        $this->filters = array('courses','activities');
        $this->orderable = array('activityname','course','startdate');
        $this->defaultcolumn = '';
    }
    function init() {
        global $DB;
     ...
     function count() {
        ...
        foreach ($aliases as $alias) {
            if($alias == 'qbassign'){
                ...
                JOIN {".$alias."} AS $alias ON $alias.id = main.instance AND m.name = '$alias' AND $alias.allowsubmissionsfromdate > $timestamp
            ...
            if($alias == 'scorm'){
                ...
                JOIN {user} u ON u.id = ue.userid  
                JOIN {".$alias."} AS $alias ON $alias.id = main.instance AND m.name = '$alias' AND $alias.timeopen > ".$timestamp." 
                WHERE $alias.id NOT IN (SELECT scormid FROM {scorm_scoes_track} WHERE userid = ".$userid.") AND m.visible = 1 AND m.name IN ($activitynames) AND main.visible = 1 AND $alias.id NOT IN(SELECT cm.module FROM {course_modules_completion} cmc JOIN {course_modules} cm ON cm.id = cmc.coursemoduleid WHERE cmc.userid = ".$userid." AND cmc.completionstate <> 0)".$filters;
            }
        }
