...
class plugin_quizzparticipationcolumns extends pluginbase{
  public function init(){
    ...
    $this->type = 'undefined';
    $this->form = true;
    $this->reporttypes = array('quizzparticipation');
  }
      ...
      case 'gradepass':
        ...
        case 'finalgrade':
            $finalgrade = $DB->get_field_sql("SELECT gg.finalgrade
                           ...
                           WHERE 1 = 1 AND gi.itemmodule = 'quiz' AND gg.userid = :userid AND gi.iteminstance = :quizid",['userid'=>$row->userid,'quizid'=>$row->id]);
              $row->{$data->column} = $finalgrade ? ROUND($finalgrade, 2) : '--';
              
            break;
        case 'submitteddate':
            $submitteddate =  $DB->get_field_sql("SELECT qa.timefinish FROM {quiz_attempts} qa WHERE qa.quiz = :quizid AND qa.userid=:userid", ['userid' => $row->userid,'quizid' => $row->id]);
