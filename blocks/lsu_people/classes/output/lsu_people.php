...
class lsu_people implements renderable {
    ...
    public static function get_icourseobj($courseid) {
        global $DB;

        $parms = ['courseid' => $courseid];

        $sql = "SELECT GROUP_CONCAT(sec.id) AS sections,
                cou.course_subject_abbreviation,
                cou.course_number,
                sec.section_number
            FROM {enrol_wds_courses} cou
            INNER JOIN {enrol_wds_sections} sec
                ON sec.course_definition_id = cou.course_definition_id
            INNER JOIN {course} c ON c.id = sec.moodle_status
            WHERE c.id = :courseid
            GROUP BY cou.id";

            $icourse = $DB->get_record_sql($sql, $parms);
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // Set the base url.
        $baseurl = new \moodle_url('/blocks/lsu_people/view.php',
        ...
        if ($this->download) {
            ...
            // Get an insterstitial course object.
            $icourse = self::get_icourseobj($this->courseid);

            // Get the coursename for this course.
            $icoursename = $icourse->course_subject_abbreviation
                . ' ' .
                $icourse->course_number;

            // Fix it.
            $coursename = rawurlencode(str_replace(' ', '_', $icoursename));

            // If we have a group, fix that too.
            if (isset($group->name)) {
                $groupname = rawurlencode(str_replace(' ', '_', $group->name));
            }

            $filename = isset($group->name) ? $groupname : $coursename;

            // Set the table to be the downloader.
            $table->is_downloading($this->download, $filename . '_roster', 'LSU Course Roster');
        }
