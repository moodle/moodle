...
class pChart {
    ...
    var $Palette = [
        ...
        "22" => ["R" => 61, "G" => 89, "B" => 171],
        "23" => ["R" => 61, "G" => 145, "B" => 64],
        "24" => ["R" => 128, "G" => 138, "B" => 135],
        "25" => ["R" => 139, "G" => 62, "B" => 47],
        "26" => ["R" => 100, "G" => 149, "B" => 237],
    ];
    ...
    function drawScale(
        ...
        if ($this->VMin == null && $this->VMax == null) {
            ...
            } else if ($ScaleMode == SCALE_ADDALL || $ScaleMode == SCALE_ADDALLSTART0) /* Experimental */ {
    ...
    function drawTitle($XPos, $YPos, $Value, $R, $G, $B, $XPos2 = -1, $YPos2 = -1, $Shadow = false) {
        ...
        if ($Shadow) {
            ...
            imagettftext(
                ...
                $this->FontSize,
                0,
                $XPos + $this->ShadowXDistance,
                $YPos + $this->ShadowYDistance,
                $C_ShadowColor,
                $this->FontName,
                $Value
            );
            ...
            foreach ($Data as $Key => $Values) {
                ...
                if ($this->BuildMap) {
                    $this->addToImageMap(
                        ...
                        $XPos + 1 + $Hsize,
                        $YPos + $Hsize + 1,
                        $DataDescription["Description"][$ColName],
                        $Data[$Key][$ColName] . $DataDescription["Unit"]["Y"],
                        "Plot"
                    );
                }
        ...
        foreach ($Series as $Key => $Serie) {
            ...
            foreach ($Data as $Key => $Values) {
                if (isset($Data[$Key][$Serie]) && is_numeric($Data[$Key][$Serie])) {
                    ...
                    $C_TextColor = $this->AllocateColor(
                        $this->Picture,
                        $this->Palette[$ColorID]["R"],
                        $this->Palette[$ColorID]["G"],
                        $this->Palette[$ColorID]["B"]
                    );
            ...
            if (isset($Data[$Key][$YSerieName]) && isset($Data[$Key][$XSerieName])) {
                ...
                if ($XLast != -1 && $YLast != -1) {
                    $this->drawLine(
                        ...
                        $X,
                        $Y,
                        $this->Palette[$PaletteID]["R"],
                        $this->Palette[$PaletteID]["G"],
                        $this->Palette[$PaletteID]["B"],
                        true
                    );
                }
    ...
    function drawFilledCubicCurve($Data, $DataDescription, $Accuracy = .1, $Alpha = 100, $AroundZero = false) {
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName) {
            ...
            for ($i = 2; $i <= $Index - 1; $i++) {
                ...
                $Yt[$i] = ($Sig - 1) / $p;
                $U[$i] =
                    ($YIn[$i + 1] - $YIn[$i]) / ($XIn[$i + 1] - $XIn[$i]) - ($YIn[$i] - $YIn[$i - 1]) / ($XIn[$i] - $XIn[$i - 1]);
                $U[$i] = (6 * $U[$i] / ($XIn[$i + 1] - $XIn[$i - 1]) - $Sig * $U[$i - 1]) / $p;
            }
    ...
    function drawOverlayBarGraph($Data, $DataDescription, $Alpha = 50) {
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName) {
            ...
            $C_Graph = $this->AllocateColor(
                $this->Layers[$GraphID],
                $this->Palette[$GraphID]["R"],
                $this->Palette[$GraphID]["G"],
                $this->Palette[$GraphID]["B"]
            );
    ...
    function drawLimitsGraph($Data, $DataDescription, $R = 0, $G = 0, $B = 0) {
        ...
        foreach ($Data as $Key => $Values) {
            ...
            $this->drawLine(
                ...
                $X2,
                $Y1,
                $this->Palette[$MaxID]["R"],
                $this->Palette[$MaxID]["G"],
                $this->Palette[$MaxID]["B"],
                false
            );
            $this->drawLine(
                $X1,
                ...
                $X2,
                $Y2,
                $this->Palette[$MinID]["R"],
                $this->Palette[$MinID]["G"],
                $this->Palette[$MinID]["B"],
                false
            );
    ...
    function drawBasicPieGraph(
        ...
        foreach ($iValues as $Key => $Value) {
            ...
            if (!($DrawLabels == PIE_NOLABEL)) {
                ...
                } else if ($DrawLabels == PIE_LABELS) {
                    $Caption = $iLabels[$Key];
                } else if ($DrawLabels == PIE_PERCENTAGE_LABEL) {
                    $Caption =
    ...
    function drawFlatPieGraphWithShadow(
        $Data,
        $DataDescription,
        $XPos,
        $YPos,
    ...
    function drawFlatPieGraph(
        ...
        foreach ($iValues as $Key => $Value) {
            ...
            for ($iAngle = $Angle; $iAngle <= $Angle + $Value * $SpliceRatio; $iAngle = $iAngle + .5) {
                ...
                if ($iAngle == $Angle || $iAngle == $Angle + $Value * $SpliceRatio ||
                    $iAngle + .5 > $Angle + $Value * $SpliceRatio) {
    ...
    function drawPieGraph(
        ...
        foreach ($iValues as $Key => $Value) {
            $XCenterPos =
                cos(($Angle - $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) * $SpliceDistance + $XPos;
            $YCenterPos =
                sin(($Angle - $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) * $SpliceDistance + $YPos;
            $XCenterPos2 =
                cos(($Angle + $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) * $SpliceDistance + $XPos;
            $YCenterPos2 =
                sin(($Angle + $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) * $SpliceDistance + $YPos;
        ...
        for ($i = $SpliceHeight - 1; $i >= 1; $i--) {
            foreach ($iValues as $Key => $Value) {
                ...
                $this->drawAntialiasPixel(
                    $Plots[0],
                    $Plots[1],
                    $this->Palette[$Key]["R"] + $ColorFactor,
                    $this->Palette[$Key]["G"] + $ColorFactor,
                    $this->Palette[$Key]["B"] + $ColorFactor
                );
                $this->drawAntialiasPixel(
                    $Plots[2],
                    $Plots[3],
    ...
    function drawFilledRectangle($X1, $Y1, $X2, $Y2, $R, $G, $B, $DrawBorder = true, $Alpha = 100, $NoFallBack = false) {
        ...
        if ($Alpha == 100) {
            ...
            if ($this->ShadowActive && !$NoFallBack) {
                $this->drawFilledRectangle(
                    $X1 + $this->ShadowXDistance,
                    $Y1 + $this->ShadowYDistance,
                    $X2 + $this->ShadowXDistance,
                    $Y2 + $this->ShadowYDistance,
                    $this->ShadowRColor,
                    $this->ShadowGColor,
                    $this->ShadowBColor,
                    false,
                ...
                if ($this->ShadowBlur != 0) {
                    ...
                    for ($i = 1; $i <= $this->ShadowBlur; $i++) {
                        $this->drawFilledRectangle(
                            $X1 + $this->ShadowXDistance - $i / 2,
                            $Y1 + $this->ShadowYDistance - $i / 2,
                            $X2 + $this->ShadowXDistance - $i / 2,
                            $Y2 + $this->ShadowYDistance - $i / 2,
                            $this->ShadowRColor,
                            $this->ShadowGColor,
                            $this->ShadowBColor,
                            false,
                            $this->ShadowAlpha - $AlphaDecay * $i,
                            true
                        );
                    }
                    for ($i = 1; $i <= $this->ShadowBlur; $i++) {
    ...
    function drawLine($X1, $Y1, $X2, $Y2, $R, $G, $B, $GraphFunction = false) {
        ...
        for ($i = 0; $i <= $Distance; $i++) {
            ...
            if (($X >= $this->GArea_X1 && $X <= $this->GArea_X2 && $Y >= $this->GArea_Y1 && $Y <= $this->GArea_Y2) ||
                !$GraphFunction) {
                if ($this->LineWidth == 1) {
                    $this->drawAntialiasPixel($X, $Y, $R, $G, $B);
                } else {
    ...
    function drawAntialiasPixel($X, $Y, $R, $G, $B, $Alpha = 100, $NoFallBack = false) {
        ...
        if ($this->ShadowActive && !$NoFallBack) {
            $this->drawAntialiasPixel(
                $X + $this->ShadowXDistance,
                $Y + $this->ShadowYDistance,
                $this->ShadowRColor,
                $this->ShadowGColor,
                $this->ShadowBColor,
                $this->ShadowAlpha,
            ...
            if ($this->ShadowBlur != 0) {
                ...
                for ($i = 1; $i <= $this->ShadowBlur; $i++) {
                    $this->drawAntialiasPixel(
                        $X + $this->ShadowXDistance - $i / 2,
                        $Y + $this->ShadowYDistance - $i / 2,
                        $this->ShadowRColor,
                        $this->ShadowGColor,
                        $this->ShadowBColor,
                        $this->ShadowAlpha - $AlphaDecay * $i,
    ...
    function addToImageMap($X1, $Y1, $X2, $Y2, $SerieName, $Value, $CallerFunction) {
        if ($this->MapFunction == null || $this->MapFunction == $CallerFunction) {
            $this->ImageMap[] =
                round($X1) . "," . round($Y1) . "," . round($X2) . "," . round($Y2) . "," . $SerieName . "," . $Value;
