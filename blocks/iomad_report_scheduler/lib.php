...
abstract class company_moodleform extends moodleform {
    ...
    public function add_colour_picker($name, $previewconfig) {
        ...
        // ...$cptemplate is adapted from the 'default' template in formslib.php's MoodleQuickForm_Renderer function
        // in MoodleQuickForm_Renderer class it is adds a {colourpicker} and {preview} tag that is replaced with the
        // $colourpicker and $preview variables below before being passed to the renderer
        // the {advancedimg} {help} bits have been taken out as the rendered doesn't appear to use them in this case.
        $cptemplate = "\n\t\t".'<div class="fitem {advanced}<!-- BEGIN required --> required<!-- END
                        required -->"><div class="fitemtitle"><label>{label}<!--
                        BEGIN required -->{req}<!-- END required --></label></div><div class="felement {type}<!-- BEGIN
                        error --> error<!-- END error -->">{colourpicker}<!-- BEGIN
                        error --><span class="error">{error}</span><br /><!-- END
                        error -->{element}{preview}</div></div>';
        ...
        $colourpicker .= html_writer::tag('div', $OUTPUT->pix_icon('i/loading', get_string('loading', 'admin'), 'moodle',
        ...
        if (!empty($previewconfig)) {
            $preview .= html_writer::empty_tag('input', array('type' => 'button', 'id' => $id.'_preview',
