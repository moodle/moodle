...
class message_course extends \block_quickmail\persistents\persistent {
    ...
    const TABLE = 'block_quickmail_msg_course';
    ...
    // Getters.
    /**
     * Checks if the course still exists as some courses may get
     * "deleted" before a queue'd message is sent.
     *
     * @param  int        courseid
     * @return bool
     */
    public function course_visible($courseid = 0) {
        global $DB;
        // First check the user exists.

        $coursetestrecord = array_values($DB->get_records_sql(
            'SELECT visible
                FROM {course}
                WHERE id = ?',
            array($courseid)
        ...
        return (bool) $coursetestrecord[0];
    }
    ...
    public function remove_recipient_from_message($messageid = 0, $userid = 0) {
        ...
        $DB->delete_records('block_quickmail_msg_course', ['message_id' => $messageid, 'course_id' => $userid]);
    }
    ...
    public static function clear_all_for_message(message $message) {
        ...
        $DB->delete_records('block_quickmail_msg_course', ['message_id' => $message->get('id')]);
    }
     ...
     * @return void
     */
    public static function mark_as_sent(message $message, $course, $moodlemessageid = 0) {
