...
class message extends \block_quickmail\persistents\persistent {
    ...
    public function get_message_recipients_all()
    {
        ...
        foreach ($recipientuserids as $useless) {
            ...
            $thisrecipientlist[] = new message_recipient(0, $temp);
        }

        return $thisrecipientlist;
    }
    ...
    public function populate_recip_course_msg()
    {
        ...
        // First check if there are recipients.
        $msgexists = $DB->get_records('block_quickmail_msg_recips', [
            'message_id' => $msgid
        ]);

        $msgexists = array_values($msgexists);
        if (count($msgexists) > 0) {
            // This function can be called from various places. If it's from CRON then it's called and then 
            // called again via adhoc task to send it. If the difference between the time the msg recipients
