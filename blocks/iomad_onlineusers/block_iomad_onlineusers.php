...
use block_iomad_onlineusers\fetcher;
...
class block_iomad_onlineusers extends block_base {
    ...
    function get_content() {
        ...
        if ($usercount === 0) {
            $usercount = get_string('nouser', 'block_iomad_onlineusers');
        } else if ($usercount === 1) {
            $usercount = get_string('numuser', 'block_iomad_onlineusers', $usercount);
        } else {
            $usercount = get_string('numusers', 'block_iomad_onlineusers', $usercount);
        }
        ...
        if (!has_capability('block/iomad_onlineusers:viewlist', $this->page->context)
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                if (isguestuser($user)) {
                    ...
                    if ($USER->id == $user->id) {
                        if ($CFG->block_iomad_onlineusers_onlinestatushiding) {
            ...
            if ($initialcount - count($users) > 0) {
                ...
                $this->content->text .= html_writer::span(
                    get_string('otherusers', 'block_iomad_onlineusers', $initialcount - count($users))
                );
