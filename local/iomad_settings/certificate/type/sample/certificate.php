...
if ($certificate->printdate > 0) {
    ...
    iif ($certificate->datefmt == 4) {
        $certificatedate = strftime('%B %Y', $certdate);
    }
    iif ($certificate->datefmt == 5) {
        $timeformat = get_string('strftimedate');
        $certificatedate = userdate($certdate, $timeformat);
    }
}

$serialnumber = iomad_settings_create_serial_number($certificate, $certrecord, $course, $certdate);
...
$pdf->SetAlpha(1);
print_seal($certificate->printseal, $orientation, $sealx, $sealy, '18', '25');
...
cert_printtext($x, $y + 92, 'C', 'freesans', '', 20, get_string('sampledate', 'local_iomad_settings').' '. $certificatedate);
...
cert_printtext($x, $custy + 6, 'C', 'freeserif', '', '20',
               iomad_settings_replacement($certificate->customtext2, $course, $serialnumber, $certdate));
cert_printtext($x, $custy + 12, 'C', 'freeserif', '', '20',
               iomad_settings_replacement($certificate->customtext3, $course, $serialnumber, $certdate));
