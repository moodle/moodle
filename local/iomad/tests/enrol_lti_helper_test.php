...
class enrol_lti_helper_testcase extends advanced_testcase {
    ...
    public function test_update_user_profile_image() {
        ...
        $this->assertSame($CFG->wwwroot . '/pluginfile.php/' . $usercontext->id . '/user/icon/iomadboost/f2?rev=' .$this->user1->picture,
    ...
    public function test_set_xpath($parameters, $expected) {
        ...
        // IOMAD: fixtures directory
        $document->load(realpath(__DIR__ . '/fixtures/enrol_lti/input.xml'));
        $xpath = new \DOMXpath($document);
        $function->invokeArgs(null, [$xpath, $parameters]);
        $result = $document->saveXML();
        $expected = file_get_contents(realpath(__DIR__ . '/fixtures/enrol_lti/' . $expected));
    ...
    public function test_create_cartridge() {
        ...
        // IOMAD
        $this->assertContains("<blti:icon>$CFG->wwwroot/theme/image.php/_s/iomadboost/theme/1/favicon</blti:icon>", $cartridge);
