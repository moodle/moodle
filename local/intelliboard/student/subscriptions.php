...
$PAGE->set_url(new moodle_url(
    "/local/intelliboard/student/subscriptions.php",
    [
        'search'  => $search,
        'sesskey' => sesskey()
    ]
...
$PAGE->navbar->add(get_string('mysubscriptions', 'local_intelliboard'));
$PAGE->requires->jquery();
$PAGE->requires->css('/local/intelliboard/assets/css/style.css');
$PAGE->set_pagetype('mysubscriptions');
$params = array(
    'do'=>'learner',
    'mode'=> 1
);
$intelliboard = intelliboard($params);
$table = new intelliboard_user_subscriptions_table('user_subscriptions', $search);
$intellicartenabled = (
        ...
        <div class="intelliboard-search clearfix">
            <form action="<?php echo $PAGE->url; ?>" method="GET">
                ...
                <span class="pull-left">
                    <input class="form-control" name="search" type="text" value="<?php echo format_string($search); ?>" placeholder="<?php echo get_string('type_here', 'local_intelliboard');?>" />
                </span>
