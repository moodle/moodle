...
$linktext = get_string('report_users_search_title', 'local_report_users_search');

// Set the url.
$linkurl = new moodle_url('/local/report_users_search/index.php');

// Build the nav bar.
company_admin_fix_breadcrumb($PAGE, $linktext, $linkurl);

// Print the page header.
$blockpage = new blockpage($PAGE, $OUTPUT, 'report_users_search', 'local', 'report_users_search_title');
$blockpage->setup();
...
$returnurl = $CFG->wwwroot."/local/report_users_search/index.php";


// Carry on with the user listing.

$columns = array("firstname", "lastname", "department", "email", "date", "search");
...
if (iomad::has_capability('block/iomad_company_admin:editallusers', $systemcontext)) {
    ...
    if ((empty($idlist) && !$foundfields) || (!empty($idlist) && $foundfields)) {
        ...
        switch($sort) {
            ...
            break;
            case "date":
                $dbsort .= " order by mss.timestamp $dir ";
            break;
        }
...
if (!empty($userlist)) {
    $totalusers = $DB->get_records_sql("SELECT mss.id as id
                                        FROM {user} u, {department} d, {company_users} cu, {block_mediasearch_searches} mss
                                        WHERE u.deleted <> 1
                                        AND mss.userid = u.id
                                        AND $userlist
                                        AND cu.userid = u.id AND cu.departmentid = d.id
                                        AND cu.companyid = :companyid
                                        $dbsort", array('companyid' => $company->id));
    $users = $DB->get_records_sql("SELECT mss.id as id,
                                          ...
                                          mss.courseid as courseid,
                                          mss.searchstring as searchstring,
                                          mss.timestamp as timestamp
                                   FROM {user} u, {department} d, {company_users} cu, {block_mediasearch_searches} mss
                                   WHERE u.deleted <> 1
                                   AND mss.userid = u.id
                                   AND $userlist
                                   AND cu.userid = u.id AND cu.departmentid = d.id
                                   AND cu.companyid = :companyid
                                   $dbsort", array('companyid' => $company->id), $page * $perpage, $perpage);
} else {
    $users = array();
}
$usercount = count($totalusers);

echo $OUTPUT->heading("$usercount ".get_string('searches', 'local_report_users_search'));
...
if (!$users) {
    $match = array();
    echo $OUTPUT->heading(get_string('nosearchesfound', 'local_report_users_search'));
    ...
    $table->head = array ($fullnamedisplay,
                          ...
                          $department),
                          $OUTPUT->action_link($accessurl, $date),
                          get_string('search'));
    ...
    foreach ($users as $user) {
        ...
        if ($user->timestamp) {
            $strlastaccess =  date('Y-m-d H:i:s', $user->timestamp);
        } else {
