...
require($CFG->dirroot."/local/qubitsuser/classes/qubits_adduser_form.php");
...
$linktext = get_string('createuser', 'local_qubitsuser').' - '.$qubitssite->name;
if ($returnto !== 0) {
...
$currenturl = new moodle_url('/local/qubitsuser/adduser.php', $pageparams);
$formurl = $CFG->wwwroot.'/local/qubitsuser/adduser.php?' .http_build_query($pageparams, '', '&');
$userform = new qubits_adduser_form($formurl, $args);
...
if ($userform->is_cancelled()) {
    ...
    if (!$userid = qubits_user::create($data)) {
        $this->verbose("Error inserting a new user in the database!");
        if (!$this->get('ignore_errors')) {
            die();
        }
    }
    redirect($returnurl, get_string('usercreated', 'local_qubitsuser'), null, \core\output\notification::NOTIFY_SUCCESS);
}
