...
/**
 * Delete OAuth2 client confirmation action.
 *
 * @module     core_admin/oauth2/server/client/actions/client_delete
...
class DeleteClientAction extends baseClientAction {
    /**
    ...
    async executeConfirmAction() {
        ...
        await Fetch.performPost(
            'core_admin',
            `oauth2/server/clients/${clientId}/delete`,
            {}
        );
     ...
     */
    async getTitleText() {
        return await getString('oauth2server_clientdeleteactiontitle', 'admin', this.target.dataset.name);
    }
     ...
     */
    async getBody() {
        return await getString('oauth2server_clientdeleteactiondesc', 'admin');
    }
    ...
    async getConfirmationButtonText() {
        return await getString('delete', 'core');
    }
...
/**
 * Initialize the client delete action.
 */
export const init = () => {
    (new DeleteClientAction()).init();
};
