...
class behat_calendar extends behat_base {
    ...
    /**
     * Visit the calendar export URL of a user, scoped to a given course.
     *
     * This mirrors the subscription URL generated by the export page. It is a step rather than a plain
     * URL visit because the URL is authenticated with a token derived from the user, and because the
     * course it points at cannot always be selected through the interface (for example when the user
     * has since been unenrolled from it).
     *
     * @Given /^I visit the calendar export URL for "(?P<username>[^"]*)" user and "(?P<course>[^"]*)" course$/
     * @param string $username The username the export URL belongs to.
     * @param string $courseshortname The shortname of the course to scope the export to.
     * @return void
     */
    public function i_visit_the_calendar_export_url_for_user_and_course(string $username, string $courseshortname): void {
        global $DB, $CFG;
        ...
        $url = new moodle_url('/calendar/export_execute.php', [
            'userid' => $user->id,
