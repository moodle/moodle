...
/**
 * Get due date info for a user's course in a learning plan.
 *
 * @param int $userid User ID
 * @param int $courseid Course ID
 * @return object|null Object with daysremaining and urgency, or null if not in a plan
 */
function local_coursematrix_get_user_course_dueinfo($userid, $courseid) {
    global $DB;

    // Find if this course is the current course in any of the user's plans.
    $userplan = $DB->get_record('local_coursematrix_user_plans', [
        'userid' => $userid,
        'currentcourseid' => $courseid,
        'status' => 'active',
    ...
    // Get the due days for this course in the plan.
    $plancourse = $DB->get_record('local_coursematrix_plan_courses', [
        'planid' => $userplan->planid,
        'courseid' => $courseid,
    ]);
