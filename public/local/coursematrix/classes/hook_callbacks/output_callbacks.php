...
class output_callbacks {
    ...
    public static function before_top_of_body(\core\hook\output\before_standard_top_of_body_html_generation $hook): void {
        ...
        // Inject course card badges for My Courses page.
...
.coursematrix-badge.completed { background: linear-gradient(135deg, #28a745, #20c997); color: white; }
.coursematrix-badge.overdue { background: linear-gradient(135deg, #dc3545, #c82333); color: white; }
.coursematrix-badge.critical { background: linear-gradient(135deg, #fd7e14, #dc3545); color: white; }
.coursematrix-badge.warning { background: linear-gradient(135deg, #ffc107, #fd7e14); color: #212529; }
.coursematrix-badge.normal { background: linear-gradient(135deg, #17a2b8, #138496); color: white; }
</style>
<script>
(function() {
    ...
    function addBadges() {
        document.querySelectorAll('a[href*="/course/view.php?id="]').forEach(function(link) {
            ...
            if (!m) return;
            var cid = m[1];
            if (processed[cid]) return;
            if (!courseStatuses[cid]) return;
            
            var card = link.closest('.card');
            if (!card) return;
            if (card.querySelector('.coursematrix-badge')) { processed[cid] = true; return; }
            
            var info = courseStatuses[cid];
...
HTML;
    ...
    private static function inject_course_page_banner($hook, $userid, $courseid): void {
        ...
        if ($dueinfo->urgency == 'overdue') {
            ...
            $text = '<strong>OVERDUE:</strong> ' . ($days == 1 ? '1 day' : $days . ' days') . ' overdue!';
            $alertclass = 'alert-danger';
            $icon = 'fa-exclamation-triangle';
        } else if ($dueinfo->urgency == 'critical') {
            $text = '<strong>URGENT:</strong> ' . $dueinfo->daysremaining . ' days remaining';
            $alertclass = 'alert-danger';
            $icon = 'fa-clock-o';
        } else if ($dueinfo->urgency == 'warning') {
            $text = $dueinfo->daysremaining . ' days remaining';
        ...
        $html = '<div class="alert ' . $alertclass . ' text-center" style="margin:0;border-radius:0;font-size:1.1em;">';
        $html .= '<i class="fa ' . $icon . ' mr-2"></i> ' . $text;
        $html .= '</div>';
