...
/**
 * Renderable class for the action bar selector in the quiz report pages.
 *
 ...
 */
class quiz_report_action_selector implements renderable, templatable {
    ...
    private function get_action_selector(): ?select_menu {
        ...
        if (
            ...
            !has_any_capability(['mod/quiz:viewreports', 'mod/quiz:grade'], $this->context)
        ) {
            return null;
        }
        require_once($CFG->dirroot . '/mod/quiz/report/reportlib.php');

        $reportlist = quiz_report_list($this->context);
        $selects = [];
        foreach ($reportlist as $report) {
            $url = new \moodle_url('/mod/quiz/report.php', ['id' => $this->context->instanceid, 'mode' => $report]);
            $selects[$url->out(false)] = get_string($report, 'quiz_' . $report);
        }
        $selectmenu = new select_menu('reportsactionselect', $selects, $PAGE->url->out(false));
