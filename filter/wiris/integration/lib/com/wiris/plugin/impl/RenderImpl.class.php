...
class com_wiris_plugin_impl_RenderImpl implements com_wiris_plugin_api_Render{
	...
	public function safeMath2Accessible($mml, $lang, $param) {
		try {
			...
			$_ex_ = ($誰申e instanceof HException) ? $誰申e->e : $誰申e;
			$ex = $_ex_;
			{
				return "";
			}
	...
	public function __call($m, $a) {
		...
		else if(isset($this->誰申dynamics[$m]) && is_callable($this->誰申dynamics[$m]))
			return call_user_func_array($this->誰申dynamics[$m], $a);
		else if('toString' == $m)
			return $this->__toString();
		else
			throw new HException('Unable to call 誰申'.$m.'誰申');
	}
