@availability @availability_trainingevent
Feature: availability_trainingevent
  In order to control student access to activities
  As a teacher
  I need to set trainingevent conditions which prevent student access
  ...
  Scenario: Test condition
    ...
    # Start to add a Page. If there aren't any trainingevents, there's no Group option.
    ...
    # Back to course page but add trainingevents.
    Given the following "trainingevents" exist:
    ...
    # Page P1 any trainingevent.
    Given I click on "Group" "button" in the "Add restriction..." "dialogue"
    And I set the field "Group" to "(Any trainingevent)"
    ...
    # Add to trainingevents and log out/in again.
    Given the following "trainingevent members" exist:
      | user     | trainingevent |
      | student1 | GI1   |
    And I log out
    And I log in as "student1"
    ...
    # P1 (any trainingevents) and P2 should show but not P3.
  ...
  Scenario: Condition display with filters
    # Teacher sets up a restriction on trainingevent G1, using multilang filter.
    Given the following "trainingevents" exist:
      | name                                                                                        | course | idnumber |
    ...
    # Student sees information about no access to trainingevent, with trainingevent name in correct language.
