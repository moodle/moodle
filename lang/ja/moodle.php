...
$string['adminhelpauthentication'] = '内部のユーザアカウント又は外部のデータベースを使用できます。';
...
$string['configforceloginforprofiles'] = 'この設定を\'Yes\'にした場合、ユーザプロフィールページを閲覧するにはログインする必要があります。デフォルトでは、\'No\'に設定されますので、学生は事前に各コースの教師のプロフィールを閲覧することができます。同時にサーチエンジンからもプロフィールページにアクセスできることを意味します。';
...
$string['configidnumber'] = 'このオプションでは次の内容を設定します。(a)ユーザがユーザIDの入力を求められない、(b)ユーザがユーザIDを空白にすることができる、又は(c)ユーザはユーザIDの入力を求められ、空白にできない。ユーザIDが入力されたときはプロファイルに表示される。';
