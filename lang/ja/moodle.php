...
$string['allow'] = '許可する';
$string['allowguests'] = 'このコースはゲストユーザも利用出来ます。';
...
$string['clamlostandactinglikevirus'] = '加えて、moodleはclamが動作失敗した時に、ファイルをウイルスとして扱うように設定されています。あなたが修正するまで、基本的に学生はファイルをアップロード出来ないことを意味します。';
$string['clammovedfile'] = 'ファイルは、あなたが指定した隔離用ディレクトリに移動されました。新しい保存場所は、$a です。';
$string['clammovedfilebasic'] = 'ファイルが隔離用ディレクトリに移動されました。';
$string['clamquarantinedirfailed'] = 'あなたが指定した隔離用ディレクトリ $a にファイルを移動出来ませんでした。これを修正しないで、ファイルが感染していることが分かった場合は、そのファイルは削除されます。';
...
$string['confignotifyloginfailures'] = 'ログインに失敗した場合、メールによる通知を行うことが出来ます。誰がメール通知を受け取りますか?';
$string['confignotifyloginthreshold'] = 'ログイン失敗通知が有効の場合、同一ユーザ又はIPアドレスが何回ログインに失敗するとメール通知しますか?';
...
$string['configpathtoclam'] = 'clam AVのパス。恐らく、/usr/bin/clamscan or /usr/bin/clamdscan のようになります。clam AVを正常に動作させる為に、この設定を行う必要があります。';
$string['configproxyhost'] = '<B>サーバ</B>のインターネットアクセスにプロクシ(例 ファイアーウォール)が必要な場合は、プロクシのホスト名とポート番号を入力してください。必要でない場合は空白にしてください。';
$string['configquarantinedir'] = 'clam AVに感染ファイルを隔離用ディレクトリに移動させたい場合は、ここにそのディレクトリを入力してください。このディレクトリは、ウェブサーバより書き込みが出来るようにする必要があります。ここを空白にした場合、又は存在しないか、書き込み権が無いディレクトリを指定した場合、感染ファイルは削除されます。末尾にスラッシュを付けないでください。';
$string['configrunclamonupload'] = 'ファイルアップロード時にclam AVを動作させますか? 正常に動作させる為には、pathtoclamに正しいパスを入力する必要があります。(Clam AVは次の場所で入手出来る無料ウイルススキャナーです。 http://www.clamav.net/)';
$string['configsecureforms'] = 'Moodleでは、フォームデータを受入れる場合の追加的なレベルのセキュリティーを使用することが出来ます。この設定をYesにした場合、ブラウザのHTTP_REFERER変数とフォーム設置のアドレスが照合されます。 ユーザがファイアーウォールソフト(例 Zonealarm)を使用してトラフィックからHTTP_REFERERを取除く設定をしている場合、稀に問題が発生する場合があります。 これはフォーム使用時に画面が固まる現象です。ユーザがログインページ(例えば)で問題を抱えている場合はこの設定をNoにしてください。しかしこの設定ではブルートフォース・パスワードアタックに対して無防備な状態になります。分からない場合は、この設定をYesのままにしてください。';
...
<LI>これでコースにアクセス出来るようになります。今後、あなたが登録したコースにアクセスするために必要なものは、ユーザ名とパスワードのみです。このページにユーザ名とパスワードの入力欄があります。
</OL>';
...
$string['makeeditable'] = '\'$a\' をサーバ上(例 apache)で編集可能にした場合、直接このページより編集出来ます。';
...
$string['uploadfailednotrecovering'] = 'ファイル $a->name に問題がある為、あなたのファイルは正常にアップロードされませんでした。<br /> この問題に関するログです:<br />$a->problem<br />この問題は未解決状態です。';
