...
$string['configcachetext'] = '对于较大的或者是开启了文本过滤器的站点，则个设置可以提高速度。在一段时间之内，已经处理过的文本的副本将会被保留。如果将这个值设得太小会减慢相应速度，但设定得太大也会使文本刷新缓慢。';
$string['configclamactlikevirus'] = '认为文件是病毒';
$string['configclamdonothing'] = '认为文件没问题';
$string['configclamfailureonupload'] = '如果您让clam扫描上传的文件，但并未配置正确clam或它因某些未知原因而运行失败了，那应该怎么做? 如果您选择了“认为文件是病毒”，那么它们将会被转移到隔离区或是被删除。如果您选择了“认为文件没问题”，那么文件将会如往常一样被转移到目标目录中。无论选择哪个，系统都会提醒管理员clam运行失败了。如果您选择“认为文件是病毒”且由于某些原因clam运行失败了(通常是因为您未能指定合法的pathtoclam值)，所有上传的文件将会被转移到隔离区或者被删除。使用这个设置要小心。';
$string['configcountry'] = '如果在此处设定一个国家或地区，则他将被用于用户注册时的缺省国家或地区。要强制用户选择一个国家或的确，保持此处为空白。';
...
$string['configfullnamedisplay'] = '这定义了如何显示名字。对于多数单一语言站点，最有效的设置是缺省的“名 + 姓”，但也可以选择隐藏名或者让当前的语言包来决定(一些语言有不同的习俗)。';
$string['configgdversion'] = '指明已安装的GD版本。默认显示的版本号是 自动探测的结果。不要轻易改变它，除非你清楚你在做什么。';
...
$string['configloginhttps'] = '打开这个选项会让Moodle在登录时使用安全的https连接，登录之后再返回到普通的http方式以保证速度。注意: 这个设定<strong>需 要</strong>Web服务器上的https已经被激活，如果没有，<strong>您可能会将自己锁在站点之外</strong>。';
$string['configloglifetime'] = '在此指定保存用户活动日志的时间长短。超出此时间段的日志将被自动删除。最好是将日志保留尽量长的时间，万一需要他们呢？但如果您的服务器非常繁忙且有性能方面的问题，您也许会希望减少保留日志的时间。';
$string['configlongtimenosee'] = '如果学生很长时间没有登录，系统可以自动将它们除名。此参数指定这个期限。';
$string['configmaxbytes'] = '限制整个站点内上传文件的尺寸。这个设置会受到PHP设定中upload_max_filesize以及Apache设定中LimitRequestBody的影响。这个设置也会影响课程和模块中选择文件尺寸的范围。';
$string['configmaxeditingtime'] = '此参数指定用户在多长时间内可以重新编辑讨论区的贴子、杂志的反馈等等。通常30分钟比较合适。';
$string['configmessaging'] = '是否应当开启在站点用户之间发送短消息的功能?';
$string['confignoreplyaddress'] = '有时电子邮件并非以用户身份发送(如论坛帖子)，还有时用户不希望别人看到自己的电子邮件地址。在这些情况下，您在此处指定的电子邮件地址将会被使用。';
$string['confignotifyloginfailures'] = '如果记录登录失败的信息，可以 通过E-mail将它们发送出去。谁应当收到这些通知呢?';
$string['confignotifyloginthreshold'] = '如果发送失败登录的通知，对于一个用户或者IP地址而言，多少次失败登录之后发送通知呢?';
$string['configopentogoogle'] = '如果使用该设置，Google将能够以访客的身份登陆网站。通过Google搜索而来的访问者也可以以访客的身份登陆网站。这个选项仅对允许访客访问的课程有效。';
...
$string['configthemelist'] = '此项为空则用户可以使用所有的合法主题风格。如果您希望让可选的主题风格列表短一些，您可以在此指定一些。譬如: standard,orangewhiten 。';
