...
$string['configslasharguments'] = '通过使用“斜杠参数”(即这里的第二选项)脚本来提供对文件(图像、上传的资料等)的访问。该方法能让文件更容易地缓存于网页浏览器和代理服务器等。但有些PHP服务器不支持这种方法，所以如果你查看上传的文件或图像有点麻烦，请选择第一个选项。';
...
$string['helpforcetimezone'] = '你可以允许用户选择他自己的时区或者让所有用户只能使用同一时区。';
...
$string['upgradelogsinfo'] = '保存日志的方式最近有些变动。为了能查看每个活动的旧日志，它们必须被升级。这个过程所花费的时间取决于你的网站（可能长达几小时），并且会给大规模网站的数据库带来很大的负荷。一旦您开始了这个工作，就必须让它结束(一直开启浏览器)。不用担心——日志升级过程中，其他人仍然可以正常地使用网站。<br /><br />打算现在升级日志吗?';
