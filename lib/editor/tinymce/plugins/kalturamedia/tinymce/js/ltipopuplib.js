...
function insertMedia() {
    ...
    var url = replaceKafUriWithToken(sourceUrl, kafuri, token);
...
/**
 * @param kafUri - the KAF URI, as configured in Kaltura settings.
 * @param sourceUrl - the URL returned from KAF for playback (BSE).
 * @param kalturaToken - the token which should replace the KAF URI.
 * @returns the url after replacement. E.g., http://1234567.kaf.kaltura.com/browseandembed/media/etc will return as http://kaltura-kaf-uri.com/browseandembed/media/etc.
 */

function replaceKafUriWithToken(sourceUrl, kafUri, kalturaToken) {
    sourceUrl = removeProtocolFromUrl(sourceUrl);
    kafUri = removeProtocolFromUrl(kafUri);
