...
Y.namespace('M.atto_equation').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    initializer: function() {
        // If there is a tex filter active - or if MathJax is loaded in
        // the page - enable this button.
        if (this.get('texfilteractive') || (typeof MathJax !== "undefined")) {
    ...
    _displayDialogue: function() {
        ...
        tabview.render();
        dialogue.show();
        // If MathJax is available, get it to render all the library buttons at once.
        if (typeof MathJax !== "undefined") {
            MathJax.Hub.Queue(["Typeset", MathJax.Hub, library.getDOMNode()]);
        }
    ...
    _updatePreview: function(e) {
        ...
        if (typeof MathJax !== "undefined") {
            // If MathJax is available, get it to update the preview without an ajax request.
            previewNode.setHTML('$$' + equation + '$$');
            MathJax.Hub.Queue(["Typeset", MathJax.Hub, previewNode.getDOMNode()]);
        } else {
            // Fall back on an ajax request to the filter.
    ...
    _getLibraryContent: function() {
        ...
        // Only render the library via ajax is MathJax is not available.
        if (typeof MathJax === "undefined") {
