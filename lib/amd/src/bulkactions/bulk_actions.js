...
export default class BulkActions {
    ...
    async renderBulkActions() {
        ...
        const actions = await Promise.all(
            this.getBulkActions().map(bulkAction => bulkAction.renderBulkActionTrigger())
        );

        // Add rendered actions to data.actions in the correct order.
        actions.forEach(actionTrigger => {
