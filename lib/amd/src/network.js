 ...
 */
define(['jquery', 'core/ajax', 'core/config', 'core/notification', 'core/str', 'core/local/modal/alert', 'core/modal_events'],
        function($, Ajax, Config, Notification, Str, Alert, ModalEvents) {
    ...
    var touchSession = function() {
        ...
        if (sessionTimeout) {
            ...
            return Ajax.call([request], true, true, false, requestTimeout)[0].then(function() {
                ...
                if (alertModal !== null) {
                    alertModal.show().then(() => {
                        if (typeof redirectUrl === 'string') {
                            alertModal.getRoot().on(ModalEvents.hidden, e => {
     ...
     * @param {string|null} redirect A url to redirect to in case of lost connection with server.
     */
