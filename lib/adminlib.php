...
function uninstall_plugin($type, $name)
{
    ...
    $DB->delete_records_select(
        ...
        'handler IN (SELECT id FROM {messageinbound_handlers} WHERE component = ?)',
        array($component)
    );
...
function enable_cli_maintenance_mode()
{
    ...
    if (isset($CFG->maintenance_message) and !html_is_blank($CFG->maintenance_message)) {
        ...
        $data = bootstrap_renderer::early_error_content($data, null, null, null);
        $data = bootstrap_renderer::plain_page(get_string(
            'sitemaintenancetitle',
            'admin',
...
class admin_setting_special_adminseesall extends admin_setting_configcheckbox
{
    ...
    public function __construct()
    {
        parent::__construct(
            ...
            get_string('adminseesall', 'admin'),
            get_string('helpadminseesall', 'admin'),
            '0'
        );
    }
...
class admin_setting_courselist_frontpage extends admin_setting
{
    ...
    public function load_choices()
    {
        ...
        $this->choices = array(
            FRONTPAGENEWS          => get_string('frontpagenews'),
...
class admin_setting_special_frontpagedesc extends admin_setting_confightmleditor
{
    ...
    public function __construct()
    {
        parent::__construct(
            'summary',
            get_string('frontpagedescription'),
            get_string('frontpagedescriptionhelp'),
            null,
            PARAM_RAW,
            60,
            15
...
class admin_setting_special_gradebookroles extends admin_setting_pickroles
{
    ...
    public function __construct()
    {
        parent::__construct(
            'gradebookroles',
            get_string('gradebookroles', 'admin'),
...
class admin_page_pluginsoverview extends admin_externalpage
{
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            'pluginsoverview',
            get_string('pluginsoverview', 'core_admin'),
    ...
    public function output_html($data, $query = '')
    {
        ...
        foreach ($formats as $format) {
            ...
            if ($format->is_enabled()) {
                ...
                if ($defaultformat === $format->name) {
                    ...
                    $hideshow = html_writer::link(
                        $url->out(false, array('action' => 'disable')),
                ...
                $class = 'dimmed_text';
                $hideshow = html_writer::link(
                    $url->out(false, array('action' => 'enable')),
            ...
            if ($cnt < count($formats) - 1) {
                $updown .= '&nbsp;' . html_writer::link(
                    $url->out(false, array('action' => 'down')),
    ...
    public function output_html($data, $query = '')
    {
        ...
        foreach ($plugins as $name => $plugin) {
            ...
            if (!$plugininfo->is_installed_and_upgraded()) {
                ...
                if ($enabled) {
                    $hideshow = html_writer::link(
                        new moodle_url($url, array('action' => 'disable')),
                        $OUTPUT->pix_icon('t/hide', $strdisable, 'moodle', array('class' => 'iconsmall'))
                    );
                } else {
                    $hideshow = html_writer::link(
                        new moodle_url($url, array('action' => 'enable')),
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    $updown = html_writer::link(
                        new moodle_url($url, array('action' => 'up')),
                ...
                if ($updowncount < count($enabledplugins)) {
                    $updown .= html_writer::link(
                        new moodle_url($url, array('action' => 'down')),
...
function admin_get_root($reload = false, $requirefulltree = true)
{
    ...
    // echo '<pre>';
    // var_dump($CFG->dirroot . '/' . $CFG->admin);
    // echo '</pre>';
    ...
    if ($node instanceof admin_category) {
        ...
        foreach ((array)$node->settings as $setting) {
            ...
            if ($error === '') {
                ...
                if (
                    is_int($defaultsetting) || $defaultsetting instanceof lang_string
