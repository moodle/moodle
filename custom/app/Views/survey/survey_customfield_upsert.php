...
try {
    for ($i = 0; $i < count($fieldNames); $i++) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM mdl_survey_customfield WHERE event_id = ?");
        $stmt->execute([$eventId]);
        $fieldIds = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if($id || $id != 0){
            $stmt = $pdo->prepare("UPDATE mdl_survey_customfield 
            SET field_name = ?, 
                name = ?, 
                sort = ?, 
                field_type = ?, 
            ...
            $stmt = $pdo->prepare("INSERT INTO mdl_survey_customfield (event_id, field_name, name, sort, field_type, field_options) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    echo "データが正常に登録されました。";
    echo "<a href='survey_customfield.php?id=$eventId'>編集画面へ戻る</a>";
} catch (PDOException $e) {
