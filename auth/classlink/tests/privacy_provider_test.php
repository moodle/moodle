...
/**
 * Privacy test for auth_classlink
 *
...
use \auth_classlink\privacy\provider;

/**
 * Privacy test for auth_classlink
 *
 * @group auth_classlink
 * @group auth_classlink_privacy
 * @group office365
 * @group office365_privacy
 */
class auth_classlink_privacy_testcase extends \core_privacy\tests\provider_testcase {
    /**
     * Tests set up.
     */
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new core_privacy\local\request\approved_contextlist($user, 'auth_classlink', [$usercontext->id]);
        provider::export_user_data($approvedlist);
        // Token.
        $data = $writer->get_data([
            get_string('privacy:metadata:auth_classlink', 'auth_classlink'),
            get_string('privacy:metadata:auth_classlink_token', 'auth_classlink')
        ]);
        ...
        $data = $writer->get_data([
            get_string('privacy:metadata:auth_classlink', 'auth_classlink'),
            get_string('privacy:metadata:auth_classlink_prevlogin', 'auth_classlink')
        ]);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('auth_classlink_token', []));
        $this->assertCount(2, $DB->get_records('auth_classlink_prevlogin', []));

        // Delete everything for the first user context.
        provider::delete_data_for_all_users_in_context($user1context);

        $this->assertCount(0, $DB->get_records('auth_classlink_token', ['userid' => $user1->id]));
        $this->assertCount(0, $DB->get_records('auth_classlink_prevlogin', ['userid' => $user1->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('auth_classlink_token', []));
        $this->assertCount(1, $DB->get_records('auth_classlink_prevlogin', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user1, 'auth_classlink', [$user1context->id]);
    ...
    static private function create_token(int $userid): \stdClass {
        global $DB;
        $record = new stdClass();
        $record->classlinkuniqid = "user@example.com";
        $record->username = "user@example.com";
        $record->userid = $userid;
        $record->classlinkusername = "user@example.com";
        $record->scope = "All";
        ...
        $record->id = $DB->insert_record('auth_classlink_token', $record);
        return $record;
    }
    ...
    static private function create_prevlogin(int $userid): \stdClass {
        ...
        $record->id = $DB->insert_record('auth_classlink_prevlogin', $record);
        return $record;
    }
