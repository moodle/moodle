...
namespace auth_classlink\privacy;
...
if (interface_exists('\core_privacy\local\request\core_userlist_provider')) {
    interface auth_classlink_userlist extends \core_privacy\local\request\core_userlist_provider {

    }
} else {
    interface auth_classlink_userlist {
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        ...
        $tables = [
            'auth_classlink_prevlogin' => [
                'userid',
                'method',
                'password',
            ],
            'auth_classlink_token' => [
                'classlinkuniqid',
                'username',
                'userid',
                'classlinkusername',
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT ctx.id
                  FROM {auth_classlink_prevlogin} pv
    ...
    public static function get_users_in_context(\core_privacy\local\request\userlist $userlist) {
        ...
        $sql = "SELECT ctx.instanceid as userid
                  FROM {auth_classlink_prevlogin} pl
                  JOIN {context} ctx
        ...
        foreach ($tables as $table => $filterparams) {
            ...
            foreach ($records as $record) {
                writer::with_context($context)->export_data([
                    get_string('privacy:metadata:auth_classlink', 'auth_classlink'),
                    get_string('privacy:metadata:'.$table, 'auth_classlink')
                ], $record);
            }
        }
    }
    ...
    protected static function get_table_user_map(\stdClass $user): array {
        $tables = [
            'auth_classlink_prevlogin' => ['userid' => $user->id],
            'auth_classlink_token' => ['userid' => $user->id],
        ];
        return $tables;
    }
    ...
    private static function delete_user_data(int $userid) {
        global $DB;
        $DB->delete_records('auth_classlink_prevlogin', ['userid' => $userid]);
        $DB->delete_records('auth_classlink_token', ['userid' => $userid]);
    }
