...
class utils {
    ...
    /**
     * Process an classlink JSON response.
     *
    ...
    public static function process_json_response($response, array $expectedstructure = array()) {
        ...
        if (empty($result) || !is_array($result)) {
            ...
            throw new \moodle_exception('errorclasslinkcall', 'auth_classlink');
        }
        ...
        if (isset($result['error'])) {
            ...
            if (isset($result['error_description'])) {
                throw new \moodle_exception('errorclasslinkcall_message', 'auth_classlink', '', $result['error_description']);
            } else {
                throw new \moodle_exception('errorclasslinkcall', 'auth_classlink');
            }
        }
    ...
    public static function debug($message, $where = '', $debugdata = null) {
        $debugmode = (bool)get_config('auth_classlink', 'debugmode');
        if ($debugmode === true) {
            $fullmessage = (!empty($where)) ? $where : 'Unknown function';
            $fullmessage .= ': '.$message;
            $fullmessage .= ' Data: '.static::tostring($debugdata);
            $event = \auth_classlink\event\action_failed::create(['other' => $fullmessage]);
            $event->trigger();
        }
    }
