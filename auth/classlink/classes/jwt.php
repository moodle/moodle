...
class jwt {
    ...
    public static function decode($encoded) {
        if (empty($encoded) || !is_string($encoded)) {
            throw new \moodle_exception('errorjwtempty', 'auth_classlink');
        }
        $jwtparts = explode('.', $encoded);
        if (count($jwtparts) !== 3) {
            throw new \moodle_exception('errorjwtmalformed', 'auth_classlink');
        }
        ...
        if (empty($header) || !is_array($header)) {
            throw new \moodle_exception('errorjwtcouldnotreadheader', 'auth_classlink');
        }
        if (!isset($header['alg'])) {
            throw new \moodle_exception('errorjwtinvalidheader', 'auth_classlink');
        }
        ...
        if (in_array($header['alg'], $jwsalgs, true) === true) {
            ...
            throw new \moodle_exception('errorjwtunsupportedalg', 'auth_classlink');
        }

        if (empty($body) || !is_array($body)) {
            throw new \moodle_exception('errorjwtbadpayload', 'auth_classlink');
        }
     ...
     * @return \auth_classlink\jwt A JWT instance.
     */
