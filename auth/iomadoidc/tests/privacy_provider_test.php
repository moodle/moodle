...
class auth_iomadoidc_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('auth_iomadoidc_token', []));
        $this->assertCount(2, $DB->get_records('auth_iomadoidc_prevlogin', []));

        // Delete everything for the first user context.
        provider::delete_data_for_all_users_in_context($user1context);

        $this->assertCount(0, $DB->get_records('auth_iomadoidc_token', ['userid' => $user1->id]));
        $this->assertCount(0, $DB->get_records('auth_iomadoidc_prevlogin', ['userid' => $user1->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('auth_iomadoidc_token', []));
        $this->assertCount(1, $DB->get_records('auth_iomadoidc_prevlogin', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user1, 'auth_iomadoidc', [$user1context->id]);
    ...
    static private function create_token(int $userid) : \stdClass {
        ...
        $record->id = $DB->insert_record('auth_iomadoidc_token', $record);
        return $record;
    }
