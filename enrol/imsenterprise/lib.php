...
class enrol_imsenterprise_plugin extends enrol_plugin {
    ...
    public function cron() {
        ...
        if (file_exists($filename)) {
            ...
            // NB We'll write this value back to the database at the end of the cron.
     ...
     */
    public function full_tag_found_in_cache($tagname, $latestline) { // Return entire element if found. Otherwise return false.
                    ...
                    if (!$createnewcourses) {
                        ...
                        foreach ($this->coursemappings as $courseattr => $imsname) {
                            ...
                            if ($imsname == 'coursecode') {
                                ...
                                $msg = 'No '.$imsname.' description tag found for '.$coursecode;
                                $msg .= ' coursecode, using '.$coursecode.' instead.';
                                $this->log_line($msg);
                                $course->{$courseattr} = $coursecode;
                            }
                        }
                        ...
                        if (strlen($group->category) > 0) {
                            ...
                            // one and create it if necessary and allowed.
                            $catid = 0;
                            $t_catname = '';
        ...
        if ($recstatus == 3) {
            ...
            if (!$DB->get_field('user', 'id', array('idnumber'=>$person->idnumber)) && $createnewusers) {
                ...
                if ((!isset($person->username)) || (strlen($person->username)==0)) {
                    $msg = 'Cannot create new user for ID#'.$person->idnumber;
                    $msg .= ' No username listed in IMS data for this person.';
                    $this->log_line($msg);
                    ...
                    // If they don't exist and they have a defined username,
                    // and $createnewusers == true, we create them.
                    ...
                    Photo processing is deactivated until we hear from Moodle dev forum about modification to gdlib.
                                 ...
                                 if($CFG->enrol_processphoto){
                                   ...
                                   if ($usernew->picture = save_profile_image($id, $person->urlphoto,'user')) {
                                       // TODO: use process_new_icon() instead.
        ...
        if ($ship->courseid && preg_match_all('{<member>(.*?)</member>}is', $tagcontents, $membermatches, PREG_SET_ORDER)) {
            ...
            foreach ($membermatches as $mmatch) {
                ...
                if (preg_match('{<role\s+roletype=["\'](.+?)["\'].*?>}is', $mmatch[1], $matches)) {
                    ...
                    // The XML that comes out of Capita Student Records
                ...
                if ($memberstoreobj->userid) {
                    ...
                    if (!$moodleroleid) {
                        $msg = "SKIPPING role $member->roletype for $memberstoreobj->userid ";
                        $msg .= "($member->idnumber) in course $memberstoreobj->course";
                        $this->log_line($msg);
                        continue;
                    }
                    ...
                    if (intval($member->status) == 1) {
                        ...
                        $einstance = $DB->get_record('enrol', array(
                        ...
                        $msg = "Enrolled user #$memberstoreobj->userid ($member->idnumber) to";
                        $msg .= " role $member->roletype in course $memberstoreobj->course";
                        $this->log_line($msg);
                        $memberstally++;
                        ...
                        if (isset($member->groupname)) {
                            ...
                            if (isset($groupids[$member->groupname])) {
                                ...
                                $member->groupid = $groupids[$member->groupname];
                            } else {
                                if ($groupid = $DB->get_field('groups', 'id',
                                        array('courseid'=>$ship->courseid, 'name'=>$member->groupname))) {
    ...
    /**
     * Pass me the INNER CONTENTS of a <timeframe> tag - beginning and/or
     * ending is returned, in unix time, zero indicating not specified
     */
    public function decode_timeframe($string) {
        $ret = new stdClass();
        $ret->begin = $ret->end = 0;
     ...
     */
    public function load_role_mappings() {
     ...
     */
    public function load_course_mappings() {
        require_once('locallib.php');
