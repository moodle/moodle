...
/**
 * One Roster Teachers for Class in School collection.
 *
 * @package    enrol_oneroster
 * @copyright  Andrew Nicols <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class teachers_for_class_in_school extends teachers {
    ...
    protected static function get_operation_id(container_interface $container): string {
        if ($container->supports(rostering_endpoint::getTeachersForClassInSchool)) {
            return rostering_endpoint::getTeachersForClassInSchool;
        }
    ...
    protected function process_params(array $params): array {
        if (!$this->container->supports(rostering_endpoint::getTeachersForClassInSchool)) {
