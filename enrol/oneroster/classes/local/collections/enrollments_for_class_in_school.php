 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrollments_for_class_in_school extends enrollments {
    ...
    protected static function get_operation_id(container_interface $container): string {
        if ($container->supports(rostering_endpoint::getEnrollmentsForClassInSchool)) {
            return rostering_endpoint::getEnrollmentsForClassInSchool;
        }
    ...
    protected function process_params(array $params): array {
        if (!$this->container->supports(rostering_endpoint::getEnrollmentsForClassInSchool)) {
