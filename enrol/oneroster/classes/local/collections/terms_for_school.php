 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class terms_for_school extends terms {
    ...
    protected static function get_operation_id(container_interface $container): string {
        if ($container->supports(rostering_endpoint::getTermsForSchool)) {
            return rostering_endpoint::getTermsForSchool;
        }

        return rostering_endpoint::getAllTerms;
    }
    ...
    protected function process_params(array $params): array {
        if (!$this->container->supports(rostering_endpoint::getTermsForSchool)) {
