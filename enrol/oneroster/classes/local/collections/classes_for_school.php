 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class classes_for_school extends classes {
    ...
    protected static function get_operation_id(container_interface $container): string {
        if ($container->supports(rostering_endpoint::getClassesForSchool)) {
            return rostering_endpoint::getClassesForSchool;
        }
    ...
    protected function process_params(array $params): array {
        if (!$this->container->supports(rostering_endpoint::getClassesForSchool)) {
            // When the getClassesForSchool endpoint is not supported it is still possible to use the getClasses
