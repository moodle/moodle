...
function temps_activite ($id, $course) {
 ...
 if( isset($_GET['selectGroup']) )  {
    ...
    $sql_utilisateur =  "SELECT user.id, user.username, user.email, user.lastname, user.firstname, user.phone1, 
	...
	while($donnees = $reponse2->fetch())    {
		...
		$t = explode( "#", $donnees['list_courses'] );
echo $donnees['list_courses'] . '<pre>';
print_r( $t );
echo '</pre>';
		
		
		foreach($t as $courseid)	{
echo '<br />';
var_dump($donnees['id'], $courseid);
echo '<br />';
			$temps_eq=temps_activite($donnees['id'], $courseid);

			// echo "temps: ".$donnees['id']." ".$courseid;
			// exit();
		
			$PreNomStagiaire = $donnees['lastname'];
			$NomStagiaire = $donnees['firstname'];
			$tef = seconds_to_hours($temps_eq);
	// var_dump($donnees['name']);	exit;
			// $RefSession = substr($donnees['name'] , -4);
			$t = explode('_', $donnees['name']);
			$RefSession = $t[1];

			$sql_nom_course = "SELECT shortname FROM `mdl_course` WHERE id = ".$courseid;
			$requete = $bdd->prepare($sql_nom_course);
			$requete->execute();

			$nom_course = $requete->fetch();
			if ($temps_eq != 0) {
				$lien_total .= '&id'.$i.'='.$donnees['id'].'&course'.$i.'='.$courseid.'&PreNomStagiaire'.$i.'=' . $PreNomStagiaire . '&NomStagiaire'.$i.'=' . $NomStagiaire . '&tef'.$i.'=' . $tef . '&RefSession'.$i.'=' . $RefSession;
				$i++;
			}
		}
	}
