...
if (isset( $_POST['userid'], $_POST['courseid'], $_POST['dated'], $_POST['datef'] ))	{
	// $str.=liste_detail($_POST['id'.$i], $_POST['course'.$i]);
	include_once '../../blocks/use_stats/locallib.php';
	
	$logs = use_stats_extract_logs($_POST['dated'], $_POST['datef'], $_POST['userid'], $_POST['courseid']);
...
// print_r( $log );
// echo '</pre>';
		if ( $log->action == 'viewed' && $log->target == 'course' )	{
			$tms[] = $log->time;
		}
	}
	
	if( ! count($tms) )	echo 'Pas de connexion';
	else echo 'tmsPremiereConnexion#' . min($tms) . "#" . max($tms);
