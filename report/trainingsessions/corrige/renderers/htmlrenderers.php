...
function report_trainingsessions_print_html(&$str, $structure, &$aggregate, &$done, $indent = '', $level = 0) {
    ...
    if (is_siteadmin()) //modif Rayan 29/07/2020, n'afficher que les sessions de travail pour les stagiaires
    {
        if (!$titled )  {
            $titled = true;
...
/*    $logs = use_stats_extract_logs($data->from, $data->to, $data->userid, $courseid);
    ...
    if (!$short) {
        ...
        //Rayan le 15/07 - On cache le temps dans les activités pour n'afficher que le temps total dans le cours
        ...
        if (in_array('elapsed', $cols)) {
            $str .= '<br/><b><span id="sample-elapsed" class="trainingsessions-main-total">';
...
//modif Rayan 15/07, pas besoin d'afficher les sessions de travail toujours à zéro
...
function report_trainingsessions_print_header_elapsed($userid, $courseid, $data, $short = false, $withcompletion = true,
                                                   $withnooutofstructure = false) {
    global $DB, $OUTPUT;
...
// ----------------------- HADRIEN 08/01/19 si admin
        if (in_array('elapsed', $cols) && is_siteadmin()) {
            ...
            $str .= get_string('coursetotaltime', 'report_trainingsessions');
            $str .= ':</span></b> '.report_trainingsessions_format_time(0 + @$data->elapsed, $durationformat);
            $totalelapsed = report_trainingsessions_format_time(0 + @$data->elapsed,$durationformat);
            if (is_siteadmin()) {
                $str .= ' ('.(0 + @$data->hits).')';
            }
            $str .= $OUTPUT->help_icon('coursetotaltime', 'report_trainingsessions');
...
function report_trainingsessions_print_session_list_elapsed( $sessions, $courseid = 0, $userid = 0) {
    global $OUTPUT, $CFG;
    ...
    if(!empty($config->printsessiontotal)){
        $elapsedTimeSession = report_trainingsessions_format_time($totalelapsed, $durationformat);
    }

    return $elapsedTimeSession;
}
