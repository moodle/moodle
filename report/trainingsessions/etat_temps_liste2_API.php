...
// exit;
    ...
    function training_reports_print_session_list_frank(&$str, $sessions, $courseid = 0){
                ...
                $str .= '<table width="100%" style="border:none;">';
                $str .= '<tr valign="top" style="border:none;">';
                $str .= '<td width="33%" style="border:none;"><b>Début de session</b></td>';
                $str .= '<td width="33%" style="border:none;"><b>Fin de session</b></td>';
                $str .= '<td width="33%" style="border:none;"><b>Durée<sup>*</sup></b></td>';
                $str .= '</tr>';
                ...
                foreach($sessions as $s){
                    ...
                    $str .= '<tr valign="top"  style="border:none;">';
                    $str .= '<td style="border:none;">'.date_fr(date("D j M Y G:i",$s->sessionstart)).'</td>';
                    $str .= '<td style="border:none;">'.date_fr(date("D j M Y G:i",$sessionenddate)).'</td>';
                    $str .= '<td style="border:none;">'.seconds_to_hours(@$s->elapsed).'</td>';
                    $str .= '</tr>';
    ...
    function liste_detail ($userid, $courseid) {
        ...
        // print result
            ...
            if ($data->output == 'html'){
                ...
                $str .= training_reports_print_session_list_frank($str, @$aggregate['sessions'], $courseid);
    ...
    function print_etat ($i) {
        ...
        $PreNomStagiaire = $_POST['PreNomStagiaire'.$i];
        $NomStagiaire = $_POST['NomStagiaire'.$i];
        $NomComplet = $PreNomStagiaire . ' ' . $NomStagiaire;
        
        $doss = $_POST['doss'];
        $RefSession = $_POST['RefSession'.$i];
        $tef = $_POST['tef'.$i];
        $tabloTemp = explode(":", $tef);
        $tef = $tabloTemp[0] . ":" . $tabloTemp[1];
       ...
       $str= "
        ...
        $str.=liste_detail($_POST['id'.$i], $_POST['course'.$i]);
...
// exit(' I = ' .$i );		
    while ($i>=0) {
        // $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Relevé de '.$_POST['NomStagiaire'.$i], "Session: ".$_POST['RefSession0'], array(0,64,255), array(0,64,128));
    ...
    // $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Relevé de '.$_POST['NomStagiaire1'], "Session: ".$_POST['RefSession1'], array(0,64,255), array(0,64,128));
    // $pdf->setHeaderTemplateAutoreset(true);
    // $pdf->AddPage();
    $html = print_etat(0);
    // $pdf->writeHTML($html, true, false, false, false, '');
...
// $pdf->Output('Releve_heures_session_'.$_POST['RefSession0'].'.pdf', 'I');
