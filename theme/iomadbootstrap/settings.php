 ...
 * Theme version info
 *
 * @package    theme_iomadbootstrapbase
...
if ($ADMIN->fulltree) {
    ...
    $simplesettings = new simple_theme_settings($settings, 'theme_iomadbootstrap');

    $simplesettings->add_file('logo');

    $simplesettings->add_checkbox('fluidwidth', 1, 1);
    ...
    foreach (range(100, 900, 100) as $weight) {
        $fontweights[$weight] = get_string("fontweight$weight", 'theme_iomadbootstrap');
    }
