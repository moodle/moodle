...
/**
 * Test the iomadboostnavbar file
 *
 * @package    theme_iomadboost
 * @covers     \theme_iomadboost\iomadboostnavbar
 * @copyright  2022 Derick Turner
 * @author    Derick Turner
 * @based on theme_boost by Peter Dias
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class iomadboostnavbar_test extends \advanced_testcase {
    /**
    ...
    public function test_remove_no_link_items(array $setup, bool $removesectionnodes, array $expected) {
        ...
        $iomadboostnavbar = $this->getMockBuilder(iomadboostnavbar::class)
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();

        $rc = new \ReflectionClass(iomadboostnavbar::class);
        $rcp = $rc->getProperty('items');
        $rcp->setAccessible(true);
        $rcp->setValue($iomadboostnavbar, $page->navbar->get_items());

        // Make the call to the function.
        $rcm = $rc->getMethod('remove_no_link_items');
        $rcm->setAccessible(true);
        $rcm->invoke($iomadboostnavbar, $removesectionnodes);

        // Get the value for the class variable that the function modifies.
        $values = $rcp->getValue($iomadboostnavbar);
        $actual = [];
    ...
    public function test_remove_items_that_exist_in_navigation(string $navmenu, array $navmenunodes, array $navbarnodes,
        ...
        $rcm->setAccessible(true);
        $rcm->invoke($iomadboostnavbar, $navigationmenu);
