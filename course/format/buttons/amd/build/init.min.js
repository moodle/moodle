...
function tooltipEvents(){var tooltips=$('.section-tooltip');for(var i=0;i<tooltips.length;i++){var item=tooltips[i];item.addEventListener('click',function(){$('#section'+this.dataset.section+' .label-content-wrapper').prepend('<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert">&times;</button><strong>Section tooltip: </strong>'+this.dataset.info+'</div>');});}}
...
if(vert){slides=5;for(var i=0;i<brakepoints.length;i++){if(brakepoints[i]==768){brp={breakpoint:brakepoints[i],settings:'unslick'};resp.push(brp);}else{brp={breakpoint:brakepoints[i],settings:{slidesToShow:slides,slidesToScroll:1,rtl:false,}};resp.push(brp);}}}else{slides=4;for(var i=0;i<brakepoints.length;i++){brp={breakpoint:brakepoints[i],settings:{slidesToShow:slides-i-1,slidesToScroll:1,rtl:dir,}};resp.push(brp);}}
var slickConfig={dots:false,autoplay:false,arrows:true,vertical:vert,verticalSwiping:vert,rtl:dir,slidesToShow:slides,slidesToScroll:1,responsive:resp,};elem.slick(slickConfig);}
