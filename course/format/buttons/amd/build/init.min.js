...
function initSlider(elem,vert){var dir,resp=[],brakepoints=[1200,992,768],brp,slides=4;(document.dir=="rtl")?dir=true:dir=false;if(vert!==undefined){vert=true;dir=false}else{vert=false;}
if(vert){for(var i=0;i<brakepoints.length;i++){if(brakepoints[i]==768){brp={breakpoint:brakepoints[i],settings:'unslick'};resp.push(brp);}else{brp={breakpoint:brakepoints[i],settings:{slidesToShow:slides,slidesToScroll:1,rtl:false,}};resp.push(brp);}}}else{for(var i=0;i<brakepoints.length;i++){brp={breakpoint:brakepoints[i],settings:{slidesToShow:--slides,slidesToScroll:1,rtl:dir,}};resp.push(brp);}}
