...
class field_controller extends \core_customfield\field_controller {
    /**
     * Plugin type colourpicker
     */
    const TYPE = 'colourpicker';

    /**
     * Add fields for editing a colourpicker field.
     *
     * @param \MoodleQuickForm $mform
     */
    public function config_form_definition(\MoodleQuickForm $mform) {

        $mform->addElement('header', 'header_specificsettings', get_string('specificsettings', 'customfield_colourpicker'));
        ...
        $mform->addElement('text', 'configdata[displaysize]', get_string('displaysize', 'customfield_colourpicker'), ['size' => 6]);
        ...
        $mform->addElement('text', 'configdata[maxlength]', get_string('maxlength', 'customfield_colourpicker'), ['size' => 6]);
        ...
        $mform->addElement('selectyesno', 'configdata[ispassword]', get_string('ispassword', 'customfield_colourpicker'));
        $mform->setType('configdata[ispassword]', PARAM_INT);

        $mform->addElement('text', 'configdata[link]', get_string('islink', 'customfield_colourpicker'), ['size' => 50]);
        $mform->setType('configdata[link]', PARAM_RAW_TRIMMED);
        $mform->addHelpButton('configdata[link]', 'islink', 'customfield_colourpicker');

        $mform->disabledIf('configdata[link]', 'configdata[ispassword]', 'eq', 1);

        $linkstargetoptions = array(
            ''       => get_string('none', 'customfield_colourpicker'),
            '_blank' => get_string('newwindow', 'customfield_colourpicker'),
            '_self'  => get_string('sameframe', 'customfield_colourpicker'),
            '_top'   => get_string('samewindow', 'customfield_colourpicker')
        );
        $mform->addElement('select', 'configdata[linktarget]', get_string('linktarget', 'customfield_colourpicker'),
            $linkstargetoptions);
    ...
    public function config_form_validation(array $data, $files = array()) : array {
        ...
        if ($maxlength < 1 || $maxlength > 1333) {
            $errors['configdata[maxlength]'] = get_string('errorconfigmaxlen', 'customfield_colourpicker');
        }

        $displaysize = (int)$data['configdata']['displaysize'];
        if ($displaysize < 1 || $displaysize > 200) {
            $errors['configdata[displaysize]'] = get_string('errorconfigdisplaysize', 'customfield_colourpicker');
        }
        ...
        if (isset($data['configdata']['link'])) {
            ...
            if (strlen($link)) {
                ...
                if (strpos($link, '$$') === false) {
                    $errors['configdata[link]'] = get_string('errorconfiglinkplaceholder', 'customfield_colourpicker');
                    ...
                    $errors['configdata[link]'] = get_string('errorconfiglinksyntax', 'customfield_colourpicker');
                }
            }
        }
